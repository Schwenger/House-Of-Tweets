	@politicians = {
		"0": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Jan Paul van Aken (* 1. Mai 1961 in Reinbek) ist ein deutscher Aktivist f\u00fcr Greenpeace und Politiker (Die Linke). Er ist Mitglied des Ausw\u00e4rtigen Ausschusses und des Unterausschusses f\u00fcr Abr\u00fcstung, R\u00fcstungskontrolle und Nichtverbreitung im Deutschen Bundestag. Van Aken zog 2009 \u00fcber die Hamburger Landesliste seiner Partei in den 17. Deutschen Bundestag ein. Von 2012 bis 2014 war er stellvertretender Parteivorsitzender der Partei Die Linke. Er war einer der acht Spitzenkandidaten seiner Partei f\u00fcr die Bundestagswahl 2013 und wurde durch diese in den 18. Deutschen Bundestag gew\u00e4hlt.",
				"en": "Jan Paul van Aken (born 1 May 1961 in Reinbek) is a German activist for Greenpeace and a politician with the Left Party. He is a member of the Foreign Affairs Committee and the Subcommittee on Disarmament, Arms Control and Non-Proliferation in the German Bundestag. Jan van Aken entered the 17th Bundestag in 2009 after he was listed on the Left electoral lists in Hamburg. Since June 2012 he has been a deputy chairperson of the Left Party. He was one of the eight lead candidates for the Left in the 2013 federal election and was therefore also elected to the 18th Bundestag."
			},
			"full_name": "Jan van Aken",
			"images": {
				"license": "custom-linke",
				"pathToImage": "0.jpg",
				"pathToThumb": "0_t.jpg"
			},
			"name": "Jan van Aken",
			"party": "DIE LINKE",
			"pid": "0",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": 936486542,
				"twitterUserName": "jan_vanaken"
			}
		},
		"101": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Bernd-Bernhard Fabritius (* 14. Mai 1965 in Agnetheln, Rum\u00e4nien) ist ein deutscher Politiker (CSU) und Pr\u00e4sident des Bundes der Vertriebenen. Der promovierte Jurist und Politologe ist seit 2013 Mitglied des Deutschen Bundestages und seit 2007 Bundesvorsitzender des Verbandes der Siebenb\u00fcrger Sachsen in Deutschland e.\u00a0V. und Pr\u00e4sident der weltweiten F\u00f6deration der Siebenb\u00fcrger Sachsen.",
				"en": "Bernd Fabritius (born 14 May 1965) is a German politician from Bavaria who is currently serving as a Member of the Bundestag for the Christian Social Union of Bavaria (CSU)."
			},
			"full_name": "Dr. Bernd Fabritius",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "101.jpg",
				"pathToThumb": "101_t.jpg"
			},
			"name": "Bernd Fabritius",
			"party": "CSU",
			"pid": "101",
			"self_bird": "kiwi",
			"twittering": {
				"twitterId": 493229926,
				"twitterUserName": "BerndFabritius"
			}
		},
		"104": {
			"citizen_bird": "heckenbraunelle",
			"cv": {
				"de": "Uwe Wolfgang Werner Feiler (* 2. November 1965 in Luhdorf bei Winsen (Luhe)) ist ein deutscher Politiker (CDU).",
				"en": "Uwe Feiler is a German politician. He is a member of the CDU."
			},
			"full_name": "Uwe Feiler",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "104.jpg",
				"pathToThumb": "104_t.jpg"
			},
			"name": "Uwe Feiler",
			"party": "CDU",
			"pid": "104",
			"self_bird": "bachstelze",
			"twittering": {
				"twitterId": 47151012,
				"twitterUserName": "UweFe"
			}
		},
		"105": {
			"citizen_bird": "rotkehlchen",
			"cv": {
				"de": "Thomas Wolfgang Feist (* 8. April 1965 in Leipzig) ist ein deutscher Politiker der CDU. Der Musik- und Kulturwissenschaftler war Referent f\u00fcr Kulturarbeit, interkulturelle Arbeit und Musik bei der evangelischen Landeskirche Sachsen und auch Vorsitzender des Bundesverband Kulturarbeit in der evangelischen Jugend.",
				"en": "Thomas Wolfgang Feist (born 8 April 1965 in Leipzig, Germany) is a German politician (CDU) as well as musicologist and cultural scientist. Thomas Feist is a member of the parliamentary association German-Israeli Parliamentary Friendship Group and German-Iranian Parliamentary Friendship Group."
			},
			"full_name": "Dr. Thomas Feist",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "105.jpg",
				"pathToThumb": "105_t.jpg"
			},
			"name": "Thomas Feist",
			"party": "CDU",
			"pid": "105",
			"self_bird": "stieglitz",
			"twittering": {
				"twitterId": 47990090,
				"twitterUserName": "drthomasfeist"
			}
		},
		"108": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Elke Ferner, geb. Widua, (* 5. Mai 1958 in Idar-Oberstein) ist eine deutsche Politikerin (SPD). Seit Dezember 2013 ist sie Parlamentarische Staatssekret\u00e4rin bei der Bundesministerin f\u00fcr Familie, Senioren, Frauen und Jugend.",
				"en": "Elke Ferner is a German politician. She is a member of the SPD."
			},
			"full_name": "Elke Ferner",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "108.jpg",
				"pathToThumb": "108_t.jpg"
			},
			"name": "Elke Ferner",
			"party": "SPD",
			"pid": "108",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 26203837,
				"twitterUserName": "Elke_Ferner"
			}
		},
		"111": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Axel Eduard Fischer (* 5. Mai 1966 in Karlsruhe) ist ein deutscher Politiker (CDU).  Seit 1998 ist er Mitglied des Deutschen Bundestages.  Fischer ist seit 2009 Mitglied des Haushaltsausschusses, Berichterstatter der CDU/CSU f\u00fcr den Etat Arbeit und Soziales und Mitglied des Fraktionsvorstandes der CDU/CSU-Bundestagsfraktion. Au\u00dferdem ist er stellvertretendes Mitglied des Ausw\u00e4rtigen Ausschusses und des Ausschusses f\u00fcr Angelegenheiten der Europ\u00e4ischen Union.  Fischer leitete von Mai 2010 bis April 2013 die Enquete-Kommission \"Internet und digitale Gesellschaft\" des Bundestages.",
				"en": "Axel Eduard Fischer (born May 5, 1966 in Karlsruhe, Baden-W\u00fcrttemberg) is a German politician. He is a member of the CDU and has been a member of the German parliament since 1998, representing Karlsruhe-Land since 2002. He is married for the second time and has six children.  Axel E. Fischer is Member of the Council of Europe Parliamentary Assembly and the Western European Union. On January 1, 2009, Fischer succeeded Edward O'Hara as Chairman of the committee for Technology and Aerospace."
			},
			"full_name": "Axel E. Fischer",
			"images": {
				"copyright": "Axel Fischer",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "111.jpg",
				"pathToThumb": "111_t.jpg"
			},
			"name": "Axel E. Fischer",
			"party": "CDU",
			"pid": "111",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": "160906608",
				"twitterUserName": "Axel_Fischer"
			}
		},
		"114": {
			"citizen_bird": "kohlmeise",
			"cv": {
				"de": "Christian Flisek (* 15. April 1974 in Bochum) ist ein deutscher Politiker (SPD) und Abgeordneter im 18. Deutschen Bundestag.  Nachdem Flisek 2012 als Bundestagskandidat der SPD in Passau nominiert worden war, zog er nach der Bundestagswahl 2013 erfolgreich in den 18. Bundestag ein.  Flisek belegt zahlreiche politische \u00c4mter. Unter anderem ist er Obmann der SPD im NSA-Untersuchungsausschuss; er h\u00e4lt eine Befragung von Edward Snowden in Berlin f\u00fcr rechtlich m\u00f6glich. Des Weiteren ist er Beauftragter f\u00fcr Existenzgr\u00fcndungen der SPD-Bundestagsfraktion und Bundesvorsitzender der Arbeitsgemeinschaft der Selbst\u00e4ndigen in der SPD. Seit 2014 ist Christian Flisek Stadtrat in Passau.",
				"en": "Christian Flisek is a German politician. He is a member of the SPD."
			},
			"full_name": "Christian Flisek",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "114.jpg",
				"pathToThumb": "114_t.jpg"
			},
			"name": "Christian Flisek",
			"party": "SPD",
			"pid": "114",
			"self_bird": "schwanzmeise",
			"twittering": {
				"twitterId": 2469449068,
				"twitterUserName": "ChristianFlisek"
			}
		},
		"117": {
			"citizen_bird": "kohlmeise",
			"cv": {
				"de": "Edgar Konrad Hartmut Franke (* 21. Januar 1960 in Gudensberg) ist ein deutscher Politiker (SPD). Seit 2009 ist er Abgeordneter des Deutschen Bundestages, wo er seit 2014 Vorsitzender des Ausschusses f\u00fcr Gesundheit ist.  Franke ist stellvertretender Vorsitzender der Kurt-Schumacher-Gesellschaft, Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag und im Vorstand der Deutschen Parlamentarischen Gesellschaft, au\u00dferdem Mitglied in der Deutsch-T\u00fcrkischen Gesellschaft und in der entsprechenden Parlamentariergruppe.",
				"en": "Edgar Franke is a German politician. He is a member of the SPD."
			},
			"full_name": "Dr. Edgar Franke",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "117.jpg",
				"pathToThumb": "117_t.jpg"
			},
			"name": "Edgar Franke",
			"party": "SPD",
			"pid": "117",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": 481393623,
				"twitterUserName": "FrankeEdgar"
			}
		},
		"118": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Ulrich Ronald Freese (* 12. April 1951 in Drevenack, damals Kreis Rees, Nordrhein-Westfalen) ist ein deutscher Gewerkschafter (IG BCE), Politiker (SPD) und Kohlelobbyist.  2013 zog er \u00fcber die brandenburgische Landesliste in den Deutschen Bundestag ein. Freese ist im 18. Deutschen Bundestag Mitglied im Ausschuss f\u00fcr Wirtschaft und Energie, dem Ausschuss f\u00fcr Gesundheit, Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft sowie dem Haushaltsausschuss. Ulrich Freese ist als Abgeordneter des Wahlkreises Cottbus/Spree-Nei\u00dfe Mitglied der SPD-Landesgruppe Brandenburg.",
				"en": "Ulrich Freese is a German politician. He is a member of the SPD."
			},
			"full_name": "Ulrich Freese",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "118.jpg",
				"pathToThumb": "118_t.jpg"
			},
			"name": "Ulrich Freese",
			"party": "SPD",
			"pid": "118",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 2317202160,
				"twitterUserName": "UliFreese"
			}
		},
		"12": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Annalena Charlotte Alma Baerbock (* 15. Dezember 1980 in Hannover) ist eine deutsche Politikerin der Gr\u00fcnen und Bundestagsabgeordnete. Sie ist Mitglied des Parteirats von B\u00fcndnis 90/Die Gr\u00fcnen und war von 2009 bis 2013 Vorsitzende des Landesverbands Brandenburg. Von November 2012 bis November 2015 geh\u00f6rte sie dem 16-k\u00f6pfigen Parteirat von B\u00fcndnis 90/Die Gr\u00fcnen an.[2] Am 26. Januar 2013 wurde sie zur Direktkandidatin f\u00fcr den Bundestagswahlkreis Potsdam \u2013 Potsdam-Mittelmark II \u2013 Teltow-Fl\u00e4ming II zur Bundestagswahl 2013 gew\u00e4hlt.  Am 26. November 2016 wurde sie erneut als Kandidatin zur Bundestagswahl 2017 gew\u00e4hlt.",
				"en": "Annalena Baerbock is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Annalena Baerbock",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "12.jpg",
				"pathToThumb": "12_t.jpg"
			},
			"name": "Annalena Baerbock",
			"party": "GR\u00dcNE",
			"pid": "12",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 2179010672,
				"twitterUserName": "ABaerbock"
			}
		},
		"121": {
			"citizen_bird": "gartenrotschwanz",
			"cv": {
				"de": "Astrid Freudenstein (* 9. Oktober 1973 in Bad Griesbach im Rottal) ist eine deutsche Politikerin (CSU).  Astrid Freudenstein ist stellvertretende Vorsitzende des CSU-Kreisverbandes Regensburg-Stadt und Mitglied im CSU-Bezirksvorstand Oberpfalz. Dar\u00fcber hinaus ist sie Mitglied im Landesvorstand der Frauen-Union Bayern.",
				"en": "Astrid Freudenstein is a German politician. She is a member of the CSU."
			},
			"full_name": "Dr. Astrid Freudenstein",
			"images": {
				"copyright": "Henning Schacht",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "121.jpg",
				"pathToThumb": "121_t.jpg"
			},
			"name": "Astrid Freudenstein",
			"party": "CSU",
			"pid": "121",
			"self_bird": "mehlschwalbe",
			"twittering": {
				"twitterId": "704024372940513280",
				"twitterUserName": "dieFreudenstein"
			}
		},
		"122": {
			"citizen_bird": "zilpzalp",
			"cv": {
				"de": "Hans-Peter Friedrich (* 10. M\u00e4rz 1957 in Naila) ist ein deutscher Politiker der CSU. Vom 3. M\u00e4rz 2011 bis zum 17. Dezember 2013 war er Bundesminister des Innern sowie seit dem 30. September 2013 kommissarischer Bundesminister f\u00fcr Ern\u00e4hrung, Landwirtschaft und Verbraucherschutz bis zum Ende des Kabinetts Merkel II. Im Kabinett Merkel III war er vom 17. Dezember 2013 bis zum 17. Februar 2014, als er seine Entlassungsurkunde erhielt, Bundesminister f\u00fcr Ern\u00e4hrung und Landwirtschaft. Aufgrund der Edathy-Aff\u00e4re hatte er am 14. Februar 2014 seinen R\u00fccktritt vom Amt erkl\u00e4rt. Von 2005 bis 2011 war Friedrich stellvertretender Vorsitzender der CDU/CSU-Bundestagsfraktion und von Oktober 2009 bis M\u00e4rz 2011 Vorsitzender der CSU-Landesgruppe. Seit dem 17. Februar 2014 ist Friedrich erneut einer der zw\u00f6lf stellvertretenden Vorsitzenden der CDU/CSU-Bundestagsfraktion.",
				"en": "Hans-Peter Friedrich (born March 10, 1957) is a German politician, representative of the Christian Social Union (CSU). On 3 March 2011 he succeeded Thomas de Maizi\u00e8re as Federal Minister of the Interior[1] and held this ministry until 17 December 2013 when he was appointed Federal Minister for Food and Agriculture. Friedrich resigned from that position in February 2014. Friedrich has a controversial history with minorities in Germany, causing outrage in 2013 after telling journalists that Islam in Germany is not something supported by history at any point."
			},
			"full_name": "Dr. Hans-Peter Friedrich",
			"images": {
				"copyright": "Henning Schacht",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "122.jpg",
				"pathToThumb": "122_t.jpg"
			},
			"name": "Hans-Peter Friedrich",
			"party": "CSU",
			"pid": "122",
			"self_bird": "star",
			"twittering": {
				"twitterId": "708224250885349376",
				"twitterUserName": "HPFriedrichCSU"
			}
		},
		"124": {
			"citizen_bird": "zilpzalp",
			"cv": {
				"de": "Michael Franz Wilhelm Fuchs (* 6. Februar 1949 in Koblenz) ist ein deutscher Politiker (CDU).  Er ist seit 2009 einer der stellvertretenden Vorsitzenden der CDU/CSU-Bundestagsfraktion, dort derzeit zust\u00e4ndig f\u00fcr Wirtschaft und Energie, Mittelstand und Tourismus.",
				"en": "Michael Fuchs is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Michael Fuchs",
			"images": {
				"copyright": "Werner Sch\u00fcring",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "124.jpg",
				"pathToThumb": "124_t.jpg"
			},
			"name": "Michael Fuchs",
			"party": "CDU",
			"pid": "124",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 56670083,
				"twitterUserName": "drmfuchs"
			}
		},
		"125": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Hans-Joachim Fuchtel (* 13. Februar 1952 in Sulz am Neckar) ist ein deutscher Politiker (CDU). Seit 1987 ist er Mitglied des Deutschen Bundestages und seit 2009 Parlamentarischer Staatssekret\u00e4r. Seit Herbst 2011 ist Fuchtel Beauftragter der Bundeskanzlerin f\u00fcr Griechenland.",
				"en": "Hans-Joachim Fuchtel is a German politician. He is a member of the CDU."
			},
			"full_name": "Hans-Joachim Fuchtel",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "125.jpg",
				"pathToThumb": "125_t.jpg"
			},
			"name": "Hans-Joachim Fuchtel",
			"party": "CDU",
			"pid": "125",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": 66750542,
				"twitterUserName": "fuchtel"
			}
		},
		"127": {
			"citizen_bird": "weisskopfseeadler",
			"cv": {
				"de": "Sigmar Hartmut Gabriel (* 12. September 1959 in Goslar) ist ein deutscher Politiker, seit dem 13. November 2009 Vorsitzender der SPD und seit dem 17. Dezember 2013 Stellvertreter der Bundeskanzlerin (deutscher Vizekanzler). Vom 17. Dezember 2013 bis zum 26. Januar 2017 war er Bundesminister f\u00fcr Wirtschaft und Energie im Kabinett Merkel III.  Seit dem 27. Januar 2017 ist Sigmar Gabriel Bundesau\u00dfenminister, nachdem der bisherige Amtsinhaber Frank-Walter Steinmeier sein Amt aufgrund der anstehenden Wahl zum Bundespr\u00e4sidenten aufgegeben hat.",
				"en": "Sigmar Hartmut Gabriel (born 12 September 1959) is a German politician who currently serves as Minister for Foreign Affairs and Vice Chancellor. He has been chairman of the Social Democratic Party of Germany (SPD) since 2009, which makes him the party\u2019s longest-serving leader since Willy Brandt. He was the Federal Minister of the Environment from 2005 to 2009 and the Federal Minister for Economic Affairs and Energy from 2013 to 2017."
			},
			"full_name": "Sigmar Gabriel",
			"images": {
				"copyright": "Dominik Butzmann / SPD",
				"license": "custom-spd",
				"pathToImage": "127.jpg",
				"pathToThumb": "127_t.jpg"
			},
			"name": "Sigmar Gabriel",
			"party": "SPD",
			"pid": "127",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 569832889,
				"twitterUserName": "sigmargabriel"
			}
		},
		"130": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Matthias Gastel (* 26. Dezember 1970 in Stuttgart) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und Unternehmer.  Bei der Bundestagswahl 2013 kandidierte er im Wahlkreis N\u00fcrtingen und zog \u00fcber die Landesliste in den Deutschen Bundestag ein.  Er ist Mitglied im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur und dort bahnpolitischer Sprecher seiner Fraktion.",
				"en": "Matthias Gastel is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Matthias Gastel",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "130.jpg",
				"pathToThumb": "130_t.jpg"
			},
			"name": "Matthias Gastel",
			"party": "GR\u00dcNE",
			"pid": "130",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 727782559,
				"twitterUserName": "MatthiasGastel"
			}
		},
		"132": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Thomas Gebhart (* 20. Dezember 1971 in Kandel) ist ein deutscher Politiker der CDU. Seit 2009 ist er Mitglied des Deutschen Bundestages f\u00fcr den Wahlkreis S\u00fcdpfalz.  Gebhart ist Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag. Er ist seit der Bundestagswahl 2009 ordentliches Mitglied im Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit, Schriftf\u00fchrer und stellvertretendes Mitglied im Haushaltsausschuss.  Er ist dar\u00fcber hinaus Mitglied in der Deutsch-Britischen und Deutsch-Schweizerischen Parlamentariergruppe sowie im Parlamentarischen Freundeskreis Berlin-Taipei. F\u00fcr die Unionsfraktion ist Gebhart Berichterstatter f\u00fcr Kreislaufwirtschaft und Ressourceneffizienz sowie Ko-Berichterstatter f\u00fcr Klimaschutz.",
				"en": "Thomas Gebhart is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Thomas Gebhart",
			"images": {
				"copyright": "Andy M. R\u00e4sch",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "132.jpg",
				"pathToThumb": "132_t.jpg"
			},
			"name": "Thomas Gebhart",
			"party": "CDU",
			"pid": "132",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": 51483238,
				"twitterUserName": "thomasgebhart"
			}
		},
		"133": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Wolfgang Gehrcke-Reymann (* 8. September 1943 in Reichau) ist ein deutscher Politiker (Die Linke).  Er war von 1998 bis 2002 stellvertretender Vorsitzender der PDS-Bundestagsfraktion.  Seit Oktober 2002 war Gehrcke Mitglied des Parteivorstands der PDS und von Juni 2007 bis 2012 des Parteivorstands der Partei Die Linke.",
				"en": "Wolfgang Gehrcke is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Wolfgang Gehrcke",
			"images": {
				"license": "custom-linke",
				"pathToImage": "133.jpg",
				"pathToThumb": "133_t.jpg"
			},
			"name": "Wolfgang Gehrcke",
			"party": "DIE LINKE",
			"pid": "133",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": 532669620,
				"twitterUserName": "WolfgangGehrcke"
			}
		},
		"134": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Kai Boris Gehring (* 26. Dezember 1977 in M\u00fclheim an der Ruhr) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen).  Von Juni 2011 bis September 2013 war er in der Fraktion Sprecher f\u00fcr Bildungs- und Hochschulpolitik. Seit Oktober 2013 ist Gehring Sprecher f\u00fcr Hochschule, Wissenschaft und Forschung. Er ist Obmann seiner Fraktion im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung und stellvertretendes Mitglied im Haushaltsausschuss. Zudem ist er stellvertretender Vorsitzender der Deutsch-Griechischen Parlamentariergruppe.",
				"en": "Kai Gehring (born 26 December 1977 in M\u00fclheim an der Ruhr) is a German politician.  Having joined the liberal party Alliance '90/The Greens in 1998, Gehring was elected to the Bundestag in the 2005 federal election. He was able to retain his seat in the 2009 election."
			},
			"full_name": "Kai Gehring",
			"images": {
				"copyright": "Harald Krichel",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "134.jpg",
				"pathToThumb": "134_t.jpg"
			},
			"name": "Kai Gehring",
			"party": "GR\u00dcNE",
			"pid": "134",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 110368456,
				"twitterUserName": "KaiGehring"
			}
		},
		"14": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Dorothee B\u00e4r (* 19. April 1978 in Bamberg als Dorothee Mantel) ist eine deutsche Politikerin (CSU).  Seit 2010 ist sie Vorsitzende des CSU Netzrates und seit 2011 Vorsitzende des CSUnet. Au\u00dferdem wurde Dorothee B\u00e4r 2011 zur stellvertretenden Bezirksvorsitzenden der CSU Unterfranken gew\u00e4hlt. Im Jahr 2012 war Dorothee B\u00e4r Teil der Versuchsgruppe einer Studie, die im Auftrag des Instituts f\u00fcr Medien- und Kommunikationspolitik (IfM) durchgef\u00fchrt wurde und das Twitter-Verhalten von Politikern untersucht hat.",
				"en": "Dorothee B\u00e4r (n\u00e9e Mantel, born 19 April 1978[1] in Bamberg, Bavaria) is a German politician that has served as the National Deputy Secretary General of the Christian Social Union of Bavaria since 2009."
			},
			"full_name": "Dorothee B\u00e4r",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "14.jpg",
				"pathToThumb": "14_t.jpg"
			},
			"name": "Dorothee B\u00e4r",
			"party": "CSU",
			"pid": "14",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 140821364,
				"twitterUserName": "DoroBaer"
			}
		},
		"141": {
			"citizen_bird": "star",
			"cv": {
				"de": "Angelika Gl\u00f6ckner (* 5. Februar 1962 in Pirmasens, Rheinland-Pfalz) ist eine deutsche Politikerin (SPD) und seit 2014 Abgeordnete im Deutschen Bundestag.  Sie ist seit dem 20. September 2014 stellvertretende Vorsitzende des SPD Unterbezirks Pirmasens-Zweibr\u00fccken.  Sie ist ordentliches Mitglied im Ausschuss f\u00fcr die Angelegenheiten der Europ\u00e4ischen Union und im Ausschuss f\u00fcr Menschenrechte und humanit\u00e4re Hilfe. Dar\u00fcber hinaus arbeitet Gl\u00f6ckner innerhalb der SPD-Fraktion des Bundestages in der Arbeitsgruppe Kommunalpolitik mit.",
				"en": "Angelika Gl\u00f6ckner is a German politician. She is a member of the SPD."
			},
			"full_name": "Angelika Gl\u00f6ckner",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "141.jpg",
				"pathToThumb": "141_t.jpg"
			},
			"name": "Angelika Gl\u00f6ckner",
			"party": "SPD",
			"pid": "141",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": "976421048",
				"twitterUserName": "A_Gloeckner"
			}
		},
		"142": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Nicole Gohlke (* 15. November 1975 in M\u00fcnchen) ist eine deutsche Politikerin. Sie ist Mitglied im Landesvorstand der Linken in Bayern und geh\u00f6rt seit Herbst 2009 als Abgeordnete dem Deutschen Bundestag an. Bei der Bundestagswahl 2013 war sie eine der acht Spitzenkandidaten der Partei. Als letzte Bundestagsabgeordnete wurde sie bis 2015 vom Bayerischen Verfassungsschutz beobachtet.",
				"en": "Nicole Gohlke is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Nicole Gohlke",
			"images": {
				"license": "custom-linke",
				"pathToImage": "142.jpg",
				"pathToThumb": "142_t.jpg"
			},
			"name": "Nicole Gohlke",
			"party": "DIE LINKE",
			"pid": "142",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 866425075,
				"twitterUserName": "NicoleGohlke"
			}
		},
		"145": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Katrin Dagmar G\u00f6ring-Eckardt (geb. Eckardt; * 3. Mai 1966 in Friedrichroda) ist eine deutsche Politikerin von B\u00fcndnis 90/Die Gr\u00fcnen. Von Oktober 2005 bis Oktober 2013 amtierte sie f\u00fcr die Fraktion von B\u00fcndnis 90/Die Gr\u00fcnen als Vizepr\u00e4sidentin des Deutschen Bundestages. Bei der Urwahl im November 2012 w\u00e4hlte die gr\u00fcne Parteibasis sie \u2013 zusammen mit J\u00fcrgen Trittin \u2013 zur Spitzenkandidatin der Gr\u00fcnen f\u00fcr die Bundestagswahl 2013. Seit Oktober 2013 ist sie neben Anton Hofreiter Vorsitzende der Bundestagsfraktion ihrer Partei, ein Amt, das sie bereits von 2002 bis 2005 neben Krista Sager bekleidet hatte. Dar\u00fcber hinaus war G\u00f6ring-Eckardt von 2009 bis September 2013 Pr\u00e4ses der Synode der Evangelischen Kirche in Deutschland (EKD) und somit Mitglied im Rat der EKD.",
				"en": "Katrin Dagmar G\u00f6ring-Eckardt (born Katrin Dagmar Eckardt on 3 May 1966 in Friedrichroda, Thuringia [then part of East Germany]), better known as Katrin G\u00f6ring-Eckardt, is a German politician from the German Green Party (officially known as Alliance '90/The Greens; German: B\u00fcndnis 90/Die Gr\u00fcnen). Starting her political activity in the now-former German Democratic Republic (East Germany) in the late 1980s, she has been a member of the German Bundestag since 1998. She became co-chair of her party caucus in the Bundestag (2002\u20132005) and the Greens' Vice President of the Bundestag on 18 October 2005, a position that she has held since then. In the November 2012 primary election, the Green Party chose her and J\u00fcrgen Trittin as the top two candidates for the Greens for the 2013 German federal election that took place on 22 September 2013. She is once again standing to be one of the top two candidates for the Greens for the 2017 German federal election."
			},
			"full_name": "Katrin G\u00f6ring-Eckardt",
			"images": {
				"copyright": "Heinrich-B\u00f6ll-Stiftung from Berlin, Deutschland",
				"license": "CC-BY-SA-2.0",
				"pathToImage": "145.jpg",
				"pathToThumb": "145_t.jpg"
			},
			"name": "Katrin G\u00f6ring-Eckardt",
			"party": "GR\u00dcNE",
			"pid": "145",
			"self_bird": "buntspecht",
			"twittering": {
				"twitterId": 626287930,
				"twitterUserName": "GoeringEckardt"
			}
		},
		"147": {
			"citizen_bird": "kleiber",
			"cv": {
				"de": "Kerstin Griese (* 6. Dezember 1966 in M\u00fcnster) ist eine deutsche Politikerin (SPD). Sie ist seit 2014 Mitglied des Deutschen Bundestages und Vorsitzende des Ausschusses f\u00fcr Arbeit und Soziales.  Von 2006 bis 2009 und seit 2011 ist sie Beauftragte der SPD-Bundestagsfraktion f\u00fcr Kirchen und Religionsgemeinschaften.",
				"en": "Kerstin Griese is a German politician. She is a member of the SPD."
			},
			"full_name": "Kerstin Griese",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "147.jpg",
				"pathToThumb": "147_t.jpg"
			},
			"name": "Kerstin Griese",
			"party": "SPD",
			"pid": "147",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 39709752,
				"twitterUserName": "KerstinGriese"
			}
		},
		"150": {
			"citizen_bird": "gimpel",
			"cv": {
				"de": "Hermann Gr\u00f6he (* 25. Februar 1961 in Uedem) ist ein deutscher Politiker (CDU) und seit dem 17. Dezember 2013 Bundesminister f\u00fcr Gesundheit im Kabinett Merkel III.  Am 1. Oktober 2008 \u00fcbernahm Gr\u00f6he die Nachfolge Hildegard M\u00fcllers als Staatsminister bei der Bundeskanzlerin (Kabinett Merkel I). Nach der Wahl zum CDU-Generalsekret\u00e4r legte er im Oktober 2009 das Amt nieder.",
				"en": "Hermann Gr\u00f6he (born 25 February 1961) is a German politician of the Christian Democratic Union (CDU) who has been serving as Minister of Health in the third cabinet of Chancellor Angela Merkel since 2013.   March 2016, Gr\u00f6he was appointed by United Nations Secretary-General Ban Ki-moon to the High-Level Commission on Health Employment and Economic Growth, which was co-chaired by presidents Fran\u00e7ois Hollande of France and Jacob Zuma of South Africa.  He later led Germany's delegation to the 2016 High Level Meeting on Ending AIDS in New York."
			},
			"full_name": "Hermann Gr\u00f6he",
			"images": {
				"copyright": "CDU Deutschlands/Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "150.jpg",
				"pathToThumb": "150_t.jpg"
			},
			"name": "Hermann Gr\u00f6he",
			"party": "CDU",
			"pid": "150",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 94363834,
				"twitterUserName": "groehe"
			}
		},
		"153": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Michael Peter Gro\u00df (* 26. Juli 1956 in Marl) ist ein deutscher Politiker (SPD). Gro\u00df ist seit Oktober 2009 Mitglied des Deutschen Bundestages. Daneben ist er Stadtverbandsvorsitzender der Marler SPD und stellvertretender Vorsitzender des SPD-Kreisverbandes Recklinghausen. Seit Januar 2016 ist Michael Gro\u00df Mitglied im Ausschuss f\u00fcr Recht und Verbraucherschutz. Er sitzt in der \u00c4ltestenratskommission f\u00fcr Bau- und Raumangelegenheiten.",
				"en": "Michael Gro\u00df is a German politician. He is a member of the SPD."
			},
			"full_name": "Michael Gro\u00df",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "153.jpg",
				"pathToThumb": "153_t.jpg"
			},
			"name": "Michael Gro\u00df",
			"party": "SPD",
			"pid": "153",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 2814949590,
				"twitterUserName": "michaelgrossmdb"
			}
		},
		"154": {
			"citizen_bird": "tuerkentaube",
			"cv": {
				"de": "Michael Grosse-Br\u00f6mer (* 12. Oktober 1960 in Oberhausen) ist ein deutscher Politiker (CDU). Seit dem 22. Mai 2012 ist er Erster Parlamentarischer Gesch\u00e4ftsf\u00fchrer der CDU/CSU-Bundestagsfraktion.  Von Januar 2006 bis M\u00e4rz 2010 war er Mitglied im erweiterten Vorstand der CDU/CSU-Bundestagsfraktion, seit seiner Wahl zum Justiziar der Fraktion ist er gesch\u00e4ftsf\u00fchrendes Vorstandsmitglied. Von Oktober 2009 bis September 2013 war er Vorsitzender der Landesgruppe Niedersachsen in der CDU/CSU-Bundestagsfraktion und bis April 2010 der rechtspolitische Sprecher der CDU/CSU-Fraktion im Deutschen Bundestag.",
				"en": "Michael Grosse-Br\u00f6mer (born 12 October 1960 in Oberhausen) is a German politician and a member of the Bundestag for the German Christian Democratic Union.  In 2012 he succeeded Peter Altmaier as First Secretary of the parliamentary group, in this position assisting the parliamentary group's chairman Volker Kauder. In addition, he is a member of the German-Austrian Parliamentary Friendship Group and the German-Irish Parliamentary Friendship Group.  Since 2014, he has been serving on the Committee on the Election of Judges (Wahlausschuss), which is in charge of appointing judges to the Federal Constitutional Court of Germany."
			},
			"full_name": "Michael Grosse-Br\u00f6mer",
			"images": {
				"copyright": "Ralf Roletschek",
				"license": "CC-BY-SA-3.0 at",
				"pathToImage": "154.jpg",
				"pathToThumb": "154_t.jpg"
			},
			"name": "Michael Grosse-Br\u00f6mer",
			"party": "CDU",
			"pid": "154",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 574424501,
				"twitterUserName": "MGrosseBroemer"
			}
		},
		"156": {
			"citizen_bird": "gartenrotschwanz",
			"cv": {
				"de": "Annette Groth (* 16. Mai 1954 in Gadderbaum, heute zu Bielefeld) ist eine baden-w\u00fcrttembergische Politikerin der Partei Die Linke und Mitglied des Deutschen Bundestags.  Sie wurde zur Vorsitzenden der Deutsch-Griechischen Parlamentariergruppe des Deutschen Bundestags gew\u00e4hlt. Dar\u00fcber hinaus ist sie Mitglied im Migrationsausschuss der parlamentarischen Versammlung des Europarates.",
				"en": "Annette Groth (born May 16, 1954 Bielefeld) is a German politician from Baden-Wuerttemberg.  Since October 2007, she is a member of the Left party in Baden-W\u00fcrttemberg, a member of the AK European integration, the BAG peace and international politics and active in the European Left Party to the feminist structures \"EL-Fem\". She is also a member of the Forum of the Rosa Luxemburg Foundation of Baden-Wuerttemberg."
			},
			"full_name": "Annette Groth",
			"images": {
				"license": "custom-linke",
				"pathToImage": "156.jpg",
				"pathToThumb": "156_t.jpg"
			},
			"name": "Annette Groth",
			"party": "DIE LINKE",
			"pid": "156",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 431503846,
				"twitterUserName": "Annette_Groth"
			}
		},
		"159": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Manfred Grund (* 3. Juli 1955 in Zeitz) ist ein deutscher Politiker (CDU). Er ist seit 1998 Parlamentarischer Gesch\u00e4ftsf\u00fchrer der CDU/CSU-Bundestagsfraktion.  Im Januar 1990 wurde Grund Mitglied der CDU. Von 1994 bis 2001 war er stellvertretender Vorsitzender des CDU-Kreisverbandes Eichsfeld und daneben von 1994 bis 2003 sowie seit 2013 Vorsitzender des CDU-Stadtverbandes Heiligenstadt. Seit 1998 geh\u00f6rt er dem CDU-Landesvorstand in Th\u00fcringen an.  Er war Gr\u00fcndungsmitglied und erster Vorsitzender des Deutsch-Moldauischen Forums, dem er vom 26. M\u00e4rz 2009 bis zum 21. November 2015 vorstand. Er ist der stellvertretende Vorsitzende der Deutsch-Kasachischen Gesellschaft.",
				"en": "Manfred Grund (born July 3, 1955) is a German politician of the Christian Democratic Union (CDU).  Grund has served as parliamentary secretary of the CDU/CSU parliamentary group and as chairman of the Thuringian Committee of the CDU/CSU parliamentary group since 1998."
			},
			"full_name": "Manfred Grund",
			"images": {
				"copyright": "Antje Kaunzner",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "159.jpg",
				"pathToThumb": "159_t.jpg"
			},
			"name": "Manfred Grund",
			"party": "CDU",
			"pid": "159",
			"self_bird": "gimpel",
			"twittering": {
				"twitterId": 18935360,
				"twitterUserName": "manfred_grund"
			}
		},
		"16": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Thomas Barei\u00df (* 15. Februar 1975 in Albstadt-Ebingen) ist ein deutscher Politiker (CDU).  Seit 2002 geh\u00f6rte er in seiner Eigenschaft als Landesvorsitzender der Jungen Union als beratendes Mitglied dem CDU-Landesvorstandes Baden-W\u00fcrttemberg an, seit dem Landesparteitag 2005 als gew\u00e4hltes Mitglied.  Am 22. Oktober 2011 wurde Thomas Barei\u00df in Uhldingen-M\u00fchlhofen zum neuen Vorsitzenden des rund 13.000 Mitglieder starken CDU-Bezirksverbands W\u00fcrttemberg-Hohenzollern gew\u00e4hlt.",
				"en": "Thomas Barei\u00df (born 15 February 1975 in Albstadt, Baden-W\u00fcrttemberg) is a German politician and member of the conservative CDU.  A business manager by profession, Barei\u00df has been a directly elected member of the Bundestag since 2005. In his first term between 2005 and 2009, he was a full member of the Committee on the Affairs of the European Union as well as of the Committee on Labor and Social Affairs. Since the 2009 elections, he has been serving on the Committee on Economic Affairs and Energy."
			},
			"full_name": "Thomas Barei\u00df",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "16.jpg",
				"pathToThumb": "16_t.jpg"
			},
			"name": "Thomas Barei\u00df",
			"party": "CDU",
			"pid": "16",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 2797812265,
				"twitterUserName": "Thomas_Bareiss"
			}
		},
		"162": {
			"citizen_bird": "eichelhaeher",
			"cv": {
				"de": "Herlind Magdalena Gundelach (* 28. Februar 1949 in Aalen) ist eine deutsche Politikerin (CDU). Sie war von 2008 bis M\u00e4rz 2011 Hamburger Senatorin f\u00fcr Wissenschaft und Forschung. Vom 29. November 2010 bis zum 7. M\u00e4rz 2011 war sie au\u00dferdem Senatorin f\u00fcr Finanzen sowie f\u00fcr Stadtentwicklung und Umwelt.",
				"en": "Herlind Gundelach, (born 1949) is a German politician, representative of the German Christian Democratic Union.  Gundelach was elected a member of the German Bundestag in the 2013 federal elections. She has since been serving as deputy chairwoman of the Committee on Cultural Affairs and Media. In addition, she is a member of the Committee on Economic Affairs and Energy, where she serves as her parliamentary group\u2019s rapporteur on energy efficiency and public procurement rules."
			},
			"full_name": "Dr. Herlind Gundelach",
			"images": {
				"copyright": "Walter M\u00fccksch",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "162.jpg",
				"pathToThumb": "162_t.jpg"
			},
			"name": "Herlind Gundelach",
			"party": "CDU",
			"pid": "162",
			"self_bird": "gartenrotschwanz",
			"twittering": {
				"twitterId": 550951920,
				"twitterUserName": "HGundelach"
			}
		},
		"165": {
			"citizen_bird": "tannenmeise",
			"cv": {
				"de": "Olav Gutting (* 14. Oktober 1970 in Karlsruhe) ist ein deutscher Politiker (CDU).  Seit 2002 ist er Mitglied des Deutschen Bundestages. Olav Gutting ist stets als direkt gew\u00e4hlter Abgeordneter des Wahlkreises Bruchsal \u2013 Schwetzingen in den Bundestag eingezogen.",
				"en": "Olav Gutting is a German politician. He is a member of the CDU."
			},
			"full_name": "Olav Gutting",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "165.jpg",
				"pathToThumb": "165_t.jpg"
			},
			"name": "Olav Gutting",
			"party": "CDU",
			"pid": "165",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 58429842,
				"twitterUserName": "olavgutting"
			}
		},
		"166": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Gregor Florian Gysi (* 16. Januar 1948 in Berlin) ist ein deutscher Rechtsanwalt, linksgerichteter Politiker sowie zwischen 1990 und 2000 und seit 2005 erneut Mitglied des Deutschen Bundestages. Er ist Mitglied der Partei Die Linke.  Von 2005 bis 2015 war er Fraktionsvorsitzender der Linksfraktion im Deutschen Bundestag. Mit dem Amtsantritt des dritten Kabinetts Merkel am 17. Dezember 2013 wurde Gysi zus\u00e4tzlich Oppositionsf\u00fchrer im Deutschen Bundestag. Am 17. Dezember 2016 w\u00e4hlte die Europ\u00e4ische Linke Gysi auf einem Parteitag in Berlin zum Pr\u00e4sidenten.",
				"en": "Gregor Gysi (born 16 January 1948) is a German attorney and key politician of the political party The Left (Die Linke)."
			},
			"full_name": "Dr. Gregor Gysi",
			"images": {
				"license": "custom-linke",
				"pathToImage": "166.jpg",
				"pathToThumb": "166_t.jpg"
			},
			"name": "Gregor Gysi",
			"party": "DIE LINKE",
			"pid": "166",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 888289790,
				"twitterUserName": "GregorGysi"
			}
		},
		"17": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Katarina Barley (* 19. November 1968 in K\u00f6ln) ist eine deutsche Juristin, Politikerin (SPD) und Abgeordnete im 18. Deutschen Bundestag. Am 11. Dezember 2015 wurde sie auf dem SPD-Bundesparteitag zur Generalsekret\u00e4rin gew\u00e4hlt.  Anfang November 2015 schlug der SPD-Vorsitzende Sigmar Gabriel Barley als Generalsekret\u00e4rin der SPD (und damit als Nachfolgerin von Yasmin Fahimi) vor.",
				"en": "Katarina Barley (* 19. November 1968 in Cologne) is a German lawyer, politician and member of the 18th German Bundestag. Her English last name comes from her British father.  In 2014, Barley briefly served as a member of the Committee on the Affairs of the European Union. In addition to her committee assignments, she is a member of the German-British Parliamentary Friendship Group.  Within the SPD parliamentary group, Barley belongs to the Parliamentary Left, a left-wing movement.  In 2015, she was proposed by party chairman Sigmar Gabriel to succeed Yasmin Fahimi in the role of general secretary of the SPD, one of the party's most senior positions."
			},
			"full_name": "Dr. Katarina Barley",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "17.jpg",
				"pathToThumb": "17_t.jpg"
			},
			"name": "Katarina Barley",
			"party": "SPD",
			"pid": "17",
			"self_bird": "haussperling",
			"twittering": {
				"twitterId": "4097559143",
				"twitterUserName": "katarinabarley"
			}
		},
		"171": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Florian Hahn (* 14. M\u00e4rz 1974 in M\u00fcnchen) ist ein deutscher Politiker. Als Abgeordneter der CSU geh\u00f6rt er seit der Wahl zum 17. Bundestag im Herbst 2009 dem Deutschen Bundestag an.  Seit 2014 ist er Sprecher der CSU-Landesgruppe f\u00fcr Ausw\u00e4rtiges, Verteidigung, Angelegenheiten der Europ\u00e4ischen Union, wirtschaftliche Zusammenarbeit und Entwicklung, Menschenrechte und Humanit\u00e4re Hilfe und Landesvorsitzender des Arbeitskreises Au\u00dfen-, Sicherheits- und Europapolitik (ASP) der CSU. Hier leitet er auch den Fachausschuss Au\u00dfenpolitik.",
				"en": "Florian Hahn is a German politician. He is a member of the CSU."
			},
			"full_name": "Florian Hahn",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "171.jpg",
				"pathToThumb": "171_t.jpg"
			},
			"name": "Florian Hahn",
			"party": "CSU",
			"pid": "171",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 403735368,
				"twitterUserName": "hahnflo"
			}
		},
		"172": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Anja Hajduk (* 8. Juni 1963 in Duisburg) ist eine deutsche Politikerin. Sie ist Mitglied im Hamburger Landesverband von B\u00fcndnis 90/Die Gr\u00fcnen und Mitglied des Deutschen Bundestages.  Sie war vom 7. Mai 2008 bis zum 29. November 2010 Senatorin und Pr\u00e4ses der Beh\u00f6rde f\u00fcr Stadtentwicklung und Umwelt der Freien und Hansestadt Hamburg.",
				"en": "Hajduk (born 8 June 1963 in Duisburg) is a German politician. She has been a member of the Alliance '90/The Greens since 1995.  In the 2013 federal elections, Hajduk was again elected member of the German Bundestag where she has since served as Chief Whip of her parliamentary group. A member of the budget committee, she serves as rapporteur on the budgets of the Federal Ministry for Economic Affairs and Energy, the Federal Ministry of Economic Cooperation and Development, the Federal Ministry of the Interior and the Bundestag. She is also a member of the so-called Confidential Committee (Vertrauensgremium) of the Budget Committee, which provides budgetary supervision for Germany\u2019s three intelligence services, BND, BfV and MAD."
			},
			"full_name": "Anja Hajduk",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "172.jpg",
				"pathToThumb": "172_t.jpg"
			},
			"name": "Anja Hajduk",
			"party": "GR\u00dcNE",
			"pid": "172",
			"self_bird": "schwanzmeise",
			"twittering": {
				"twitterId": 4724923756,
				"twitterUserName": "HajdukBundestag"
			}
		},
		"173": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Metin Hakverdi (* 25. Juni 1969 in Hamburg) ist ein Hamburger Politiker der SPD. Seit 2013 ist er Mitglied des Deutschen Bundestages.  Im Dezember 2012 wurde Hakverdi als Kandidat der SPD f\u00fcr den Wahlkreis Hamburg-Bergedorf \u2013 Harburg als Nachfolger von Hans-Ulrich Klose nominiert. Bei der Bundestagswahl 2013 wurde er als Direktkandidat in den Bundestag gew\u00e4hlt. Im Bundestag ist er Mitglied des Finanzausschusses, des Parlamentarischen Finanzmarktgremiums und des Ausschusses f\u00fcr Recht und Verbraucherschutz.",
				"en": "Metin Hakverdi is a German politician. He is a member of the SPD."
			},
			"full_name": "Metin Hakverdi",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "173.jpg",
				"pathToThumb": "173_t.jpg"
			},
			"name": "Metin Hakverdi",
			"party": "SPD",
			"pid": "173",
			"self_bird": "eichelhaeher",
			"twittering": {
				"twitterId": 1043288618,
				"twitterUserName": "MetinHakverdi"
			}
		},
		"175": {
			"citizen_bird": "gartenrotschwanz",
			"cv": {
				"de": "Heike H\u00e4nsel (* 1. Januar 1966 in Stuttgart) ist eine deutsche Politikerin (Die Linke). Seit 2005 ist Heike H\u00e4nsel Mitglied des Deutschen Bundestages. Hier ist sie entwicklungspolitische Sprecherin der Linksfraktion. Sie ist \u00fcber die Landesliste Baden-W\u00fcrttemberg in den Bundestag eingezogen. Seit dem 3. November 2015 ist sie Stellvertretende Vorsitzende der Linksfraktion.",
				"en": "Heike H\u00e4nsel is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Heike H\u00e4nsel",
			"images": {
				"license": "custom-linke",
				"pathToImage": "175.jpg",
				"pathToThumb": "175_t.jpg"
			},
			"name": "Heike H\u00e4nsel",
			"party": "DIE LINKE",
			"pid": "175",
			"self_bird": "gimpel",
			"twittering": {
				"twitterId": 332264734,
				"twitterUserName": "HeikeHaensel"
			}
		},
		"176": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Stephan Harbarth (* 19. Dezember 1971 in Heidelberg) ist ein deutscher Politiker (CDU) und Rechtsanwalt.  Ende August 2010 wurde Harbarth in den Bundesfachausschuss Wirtschafts-, Haushalts- und Finanzpolitik der CDU Deutschlands berufen. 2011 wurde Harbarth als Nachfolger von Georg Wacker zum Kreisvorsitzenden der CDU Rhein-Neckar gew\u00e4hlt, seit 2013 ist er Mitglied des Landesvorstandes der CDU Baden-W\u00fcrttemberg. Harbarth ist Mitglied im CDU-Bundesvorstand.",
				"en": "Stephan Harbarth is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Stephan Harbarth",
			"images": {
				"copyright": "Dr. Stephan Harbarth/Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "176.jpg",
				"pathToThumb": "176_t.jpg"
			},
			"name": "Stephan Harbarth",
			"party": "CDU",
			"pid": "176",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 56762889,
				"twitterUserName": "stephanharbarth"
			}
		},
		"177": {
			"citizen_bird": "star",
			"cv": {
				"de": "J\u00fcrgen Hardt (* 30. Mai 1963 in Hofheim am Taunus) ist ein deutscher Politiker (CDU). J\u00fcrgen Hardt ist au\u00dfenpolitischer Sprecher der CDU/CSU-Bundestagsfraktion und geh\u00f6rt dem Fraktionsvorstand an.  Hardt trat 1981 in die CDU ein. Von 1987 bis 1989 war er Bundesvorsitzender des Rings Christlich-Demokratischer Studenten. Von 2004 bis 2009 geh\u00f6rte er dem Rat der Stadt Wuppertal an. Von 2003 bis 2014 war er Kreisvorsitzender der CDU Wuppertal. Seit 2005 ist J\u00fcrgen Hardt stellvertretender Vorsitzender des CDU-Bezirks Bergisches Land.  Hardt ist Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag, der Parlamentarischen Versammlung der NATO und der Parlamentarischen Versammlung der OSZE. Au\u00dferdem ist er Mitglied im Vorstand der Europ\u00e4ischen Volkspartei.",
				"en": "J\u00fcrgen Hardt (born 30 May 1963) is a German politician. He is a Christian Democratic Union member of the Bundestag.  Hardt was Federal chairman of the Association of Christian Democratic Students (RCDS) from 1987 to 1989 and from 2004 to 2009 member of Wuppertal city council and chairman of its economics committee.  Since 2003 he has been serving as chairman of the Wuppertal county branch of the CDU and since 2005 deputy chairman of the Bergisches Land district branch of the CDU.  In April 2014, Hardt succeeded Philipp Mi\u00dffelder as Coordinator of Transatlantic Cooperation."
			},
			"full_name": "J\u00fcrgen Hardt",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "177.jpg",
				"pathToThumb": "177_t.jpg"
			},
			"name": "J\u00fcrgen Hardt",
			"party": "CDU",
			"pid": "177",
			"self_bird": "haussperling",
			"twittering": {
				"twitterId": 38467606,
				"twitterUserName": "juergenhardt"
			}
		},
		"178": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Michael Hartmann (* 11. Mai 1963 in Pirmasens) ist ein deutscher Politiker (SPD). Seit 2002 ist er Mitglied des Deutschen Bundestages. Hartmann ist ein Fachpolitiker f\u00fcr Innere Sicherheit und war innenpolitischer Sprecher der SPD-Fraktion sowie Obmann seiner Fraktion im Innenausschuss und Mitglied des Parlamentarischen Kontrollgremiums.",
				"en": "Michael Hartmann is a German politician. He is a member of the SPD."
			},
			"full_name": "Michael Hartmann",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "178.jpg",
				"pathToThumb": "178_t.jpg"
			},
			"name": "Michael Hartmann",
			"party": "SPD",
			"pid": "178",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 434884982,
				"twitterUserName": "HartmannMdB"
			}
		},
		"179": {
			"citizen_bird": "schneeeule",
			"cv": {
				"de": "Sebastian Hartmann (* 7. Juli 1977 in Oberhausen) ist ein deutscher Politiker (SPD).  Von 2011 bis 2013 war er Assistent des Europaabgeordneten und Pr\u00e4sidenten des Europ\u00e4ischen Parlaments, Martin Schulz. Bei der Bundestagswahl 2013 trat er im Wahlkreis Rhein-Sieg-Kreis I f\u00fcr das Direktmandat an. Zwar unterlag er dabei Elisabeth Winkelmeier-Becker von der CDU, daf\u00fcr zog er aber \u00fcber die Landesliste in den Deutschen Bundestag ein.  Seit 2014 ist er stellvertretender Landrat des Rhein-Sieg-Kreises.",
				"en": "Sebastian Hartmann is a German politician. He is a member of the SPD."
			},
			"full_name": "Sebastian Hartmann",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "179.jpg",
				"pathToThumb": "179_t.jpg"
			},
			"name": "Sebastian Hartmann",
			"party": "SPD",
			"pid": "179",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": 1076269195,
				"twitterUserName": "sebast_hartmann"
			}
		},
		"181": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Britta Ha\u00dfelmann (* 10. Dezember 1961 in Straelen am Niederrhein) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen).  Seit 2005 ist sie Mitglied des Deutschen Bundestages. Hier ist sie Sprecherin der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen f\u00fcr Demografie und Altenpolitik, sowie seit Fr\u00fchjahr 2007 Sprecherin f\u00fcr Kommunalpolitik.  Bei der Bundestagswahl 2009 war Britta Ha\u00dfelmann Kandidatin im Bundestagswahlkreis Bielefeld und ist \u00fcber die Landesliste Nordrhein-Westfalen in den Bundestag eingezogen. W\u00e4hrend dieser Legislaturperiode war sie eine von drei Parlamentarischen Gesch\u00e4ftsf\u00fchrern.  Nach der Bundestagswahl 2013 wurde Ha\u00dfelmann am 8. Oktober 2013 zur ersten Parlamentarischen Gesch\u00e4ftsf\u00fchrerin ihrer Bundestagsfraktion gew\u00e4hlt und tritt die Nachfolge von Volker Beck an, der nicht mehr kandidierte.",
				"en": "Britta Ha\u00dfelmann is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Britta Ha\u00dfelmann",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "181.jpg",
				"pathToThumb": "181_t.jpg"
			},
			"name": "Britta Ha\u00dfelmann",
			"party": "GR\u00dcNE",
			"pid": "181",
			"self_bird": "goldammer",
			"twittering": {
				"twitterId": 1131092102,
				"twitterUserName": "BriHasselmann"
			}
		},
		"182": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Matthias Hauer (* 18. Dezember 1977 in Hattingen) ist ein deutscher Politiker (CDU).  Hauer ist seit 2013 Mitglied des Deutschen Bundestages. Bei der Bundestagswahl 2013 gewann er das Direktmandat im Wahlkreis Essen III, der bis dahin als SPD-Hochburg galt, gegen Petra Hinz (SPD).",
				"en": "Matthias Hauer is a German politician. He is a member of the CDU."
			},
			"full_name": "Matthias Hauer",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "182.jpg",
				"pathToThumb": "182_t.jpg"
			},
			"name": "Matthias Hauer",
			"party": "CDU",
			"pid": "182",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": 1158585210,
				"twitterUserName": "MatthiasHauer"
			}
		},
		"183": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Mark Lars Carsten Hauptmann (* 29. April 1984 in Weimar) ist ein deutscher Politiker (CDU). Er ist seit 2013 Mitglied des 18. Deutschen Bundestages.  Seit 2003 ist er Mitglied der CDU und engagierte sich im Kreisvorstand der CDU Suhl[7] sowie im Landesfachausschuss Europa[8] auf Landesebene. Seit 2014 ist er Mitglied im Bundesfachausschuss Au\u00dfen-, Sicherheits-, Entwicklungs- und Menschenrechtspolitik der CDU Deutschlands.  Er ist ordentliches Mitglied im 'Ausschuss f\u00fcr Wirtschaft und Energie' und stellvertretendes Mitglied im Ausw\u00e4rtigen Ausschuss.",
				"en": "Mark Lars Carsten Hauptmann (born April 29, 1984 in Weimar/Thuringia) is a German politician and member of the Christian Democratic Union of Germany (CDU) and since 2013 member of the 18th German Bundestag.  "
			},
			"full_name": "Mark Hauptmann",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "183.jpg",
				"pathToThumb": "183_t.jpg"
			},
			"name": "Mark Hauptmann",
			"party": "CDU",
			"pid": "183",
			"self_bird": "rotkehlchen",
			"twittering": {
				"twitterId": 1370491088,
				"twitterUserName": "MarkHauptmann"
			}
		},
		"184": {
			"citizen_bird": "hausrotschwanz",
			"cv": {
				"de": "Stefan Heck (* 18. August 1982 in Marburg) ist ein deutscher Politiker (CDU) und seit 2013 Mitglied des 18. Deutschen Bundestags.  Nachdem er bereits bei der Bundestagswahl 2009 erfolglos im Bundestagswahlkreis Marburg kandidiert hatte, zog er 2013 \u00fcber Platz 12 der CDU-Landesliste Hessen in den Bundestag ein. Dr. Stefan Heck ist im Deutschen Bundestag ordentliches Mitglied in dem Ausschuss f\u00fcr Wahlpr\u00fcfung, Immunit\u00e4t und Gesch\u00e4ftsordnung sowie im Ausschuss f\u00fcr Recht und Verbraucherschutz.",
				"en": "Stefan Heck is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Stefan Heck",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "184.jpg",
				"pathToThumb": "184_t.jpg"
			},
			"name": "Stefan Heck",
			"party": "CDU",
			"pid": "184",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 2976064481,
				"twitterUserName": "StHeck"
			}
		},
		"186": {
			"citizen_bird": "star",
			"cv": {
				"de": "Matthias Heider (* 18. Mai 1966 in L\u00fcdenscheid) ist ein deutscher Politiker (CDU).  Seit 2008 ist er stellvertretender Vorsitzender der Mittelstands- und Wirtschaftsvereinigung M\u00e4rkischer Kreis. Seit 2010 ist Matthias Heider dar\u00fcber hinaus stellvertretender Vorsitzender der CDU im M\u00e4rkischen Kreis. Er wurde au\u00dferdem zum Bezirksvorsitzenden der Mittelstands- und Wirtschaftsvereinigung in S\u00fcdwestfalen gew\u00e4hlt.",
				"en": "Matthias Heider is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Matthias Heider",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "186.jpg",
				"pathToThumb": "186_t.jpg"
			},
			"name": "Matthias Heider",
			"party": "CDU",
			"pid": "186",
			"self_bird": "stieglitz",
			"twittering": {
				"twitterId": 154064825,
				"twitterUserName": "MatthiasHeider"
			}
		},
		"188": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Hubertus Heil (* 3. November 1972 in Hildesheim) ist ein deutscher Politiker (SPD). Er ist seit 1998 Mitglied des Deutschen Bundestages und derzeit stellvertretender Fraktionsvorsitzender der Bundestagsfraktion seiner Partei mit Zust\u00e4ndigkeit f\u00fcr die Themen Wirtschaft und Energie, Bildung und Forschung.  Seit 1998 ist er Mitglied des Deutschen Bundestages. Er geh\u00f6rte von Oktober 2002 bis November 2005 dem Vorstand der SPD-Landesgruppe Niedersachsen und von Oktober 2002 bis November 2004 dem Vorstand der SPD-Bundestagsfraktion an. Heil war von April 2003 bis Oktober 2005 Vorsitzender der Fraktionsarbeitsgruppe Telekommunikation und Post. Er geh\u00f6rt zu den Gr\u00fcndungsmitgliedern des Netzwerkes Berlin, deren Sprecher er von 2003 bis 2005 war. Hubertus Heil geh\u00f6rt als beratendes Mitglied der Grundwertekommission der SPD an. Heil ist gegen ein Bedingungsloses Grundeinkommen.",
				"en": "Hubertus Heil (born 3 November 1972 in Hildesheim, Lower Saxony) is a German politician. In 2005 he became general secretary of the Social Democratic Party of Germany (SPD).  In October 2016, Heil accompanied Federal Minister for Economic Affairs and Energy Sigmar Gabriel to Tehran to attend the German-Iranian Joint Economic Commission\u2019s first meeting in 15 years."
			},
			"full_name": "Hubertus Heil",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "188.jpg",
				"pathToThumb": "188_t.jpg"
			},
			"name": "Hubertus Heil",
			"party": "SPD",
			"pid": "188",
			"self_bird": "buntspecht",
			"twittering": {
				"twitterId": 15943222,
				"twitterUserName": "hubertus_heil"
			}
		},
		"189": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Mechthild Heil, geborene Rumpf, (* 23. August 1961 in Andernach) ist eine deutsche Politikerin der CDU. Seit 2009 ist sie Mitglied des Deutschen Bundestages. Sie ist Verbraucherschutzbeauftragte der CDU/CSU-Bundestagsfraktion.  Im Januar 2014 wurde Heil vom Fraktionsvorsitzenden Volker Kauder zur Verbraucherschutzbeauftragten der CDU/CSU-Bundestagsfraktion ernannt. Dieses Amt hatte sie bereits in der vergangenen Wahlperiode inne. Sie ist Ordentliches Mitglied im Ausschuss f\u00fcr Recht und Verbraucherschutz und Stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft[3].",
				"en": "Mechthild Heil is a German politician. She is a member of the CDU."
			},
			"full_name": "Mechthild Heil",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "189.jpg",
				"pathToThumb": "189_t.jpg"
			},
			"name": "Mechthild Heil",
			"party": "CDU",
			"pid": "189",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 974367631,
				"twitterUserName": "MechthildHeil"
			}
		},
		"190": {
			"citizen_bird": "gartenbaumlaeufer",
			"cv": {
				"de": "Rosemarie Anna Gertrud Hein (* 17. Januar 1953 in Leipzig) ist eine deutsche Politikerin der Partei Die Linke, von 1990 bis 2006 Mitglied des Landtags von Sachsen-Anhalt und seit 2009 Mitglied des Deutschen Bundestags.",
				"en": "Anna Gertrud Rosemarie Hein (born January 17, 1953 in Leipzig) is a German politician, currently Education spokeswoman for the Left Party. From 1990 to 2006 she was a member of the Landtag of Saxony-Anhalt, and since 2009 she has been a member of the Bundestag.  "
			},
			"full_name": "Dr. Rosemarie Hein",
			"images": {
				"copyright": "Moko1998, Wikimedia Commons",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "190.jpg",
				"pathToThumb": "190_t.jpg"
			},
			"name": "Rosemarie Hein",
			"party": "DIE LINKE",
			"pid": "190",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": 73468394,
				"twitterUserName": "RosemarieHein"
			}
		},
		"191": {
			"citizen_bird": "ringeltaube",
			"cv": {
				"de": "Frank Heinrich (* 25. Januar 1964 in Siegen) ist ein deutscher Theologe, Sozialp\u00e4dagoge und Politiker (CDU).  Als einer der wenigen Unionspolitiker stimmte Heinrich im Oktober 2010 gegen die Laufzeitverl\u00e4ngerung deutscher Kernkraftwerke. Seit Sommer 2012 setzt sich Heinrich gemeinsam mit zw\u00f6lf weiteren Unions-Bundestagsabgeordneten f\u00fcr die steuerliche Gleichstellung im Rahmen des Lebenspartnerschaftsgesetz ein. Um wirtschaftliche Partnerschaften zwischen Chemnitz und afrikanischen L\u00e4ndern aufzubauen, initiierte Heinrich im Juni 2014 die Netzwerktagung \u201eBusiness trifft Afrika\u201c gemeinsam mit der Industrie- und Handelskammer Chemnitz, die seither j\u00e4hrlich stattfindet und gro\u00dfe Resonanz erf\u00e4hrt.",
				"en": "Frank Heinrich (born January 25, 1964 in Siegen) is a German theologian, social pedagogue and politician (CDU).  Since summer 2012 Heinrich has been involved, together with 12 other CDU colleagues in the strengthening of tax equality of same-sex partnerships within the law on civil partnership. For establishing economic collaborations between Chemnitz and African countries, Heinrich and the Chamber of industry and commerce Chemnitz initiated the network conference \u201cBusiness trifft Afrika\u201d (\u201cBusiness meets Africa\u201d). Starting in 2014 it became an annual event."
			},
			"full_name": "Frank Heinrich",
			"images": {
				"copyright": "Susanne Domaratius",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "191.jpg",
				"pathToThumb": "191_t.jpg"
			},
			"name": "Frank Heinrich",
			"party": "CDU",
			"pid": "191",
			"self_bird": "dohle",
			"twittering": {
				"twitterId": 43959185,
				"twitterUserName": "FrankHeinrich"
			}
		},
		"192": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "Gabriela Heinrich (* 18. April 1963 in Berlin) ist eine deutsche Politikerin (SPD).  Bei der Bundestagswahl 2013 kandidierte sie im Bundestagswahlkreis N\u00fcrnberg-Nord. Sie konnte zwar das Direktmandat nicht gewinnen, zog aber \u00fcber die bayrische Landesliste der SPD in den Bundestag ein.",
				"en": "Gabriela Heinrich is a German politician. She is a member of the SPD."
			},
			"full_name": "Gabriela Heinrich",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "192.jpg",
				"pathToThumb": "192_t.jpg"
			},
			"name": "Gabriela Heinrich",
			"party": "SPD",
			"pid": "192",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": "296121556",
				"twitterUserName": "GaHeinrich"
			}
		},
		"193": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Marcus Held (* 15. Oktober 1977 in Ludwigshafen am Rhein) ist ein deutscher Politiker (SPD), Stadtb\u00fcrgermeister von Oppenheim und Mitglied des Deutschen Bundestages.  Bei der Bundestagswahl 2013 kandidierte er als Direktkandidat f\u00fcr den Bundestagswahlkreis Worms und zog \u00fcber die Landesliste in den Bundestag ein. Er geh\u00f6rt dort dem Ausschuss f\u00fcr Wirtschaft und Energie als ordentliches Mitglied sowie dem Ausschuss f\u00fcr Umwelt und Bauen als Stellvertreter an.",
				"en": "Marcus Held is a German politician. He is a member of the SPD."
			},
			"full_name": "Marcus Held",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "193.jpg",
				"pathToThumb": "193_t.jpg"
			},
			"name": "Marcus Held",
			"party": "SPD",
			"pid": "193",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": "2860802891",
				"twitterUserName": "MarcusHeld_SPD"
			}
		},
		"196": {
			"citizen_bird": "fitis",
			"cv": {
				"de": "Wolfgang Hellmich (* 5. Mai 1958 in Meyerich bei Welver) ist ein deutscher Politiker (SPD).  In der 18. Legislaturperiode ist er Ordentliches Mitglied im Verteidigungsausschuss und seit 20. Mai 2015 dessen Vorsitzender als Nachfolger von Hans-Peter Bartels, der zum Wehrbeauftragten des Deutschen Bundestages ernannt wurde. Im Ausschuss f\u00fcr Gesundheit ist er stellvertretendes Mitglied. Dar\u00fcber hinaus ist Hellmich stellvertretendes Mitglied des Unterausschusses Abr\u00fcstung, R\u00fcstungskontrolle und Nichtverbreitung sowie der Parlamentarischen Versammlung der NATO (kurz: NATO PV). Er ist Mitglied und Leiter der Deutschen Delegation in der Interparlamentarischen Konferenz f\u00fcr die Gemeinsame Au\u00dfen- und Sicherheitspolitik (GASP) und die Gemeinsame Sicherheits- und Verteidigungspolitik (GSVP) der EU. Zudem ist er Mitglied der IGBCE und der IG Metall.",
				"en": "Wolfgang Hellmich is a German politician. He is a member of the SPD."
			},
			"full_name": "Wolfgang Hellmich",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "196.jpg",
				"pathToThumb": "196_t.jpg"
			},
			"name": "Wolfgang Hellmich",
			"party": "SPD",
			"pid": "196",
			"self_bird": "saatkraehe",
			"twittering": {
				"twitterId": 19924848,
				"twitterUserName": "WHellmich"
			}
		},
		"2": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Katrin Albsteiger (* 20. November 1983 in Ulm als Katrin Poleschner) ist eine deutsche Politikerin der CSU. Sie ist seit der 18. Wahlperiode Mitglied des Deutschen Bundestages sowie eine der vier stellvertretenden Bundesvorsitzenden der Jungen Union.  Im Bundestag ist Albsteiger ordentliches Mitglied im Ausschuss f\u00fcr die Angelegenheiten der Europ\u00e4ischen Union und im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung sowie Schriftf\u00fchrerin des Bundestages. Sie ist stellvertretendes Mitglied im Ausschuss f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung und im Innenausschuss. Au\u00dferdem ist sie Delegierte in der Interparlamentarischen Union (IPU).[11] Albsteiger war die einzige Abgeordnete der CSU, die im Mai 2014 gegen das Rentenpaket der Bundesregierung stimmte. Dies begr\u00fcndete sie mit ihrer Ablehnung der \u201eRente mit 63\u201c, die die junge Generation zu einseitig belaste.",
				"en": "Katrin Albsteiger (n\u00e9e Poleschner, born 20 November 1983) is a German politician from the Christian Social Union of Bavaria.  In parliament, Albsteiger serves on the Committee on European Affairs and on the Committee on Education, Research and Technology Assessment. She is her parliamentary group\u2019s rapporteur on youth unemployment and gender equality as well as on Greece, Cyprus and Austria."
			},
			"full_name": "Katrin Albsteiger",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "2.jpg",
				"pathToThumb": "2_t.jpg"
			},
			"name": "Katrin Albsteiger",
			"party": "CSU",
			"pid": "2",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 429357796,
				"twitterUserName": "dieAlbsteigerin"
			}
		},
		"204": {
			"citizen_bird": "tannenmeise",
			"cv": {
				"de": "Gabriele Hiller-Ohm (geb. Ohm; * 28. Februar 1953 in L\u00fcbeck) ist eine deutsche Politikerin (SPD).  Seit 2002 ist sie Mitglied des Deutschen Bundestages. Sie ist Mitglied im Ausschuss f\u00fcr Arbeit und Soziales sowie im Ausschuss f\u00fcr Tourismus, seit dem 18. Bundestag ist sie stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft. Seit 2014 ist Gabriele Hiller-Ohm Vorsitzende der Arbeitsgruppe Tourismus und damit tourismuspolitische Sprecherin der SPD-Bundestagsfraktion. Seit 2005 ist sie stellvertretende Sprecherin der Landesgruppe Schleswig-Holstein in der SPD-Bundestagsfraktion.",
				"en": "Gabriele Hiller-Ohm is a German politician. She is a member of the SPD."
			},
			"full_name": "Gabriele Hiller-Ohm",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "204.jpg",
				"pathToThumb": "204_t.jpg"
			},
			"name": "Gabriele Hiller-Ohm",
			"party": "SPD",
			"pid": "204",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 25816024,
				"twitterUserName": "GabiHillerOhm"
			}
		},
		"209": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "Christian Hirte (* 23. Mai 1976 in Bad Salzungen) ist ein deutscher Politiker (CDU). Er ist seit 2008 Mitglied des Deutschen Bundestages und seit 2014 Stellvertretender Landesvorsitzender der CDU Th\u00fcringen.  Er ist derzeit ordentliches Mitglied im Haushaltsausschuss und im Rechnungspr\u00fcfungsausschuss des Haushaltsausschusses sowie stellvertretendes Mitglied im Finanzausschuss. In der vergangenen Wahlperiode geh\u00f6rte Christian Hirte dem Ausschuss f\u00fcr Tourismus, dem Ausschuss f\u00fcr Umwelt, Naturschutz und Reaktorsicherheit und als stellvertretendes Mitglied dem Verteidigungsausschuss an. Dar\u00fcber hinaus war er in der 16. Wahlperiode Obmann der CDU-CSU-Fraktion im Unterausschuss Neue Medien sowie Ausschussmitglied des 2. Untersuchungsausschusses bez\u00fcglich der Hypo Real Estate (HRE), wobei Christian Hirte die CDU-CSU-Fraktion vertreten hat.",
				"en": "Christian Hirte is a German politician. He is a member of the CDU."
			},
			"full_name": "Christian Hirte",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "209.jpg",
				"pathToThumb": "209_t.jpg"
			},
			"name": "Christian Hirte",
			"party": "CDU",
			"pid": "209",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 37642893,
				"twitterUserName": "ChristianHirte"
			}
		},
		"210": {
			"citizen_bird": "dohle",
			"cv": {
				"de": "Heribert Hirte (* 31. M\u00e4rz 1958 in K\u00f6ln) ist Professor f\u00fcr Rechtswissenschaft an der Universit\u00e4t Hamburg und dort Gesch\u00e4ftsf\u00fchrender Direktor des Seminars f\u00fcr Handels-, Schifffahrts- und Wirtschaftsrecht. Zudem gewann er bei der Bundestagswahl 2013 als einziger K\u00f6lner CDU-Kandidat ein Direktmandat f\u00fcr den Deutschen Bundestag, und zwar im Wahlkreis K\u00f6ln II.",
				"en": "Heribert Hirte is a German politician. He is a member of the CDU."
			},
			"full_name": "Prof. Dr. Heribert Hirte",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "210.jpg",
				"pathToThumb": "210_t.jpg"
			},
			"name": "Heribert Hirte",
			"party": "CDU",
			"pid": "210",
			"self_bird": "buchfink",
			"twittering": {
				"twitterId": 1513671236,
				"twitterUserName": "HHirte"
			}
		},
		"211": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Thomas Hitschler (* 22. Juni 1982 in Landau) ist ein deutscher Politiker (SPD).  Er ist Vorsitzender des SPD-Unterbezirks S\u00fcdpfalz. Seit den Kommunalwahlen 2014 geh\u00f6rt er zudem dem Kreistag S\u00fcdliche Weinstra\u00dfe an. Hitschler ist Vorstandsmitglied im Netzwerk Berlin, Mitherausgeber der Zeitschrift Berliner Republik[3] und stellvertretender Vorsitzender des gleichnamigen Vereins. Zudem geh\u00f6rt er der Kontrollkommission der SPD an. Im Juli 2016 wurde Hitschler in den Bezirksvorstand der AWO Pfalz gew\u00e4hlt.",
				"en": "Thomas Hitschler is a German politician. He is a member of the SPD."
			},
			"full_name": "Thomas Hitschler",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "211.jpg",
				"pathToThumb": "211_t.jpg"
			},
			"name": "Thomas Hitschler",
			"party": "SPD",
			"pid": "211",
			"self_bird": "ringeltaube",
			"twittering": {
				"twitterId": 292151515,
				"twitterUserName": "ThomasHitschler"
			}
		},
		"214": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "Thorsten Hoffmann (* 5. Februar 1961 in Castrop-Rauxel) ist ein deutscher Politiker (CDU) aus Dortmund. Hoffmann ist seit 2015 Mitglied des Deutschen Bundestages. Er geh\u00f6rt dort dem Innenausschuss als ordentliches, und dem 2. NSU-Untersuchungsausschuss sowie dem Petitionsausschuss als stellvertretendes Mitglied an. Er ist Mitglied der Arbeitsgruppe Innen, der Arbeitnehmergruppe, der Arbeitsgruppe Kommunalpolitik und der Arbeitsgruppe Ruhrgebiet.",
				"en": "Thorsten Hoffmann (born 5. February 1961) is a German politician of the Christian Democratic Union (CDU) from Dortmund. In 2009, he was elected into Dortmund city council, of which he is still a member. Since 2011, Thorsten Hoffmann is chairman of the Christian Democratic Union in the district of Dortmund-Hombruch and in the urban quarter of Dortmund-Kirchh\u00f6rde. Since 2015, Thorsten Hoffmann is a member of the German Bundestag."
			},
			"full_name": "Thorsten Hoffmann",
			"images": {
				"copyright": "Falko W\u00fcbbecke",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "214.jpg",
				"pathToThumb": "214_t.jpg"
			},
			"name": "Thorsten Hoffmann",
			"party": "CDU",
			"pid": "214",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 588942667,
				"twitterUserName": "HoffmannMdB"
			}
		},
		"215": {
			"citizen_bird": "tannenmeise",
			"cv": {
				"de": "Anton \u201eToni\u201c Hofreiter (* 2. Februar 1970 in M\u00fcnchen) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und Biologe. Er ist seit 2005 Mitglied des Deutschen Bundestages und war dort von 2011 bis 2013 Vorsitzender des Ausschusses f\u00fcr Verkehr, Bau und Stadtentwicklung. Nach der Bundestagswahl 2013 bewarb er sich um den Vorsitz der Bundestagsfraktion seiner Partei. Am 8. Oktober 2013 wurde er von 49 der 61 Gr\u00fcnen-Bundestagsabgeordneten in dieses Amt gew\u00e4hlt, das er mit der Co-Vorsitzenden Katrin G\u00f6ring-Eckardt teilt.",
				"en": "Anton \"Toni\" Hofreiter (born 2 February 1970 in Munich) is a German biologist and politician.  As a member of Alliance '90/The Greens, Hofreiter has been a member of the Bundestag since the 2005 elections. Between 2005 and 2013, he served as member of the Committee on Transport, Building and Urban Development; he served as chairman of the committee from 2011 until 2013. He was also member of the German-Austrian Parliamentary Friendship Group. Hofreiter has been co-chair of the Green Party's parliamentary group, together with Katrin G\u00f6ring-Eckardt, since October 2013."
			},
			"full_name": "Dr. Anton Hofreiter",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "215.jpg",
				"pathToThumb": "215_t.jpg"
			},
			"name": "Anton Hofreiter",
			"party": "GR\u00dcNE",
			"pid": "215",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 442678902,
				"twitterUserName": "ToniHofreiter"
			}
		},
		"216": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Inge Dora Minna H\u00f6ger (* 29. Oktober 1950 in Diepholz) ist eine deutsche Politikerin (Die Linke).  Von 2006 bis 2013 war sie Mitglied im Verteidigungsausschuss und hier f\u00fcr die Auslandseins\u00e4tze auf dem Balkan (Operation Althea, KFOR) und im Mittelmeer zust\u00e4ndig. Auch dem Untersuchungsausschuss zur Aufkl\u00e4rung der Gefangenschaft von Murat Kurnaz in Guant\u00e1namo geh\u00f6rte sie an.  In der 17. Legislaturperiode war H\u00f6ger Obfrau ihrer Fraktion im Untersuchungsausschuss \u00fcber den Luftangriff bei Kundus. Auch dem Untersuchungsausschuss \u00fcber die Beschaffung der Euro-Hawk-Drohne geh\u00f6rte H\u00f6ger an.  Seit 2009 ist sie abr\u00fcstungspolitische Sprecherin ihrer Fraktion und zudem Mitglied im Unterausschuss f\u00fcr Abr\u00fcstung. Nach der Wahl 2013 arbeitet sie au\u00dferdem im Ausschuss f\u00fcr Menschenrechte und humanit\u00e4re Hilfe mit.",
				"en": "Inge H\u00f6ger (born October 29, 1950 in Diepholz) is a German politician. From 2005 to 2006 she was a deputy chair of The Left parliamentary group in the Bundestag.  Since 2005, Inge H\u00f6ger has been a member of the Bundestag. At the 2009 election, she unsuccessfully contested the Herford \u2013 Minden-L\u00fcbbecke II constituency, but was elected from the North Rhine Westphalia land list."
			},
			"full_name": "Inge H\u00f6ger",
			"images": {
				"license": "custom-linke",
				"pathToImage": "216.jpg",
				"pathToThumb": "216_t.jpg"
			},
			"name": "Inge H\u00f6ger",
			"party": "DIE LINKE",
			"pid": "216",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": "1595306917",
				"twitterUserName": "Scheringer_W"
			}
		},
		"217": {
			"citizen_bird": "grauschnaepper",
			"cv": {
				"de": "Eva Alexandra Ingrid Irmgard Anna H\u00f6gl, geborene Kampmeyer, (* 6. Januar 1969 in Osnabr\u00fcck) ist eine Politikerin der SPD, seit 2009 Mitglied des Deutschen Bundestages und zurzeit stellvertretende Fraktionsvorsitzende der SPD-Bundestagsfraktion.  H\u00f6gl ist Mitglied bei der Gewerkschaft ver.di, Pro Asyl, AWO, Deutscher Juristinnenbund e.V., DAFG \u2013 Deutsch-Arabische Freundschaftsgesellschaft e.V. (Vorstandsmitglied), Deutsch-Britische Gesellschaft, Europa-Union Deutschland e.V. (Vizepr\u00e4sidentin), Transparency International e.V., Marie-Schlei-Verein, evangelisch-lutherische Kirche, Kunstverein Haus am L\u00fctzowplatz, F\u00f6rderverein Willy-Brandt-Zentrum Jerusalem, Lebenshilfe e.V., Gegen Vergessen f\u00fcr Demokratie e.V., Schlichtungsstelle der Rechtsanwaltschaft (Beirat), Deutsche Vereinigung f\u00fcr Parlamentsfragen e.V. (Vorstand), und Berliner Ratschlag f\u00fcr Demokratie.",
				"en": "Eva H\u00f6gl is a German politician. She is a member of the SPD."
			},
			"full_name": "Dr. Eva H\u00f6gl",
			"images": {
				"copyright": "Eva H\u00f6gl",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "217.jpg",
				"pathToThumb": "217_t.jpg"
			},
			"name": "Eva H\u00f6gl",
			"party": "SPD",
			"pid": "217",
			"self_bird": "eichelhaeher",
			"twittering": {
				"twitterId": 35846585,
				"twitterUserName": "EvaHoegl"
			}
		},
		"218": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "B\u00e4rbel H\u00f6hn geb. Christensen (* 4. Mai 1952 in Flensburg) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie ist Vorsitzende des Ausschusses f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit. Sie war seit 1990 ohne Unterbrechung Landtagsabgeordnete, Landesministerin oder Bundestagsabgeordnete.  Bei der Bundestagswahl im September 2005 wurde sie Mitglied des Deutschen Bundestages. Hier war sie von Oktober 2005 bis Mai 2006 Vorsitzende des Ausschusses f\u00fcr Ern\u00e4hrung, Landwirtschaft und Verbraucherschutz. Von Mai 2006 bis 2013 war sie stellvertretende Vorsitzende der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen und politische Koordinatorin des Fraktionsarbeitskreises II Umwelt & Energie, Verbraucher & Agrar, Verkehr & Bau, Tourismus, Sport.  Ehrenamtlich engagiert sich B\u00e4rbel H\u00f6hn aktiv in der Bio-Bewegung. F\u00fcr Aufsehen sorgte sie im Jahre 2001, als sie mit einigen Landwirten gegen K\u00fcrzungen des Agrarzuschusses demonstrierte.",
				"en": "B\u00e4rbel H\u00f6hn (born May 4, 1952) is a German politician for Alliance '90/The Greens. She has been a member of the Bundestag since the German federal election, 2005, after serving as Minister of Agriculture of Northrhine-Westphalia from 1995 to 2005.  Following the 2013 federal elections, H\u00f6hn stated that agreeing to a coalition with Chancellor Angela Merkel would be a \u201ckamikaze\u201d act for the Green Party. Exploratory coalition talks with Merkel\u2019s Christian Democratic Union ended soon after, without results.  Since 2014, H\u00f6hn has been chairwoman of the Committee on the Environment, Nature Conservation and Nuclear Safety. Having participated in various UN climate conferences, she is a regular speaker at international climate and energy conventions."
			},
			"full_name": "B\u00e4rbel H\u00f6hn",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "218.jpg",
				"pathToThumb": "218_t.jpg"
			},
			"name": "B\u00e4rbel H\u00f6hn",
			"party": "GR\u00dcNE",
			"pid": "218",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": 22900494,
				"twitterUserName": "BaerbelHoehn"
			}
		},
		"22": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Matthias Bartke (* 16. Januar 1959 in Bremen) ist ein deutscher Politiker der Sozialdemokratischen Partei Deutschlands (SPD) und Mitglied des Deutschen Bundestages in der 18. Wahlperiode. Seit Januar 2016 ist er als Justiziar Mitglied des gesch\u00e4ftsf\u00fchrenden Vorstands der SPD-Bundestagsfraktion.  Er ist im Bundestag Mitglied im Ausschuss f\u00fcr Arbeit und Soziales, wo er die Schwerpunkte Arbeitsmarkt- und Behindertenpolitik bearbeitet; gleichzeitig ist er Mitglied im Ausschuss f\u00fcr Recht und Verbraucherschutz sowie stellvertretendes Mitglied im Ausw\u00e4rtigen Ausschuss. Seit Januar 2016 ist Bartke Mitglied im \u00c4ltestenrat sowie im Ausschuss f\u00fcr Wahlpr\u00fcfung, Immunit\u00e4t und Gesch\u00e4ftsordnung.",
				"en": "Matthias Bartke (born 16 January 1959) is a German politician of the Social Democratic Party of Germany (SPD). He was elected a member of the German parliament (Bundestag) in the federal election of 2013. Since January 2016 he is the legal advisor and thus a member of the Board of Managing Directors of the SPD-Parliamentary Group."
			},
			"full_name": "Dr. Matthias Bartke",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "22.jpg",
				"pathToThumb": "22_t.jpg"
			},
			"name": "Matthias Bartke",
			"party": "SPD",
			"pid": "22",
			"self_bird": "gartengrasmuecke",
			"twittering": {
				"twitterId": 3376953843,
				"twitterUserName": "matthiasbartke"
			}
		},
		"220": {
			"citizen_bird": "weisskopfseeadler",
			"cv": {
				"de": "Franz-Josef Holzenkamp (* 8. Januar 1960 in Kamperfehn) ist ein deutscher Politiker (CDU).  Seit 2005 ist Franz-Josef Holzenkamp Mitglied des Deutschen Bundestages. Er ist dabei stets als direkt gew\u00e4hlter Abgeordneter des Wahlkreises Cloppenburg \u2013 Vechta in den Bundestag eingezogen.  Er ist Vorsitzender der Arbeitsgruppe Ern\u00e4hrung und Landwirtschaft der CDU/CSU-Bundestagsfraktion. Neben seinem Bundestagsmandat ist Holzenkamp Aufsichtsratvorsitzender bei der AGRAVIS Raiffeisen AG und Mitglied in verschiedenen Gremien der LVM.",
				"en": "Franz-Josef Holzenkamp is a German politician. He is a member of the CDU."
			},
			"full_name": "Franz-Josef Holzenkamp",
			"images": {
				"copyright": "Matthias Niehues",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "220.jpg",
				"pathToThumb": "220_t.jpg"
			},
			"name": "Franz-Josef Holzenkamp",
			"party": "CDU",
			"pid": "220",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": "2158321941",
				"twitterUserName": "fj_josef"
			}
		},
		"223": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Bettina Hornhues, geb. Bartels (* 19. Juli 1972 in Bremen) ist eine deutsche Bankkauffrau, Politikerin (CDU) und seit 2013 Mitglied im Deutschen Bundestag.  Sie ist Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend, dort ist sie Berichterstatterin f\u00fcr die Vereinbarkeit von Familie und Beruf. Hornhues war 2000/01 Betriebsratsmitglied der NordFinanz Bank und war von 2010 bis 2013 stellvertretendes Betriebsratsmitglied der NordFinanz Bank.",
				"en": "Bettina Hornhues is a German politician. She is a member of the CDU."
			},
			"full_name": "Bettina Hornhues",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "223.jpg",
				"pathToThumb": "223_t.jpg"
			},
			"name": "Bettina Hornhues",
			"party": "CDU",
			"pid": "223",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 275204467,
				"twitterUserName": "BettinaHornhues"
			}
		},
		"224": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Charles Muhamed Huber (* 3. Dezember 1956 in M\u00fcnchen als Karl-Heinz Huber) ist ein deutscher Schauspieler, Autor und Politiker. Bei der Bundestagswahl 2013 wurde er f\u00fcr die CDU in den Deutschen Bundestag gew\u00e4hlt.  Im Deutschen Bundestag ist Huber seither ordentliches Mitglied im Ausschuss f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung, stellvertretendes Mitglied im Ausw\u00e4rtigen Ausschuss, im Unterausschuss Zivile Krisenpr\u00e4vention, Konfliktbearbeitung und vernetztes Handeln sowie im Ausschuss f\u00fcr Wirtschaft und Energie. Er bekleidet zudem das Amt des stellvertretenden Vorsitzenden des Arbeitskreises Afrika der CDU/CSU-Bundestagsfraktion sowie des Vorsitzenden der Parlamentariergruppe Englisch- und Portugiesischsprachige Staaten West- und Zentralafrikas. Huber ist stellvertretendes Mitglied der Parlamentarischen Versammlung der NATO.",
				"en": "Charles M. Huber (born December 3, 1956[1] in Munich as Karl-Heinz Huber) is a German politician and actor.  He was one of the first two Bundestag members of African ancestry, alongside Karamba Diaby, who were both elected on September 23, 2013. Huber is a member of the Christian Democratic Union, and was elected in the federal state of Hesse whereas he failed to win the Darmstadt constituency.  In addition, Huber is the chairman of the Parliamentary Friendship Group for Relations with the English- and Portuguese-speaking States of West and Central Africa (Gambia, Ghana, Guinea-Bissau, Cape Verde, Liberia, Nigeria, Sierra Leone); a member of the Parliamentary Friendship Group for Relations with the Francophone States of West and Central Africa; and a member of the Parliamentary Friendship Group for Relations with the States of Central America."
			},
			"full_name": "Charles M. Huber",
			"images": {
				"copyright": "DBT/Renate Blanke",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "224.jpg",
				"pathToThumb": "224_t.jpg"
			},
			"name": "Charles M. Huber",
			"party": "CDU",
			"pid": "224",
			"self_bird": "dohle",
			"twittering": {
				"twitterId": 81142025,
				"twitterUserName": "charlesmhuber49"
			}
		},
		"226": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Andrej Konstantin Hunko (* 29. September 1963 in M\u00fcnchen) ist Mitglied des Deutschen Bundestags und der Parlamentarischen Versammlung des Europarates f\u00fcr die Partei Die Linke.  Zur Bundestagswahl 2013 hat ihn die Wahlversammlung der Mitglieder der Partei Die Linke in der Stadt Aachen ohne Gegenstimmen erneut als Direktkandidaten f\u00fcr den Wahlkreis Aachen-Stadt aufgestellt.",
				"en": "Andrej Konstantin Hunko (born 29 September 1963 in Munich) is a German MP of The Left party for Aachen."
			},
			"full_name": "Andrej Hunko",
			"images": {
				"license": "custom-linke",
				"pathToImage": "226.jpg",
				"pathToThumb": "226_t.jpg"
			},
			"name": "Andrej Hunko",
			"party": "DIE LINKE",
			"pid": "226",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 87281380,
				"twitterUserName": "AndrejHunko"
			}
		},
		"229": {
			"citizen_bird": "zilpzalp",
			"cv": {
				"de": "Matthias Ilgen (* 6. Dezember 1983 in Husum) ist ehemaliger Wrestler und deutscher Politiker der SPD. Er wurde bei der Bundestagswahl 2013 \u00fcber die Landesliste Schleswig-Holstein in den 18.\u00a0Deutschen Bundestag gew\u00e4hlt.  Dort will er nach eigenen Angaben die Energiewende in Schleswig-Holstein vorantreiben. Matthias Ilgen ist ordentliches Mitglied im Ausschuss f\u00fcr Verteidigung und Ausschuss f\u00fcr Wirtschaft und Energie",
				"en": "Matthias Ilgen is a German politician. He is a member of the SPD."
			},
			"full_name": "Matthias Ilgen",
			"images": {
				"copyright": "A. Birresborn",
				"license": "custom-spd",
				"pathToImage": "229.jpg",
				"pathToThumb": "229_t.jpg"
			},
			"name": "Matthias Ilgen",
			"party": "SPD",
			"pid": "229",
			"self_bird": "stieglitz",
			"twittering": {
				"twitterId": 2156382000,
				"twitterUserName": "MatthiasIlgen"
			}
		},
		"23": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "S\u00f6ren Bartol (* 4. September 1974 in Hamburg) ist ein deutscher Politiker der Sozialdemokratischen Partei Deutschlands (SPD).  Im Deutschen Bundestag war Bartol von Februar 2005 bis September 2009 Sprecher der Landesgruppe Hessen. Von Dezember 2005 bis Oktober 2011 war er zun\u00e4chst stellvertretender Sprecher, dann bis Januar 2014 Sprecher der Arbeitsgruppe \u201eVerkehr, Bau und Stadtentwicklung\u201c der SPD-Bundestagsfraktion. Seitdem ist er stellvertretendes Mitglied in der Arbeitsgruppe \u201eDigitale Agenda\u201c.  S\u00f6ren Bartol geh\u00f6rt seit Januar 2008 dem Vorstand der SPD-Bundestagsfraktion an. Seit Dezember 2013 ist Bartol stellvertretender Vorsitzender der SPD-Bundestagsfraktion f\u00fcr den Bereich Verkehr, Bau und digitale Infrastruktur sowie Digitale Agenda.  Bartol ist Mitglied im Netzwerk Berlin, das innerhalb der SPD zu den Reformern gez\u00e4hlt wird.",
				"en": "S\u00f6ren Bartol (born 4 September 1974, in Hamburg) is a German politician and member of the SPD.  Bartol has been a member of the German Bundestag since the 2002 federal election, representing the electoral district of Marburg.  Bartol is a member of the Parliamentary Friendship Group for Relations with the States of Central America, the Parliamentary Friendship Group for Relations with the States of South America \u2013 which is in charge of maintaining inter-parliamentary relations with Argentina, Bolivia, Chile, Ecuador, Guyana, Colombia, Paraguay, Peru, Suriname, Uruguay, Venezuela \u2013 and the German-Brazilian Parliamentary Friendship Group."
			},
			"full_name": "S\u00f6ren Bartol",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "23.jpg",
				"pathToThumb": "23_t.jpg"
			},
			"name": "S\u00f6ren Bartol",
			"party": "SPD",
			"pid": "23",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 57293969,
				"twitterUserName": "soerenbartol"
			}
		},
		"231": {
			"citizen_bird": "gartenbaumlaeufer",
			"cv": {
				"de": "Dieter Gerald Janecek (* 25. Mai 1976 in Pirmasens) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und Mitglied des Deutschen Bundestages.  Janecek gilt als regelm\u00e4\u00dfiger Ansprechpartner von Industrie und Mittelstand und ist f\u00fcr seine aktive Rolle in den sozialen Medien bekannt.",
				"en": "Dieter Janecek is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Dieter Janecek",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "231.jpg",
				"pathToThumb": "231_t.jpg"
			},
			"name": "Dieter Janecek",
			"party": "GR\u00dcNE",
			"pid": "231",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 52024480,
				"twitterUserName": "DJanecek"
			}
		},
		"233": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Thomas Jarzombek (* 28. April 1973 in D\u00fcsseldorf) ist ein deutscher Politiker (CDU), Mitglied des Bundestages, Sprecher der CDU/CSU Bundestagsfraktion f\u00fcr Digitale Agenda und Vorsitzender der CDU D\u00fcsseldorf.  Im Rahmen der Initiative Wissenschaftsjahr 2014 Die Digitale Gesellschaft des Bundesministeriums f\u00fcr Bildung und Forschung und der Gesellschaft f\u00fcr Informatik wurde er mit der Auszeichnung Deutschlands Digitale K\u00f6pfe geehrt. Die Auszeichnung gilt jenen, die mit ihren Ideen und Projekten die digitale Entwicklung in Deutschland vorantreiben.",
				"en": "Thomas Jarzombek is a German politician. He is a member of the CDU."
			},
			"full_name": "Thomas Jarzombek",
			"images": {
				"copyright": "Tobias Koch",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "233.jpg",
				"pathToThumb": "233_t.jpg"
			},
			"name": "Thomas Jarzombek",
			"party": "CDU",
			"pid": "233",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 37105136,
				"twitterUserName": "tj_tweets"
			}
		},
		"234": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Ursula \u201eUlla\u201c Jelpke (* 9. Juni 1951 in Hamburg) ist eine deutsche Publizistin und Politikerin (Die Linke, zuvor KB, GAL und PDS). Von 1981 bis 1989 geh\u00f6rte sie f\u00fcr die GAL der Hamburgischen B\u00fcrgerschaft an, von 1990 bis 2002 dem Bundestag (parteiloses Mitglied der PDS-Abgeordnetengruppe). Von 2002 bis 2005 war sie Innenressortleiterin der jungen Welt. Seit 2005 geh\u00f6rt sie f\u00fcr die PDS bzw. Linke wieder dem Bundestag an. In ihrer Partei geh\u00f6rt sie seit 2012 zum SprecherInnnenrat der Str\u00f6mung \u201eAntikapitalistische Linke\u201c.",
				"en": "Ursula \"Ulla\" Jelpke (born 9 June 1951 in Hamburg) is a Marxist German journalist and politician. Jelpke is member of the German Bundestag where she is domestic affairs spokesperson for the party Die Linke and represents the party in the internal affairs committee and the legal affairs committee."
			},
			"full_name": "Ulla Jelpke",
			"images": {
				"license": "custom-linke",
				"pathToImage": "234.jpg",
				"pathToThumb": "234_t.jpg"
			},
			"name": "Ulla Jelpke",
			"party": "DIE LINKE",
			"pid": "234",
			"self_bird": "bachstelze",
			"twittering": {
				"twitterId": "2837971145",
				"twitterUserName": "UllaJelpke"
			}
		},
		"238": {
			"citizen_bird": "star",
			"cv": {
				"de": "Franz Josef Friedrich Jung (* 5. M\u00e4rz 1949 in Erbach) ist ein deutscher Politiker (CDU). Er war von 1998 bis 2014 stellvertretender Vorsitzender der hessischen CDU.  2005 zog Jung \u00fcber die Landesliste Hessen in den Bundestag ein. 2009 und 2013 gewann Jung das Direktmandat f\u00fcr den Deutschen Bundestag im Wahlkreis Gro\u00df-Gerau.  Am 22. November 2005 (nach der Bundestagswahl 2005) wurde Jung als Bundesminister der Verteidigung in die von Bundeskanzlerin Angela Merkel gef\u00fchrte Bundesregierung berufen (Kabinett Merkel I).  Am 28. Oktober 2009 (nach der Bundestagswahl 2009) \u00fcbernahm er das Bundesministerium f\u00fcr Arbeit und Soziales im Kabinett Merkel II. Jungs Nachfolger im Verteidigungsministerium wurde Karl-Theodor zu Guttenberg.  Im 18. Bundestag ist Jung stellvertretender Fraktionsvorsitzender und Beauftragter f\u00fcr Kirchen und Religionsgemeinschaften der CDU/CSU-Bundestagsfraktion. Er ist stellvertretendes Mitglied im Rechtsausschuss, im Petitionsausschuss, im Ausw\u00e4rtigen Ausschuss und im Verteidigungsausschuss des Deutschen Bundestages.",
				"en": "Franz Josef Jung (born 5 March 1949) is a German politician of the Christian Democratic Union (CDU). In October 2009 he became Minister of Labour and Social Affairs but resigned a month later.  From 2009 until 2013, Jung was a member of the parliamentary body in charge of selecting the judges of the Highest Courts of Justice, namely the Federal Court of Justice (BGH), the Federal Administrative Court (BVerwG), the Federal Fiscal Court (BFH), the Federal Labour Court (BAG), and the Federal Social Court (BSG).  After the 2013 elections, Jung was appointed deputy chairperson of the CDU/CSU parliamentary group in charge of food, agriculture and consumer protection. Following the death of Andreas Schockenhoff, he took on the foreign policy portfolio in early 2015."
			},
			"full_name": "Dr. Franz Josef Jung",
			"images": {
				"copyright": "MdB-B\u00fcro Dr. Jung",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "238.jpg",
				"pathToThumb": "238_t.jpg"
			},
			"name": "Franz Josef Jung",
			"party": "CDU",
			"pid": "238",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 860067295,
				"twitterUserName": "jungfj"
			}
		},
		"239": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Xaver Jung (* 23. M\u00e4rz 1962 in Kusel) ist ein deutscher Politiker und Bundestagsabgeordneter der (CDU).  Bei der Bundestagswahl 2013 wurde Jung \u00fcber Platz 13 der CDU-Landesliste Rheinland-Pfalz zum Mitglied des Deutschen Bundestages gew\u00e4hlt. Daneben war er Direktkandidat im Bundestagswahlkreis Kaiserslautern gewesen. Als Abgeordneter sitzt Jung im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung. Er ist als Berichterstatter f\u00fcr die vorschulische und allgemeine schulische Bildung, sowie f\u00fcr die Weiterbildung verantwortlich.",
				"en": "Xaver Jung is a German politician. He is a member of the CDU."
			},
			"full_name": "Xaver Jung",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "239.jpg",
				"pathToThumb": "239_t.jpg"
			},
			"name": "Xaver Jung",
			"party": "CDU",
			"pid": "239",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 1313226002,
				"twitterUserName": "XaverJung"
			}
		},
		"24": {
			"citizen_bird": "kohlmeise",
			"cv": {
				"de": "Dietmar Gerhard Bartsch (* 31. M\u00e4rz 1958 in Stralsund) ist ein deutscher Politiker (Die Linke). Er ist Mitglied des Deutschen Bundestages und seit dem 13. Oktober 2015 zusammen mit Sahra Wagenknecht Vorsitzender der Linksfraktion und damit Oppositionsf\u00fchrer des 18. Bundestags. Er war zuvor Bundesschatzmeister der PDS, Bundesgesch\u00e4ftsf\u00fchrer der Linkspartei sowie Zweiter Stellvertretender Fraktionschef. Er wird, im Gegensatz zu Wagenknecht, zum moderaten, reformorientierten Parteifl\u00fcgel gez\u00e4hlt.",
				"en": "Dietmar Gerhard Bartsch (born 31 March 1958, in Stralsund) is a German politician, former Federal Whip (Bundesgesch\u00e4ftsf\u00fchrer) of the Party of Democratic Socialism (1997\u20132002, 2005\u20132007) and Die Linke (2007\u20132010) and member of the Bundestag. He is married and has two children. Since October 2015, Bartsch has served as the co-leader of Die Linke's parliamentary group, a position he holds with Sahra Wagenknecht. Bartsch represents the moderate, reformist wing of the party."
			},
			"full_name": "Dr. Dietmar Bartsch",
			"images": {
				"license": "custom-linke",
				"pathToImage": "24.jpg",
				"pathToThumb": "24_t.jpg"
			},
			"name": "Dietmar Bartsch",
			"party": "DIE LINKE",
			"pid": "24",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 2320519958,
				"twitterUserName": "DietmarBartsch"
			}
		},
		"240": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Frank Michael Junge (* 5. Mai 1967 in Halle/Saale) ist ein deutscher Politiker (SPD).  Zur Bundestagswahl 2013 kandidierte Junge als Direktkandidat im Bundestagswahlkreis Ludwigslust-Parchim II \u2013 Nordwestmecklenburg II \u2013 Landkreis Rostock I. Des Weiteren trat er auf Platz zwei der Landesliste der SPD Mecklenburg-Vorpommern an und zog \u00fcber diese in den Bundestag ein.  Frank Junge ist ordentliches Mitglied im Ausschuss f\u00fcr Tourismus und im Finanzausschuss, zudem ist er stellvertretendes Mitglied im Unterausschuss \u201eB\u00fcrgerschaftliches Engagement\u201c des Ausschusses f\u00fcr Familie, Senioren, Frauen und Jugend.",
				"en": "Frank Junge is a German politician. He is a member of the SPD."
			},
			"full_name": "Frank Junge",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "240.jpg",
				"pathToThumb": "240_t.jpg"
			},
			"name": "Frank Junge",
			"party": "SPD",
			"pid": "240",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 77928749,
				"twitterUserName": "fjunge"
			}
		},
		"241": {
			"citizen_bird": "heckenbraunelle",
			"cv": {
				"de": "Josip Juratovic (* 15. Januar 1959 in Koprivnica, Jugoslawien, heute Kroatien) ist ein deutscher Politiker (SPD) und seit 2005 Mitglied des Deutschen Bundestags.  Juratovic ist im Bundestag Mitglied des Ausw\u00e4rtigen Ausschusses. F\u00fcr die SPD-Fraktion ist er zust\u00e4ndig f\u00fcr die Regionen S\u00fcdosteuropa, T\u00fcrkei und Staaten der \u00d6stlichen Partnerschaft, f\u00fcr die Zivile Krisenpr\u00e4vention sowie Menschenrechte, Entwicklung, Humanit\u00e4re Hilfe & Migration. Au\u00dferdem ist Juratovic Obmann der SPD-Fraktion im Unterausschuss Vereinte Nationen, Internationale Organisationen und Globalisierung sowie Mitglied der Bundestagsdelegation in folgenden internationalen parlamentarischen Versammlungen: Parlamentarische Versammlung des Europarates, Interparlamentarische Konferenz f\u00fcr die Gemeinsame Au\u00dfen- und Sicherheitspolitik und die Gemeinsame Sicherheits- und Verteidigungspolitik, sowie Parlamentarische Versammlung der OSZE.",
				"en": "Josip Juratovic is a German politician. He is a member of the SPD."
			},
			"full_name": "Josip Juratovic",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "241.jpg",
				"pathToThumb": "241_t.jpg"
			},
			"name": "Josip Juratovic",
			"party": "SPD",
			"pid": "241",
			"self_bird": "elster",
			"twittering": {
				"twitterId": "738731811380076544",
				"twitterUserName": "josip_juratovic"
			}
		},
		"243": {
			"citizen_bird": "dohle",
			"cv": {
				"de": "Egon J\u00fcttner (* 20. Mai 1942 in Gurschdorf, Landkreis Freiwaldau) ist ein deutscher Politiker (CDU).  Er ist Mitglied des Ausw\u00e4rtigen Ausschusses und des Ausschusses f\u00fcr Menschenrechte und Humanit\u00e4re Hilfe und stellvertretender Vorsitzender des Unterausschusses Vereinte Nationen, Internationalisierung und Globalisierung.  J\u00fcttner ist Mitglied der Parlamentarischen Versammlung der OSZE und der Europa-Union Parlamentariergruppe Deutscher Bundestag.",
				"en": "Egon J\u00fcttner is a German politician. He is a member of the CDU."
			},
			"full_name": "Prof. Dr. Egon J\u00fcttner",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "243.jpg",
				"pathToThumb": "243_t.jpg"
			},
			"name": "Egon J\u00fcttner",
			"party": "CDU",
			"pid": "243",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 871704942,
				"twitterUserName": "EgonJuettner"
			}
		},
		"244": {
			"citizen_bird": "schneeeule",
			"cv": {
				"de": "Oliver Kaczmarek (* 8. August 1970 in Kamen) ist ein deutscher Politiker (SPD). Seit 2009 ist er direkt gew\u00e4hlter Bundestagsabgeordneter f\u00fcr den Wahlkreis Unna I. Bereits seit 2005 ist Oliver Kaczmarek Vorsitzender des SPD-Unterbezirks Kreis Unna.  Im Deutschen Bundestag ist Oliver Kaczmarek f\u00fcr die SPD-Bundestagsfraktion stellvertretender bildungspolitischer Sprecher und ordentliches Mitglied im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung. Zus\u00e4tzlich ist er stellvertretendes Mitglied im Ausschuss f\u00fcr Arbeit und Soziales und im Petitionsausschuss sowie Mitglied im SPD-Fraktionsvorstand und im Vorstand der Landesgruppe der SPD-Bundestagsabgeordneten aus Nordrhein-Westfalen. Im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung ist er f\u00fcr die SPD-Fraktion unter anderem zust\u00e4ndig f\u00fcr die Themen: Exzellenzinitiative und Hochschulpakt, das Bundesausbildungsf\u00f6rderungsgesetz und Inklusive Bildung. Oliver Kaczmarek ist au\u00dferdem Vorsitzender der interfraktionellen Deutsch-Belarussischen-Parlamentariergruppe des Deutschen Bundestages.",
				"en": "Oliver Kaczmarek is a German politician. He is a member of the SPD."
			},
			"full_name": "Oliver Kaczmarek",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "244.jpg",
				"pathToThumb": "244_t.jpg"
			},
			"name": "Oliver Kaczmarek",
			"party": "SPD",
			"pid": "244",
			"self_bird": "weisskopfseeadler",
			"twittering": {
				"twitterId": 231440493,
				"twitterUserName": "KaczmarekOliver"
			}
		},
		"245": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Johannes Kahrs (* 15. September 1963 in Bremen) ist ein deutscher Politiker (SPD) und seit 1998 stets direkt gew\u00e4hltes Mitglied des Deutschen Bundestages f\u00fcr den Wahlkreis Hamburg-Mitte. F\u00fcr die SPD ist er Sprecher der Bundestagsfraktion im Haushaltsausschuss, Sprecher des konservativen Seeheimer Kreises und Kreisvorsitzender in Hamburg-Mitte.",
				"en": "Johannes Kahrs (born 15 September 1963 in Bremen) is a German politician. He is a member of the Social Democratic Party of Germany."
			},
			"full_name": "Johannes Kahrs",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "245.jpg",
				"pathToThumb": "245_t.jpg"
			},
			"name": "Johannes Kahrs",
			"party": "SPD",
			"pid": "245",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 35687457,
				"twitterUserName": "kahrs"
			}
		},
		"25": {
			"citizen_bird": "grauschnaepper",
			"cv": {
				"de": "B\u00e4rbel Bas (* 3. Mai 1968 in Walsum) ist eine deutsche Politikerin (SPD) aus Duisburg. Seit der Bundestagswahl 2009 ist sie Mitglied des Deutschen Bundestages und seit Dezember 2013 Parlamentarische Gesch\u00e4ftsf\u00fchrerin der SPD-Bundestagsfraktion.  Bas ist Schirmherrin des Malteser Hospizes St. Raphael in Duisburg-Huckingen und Mitglied des Stiftungsrates der Stiftung Humanit\u00e4re Hilfe f\u00fcr durch Blutprodukte HIV-infizierte Personen. Seit Juli 2015 ist sie Mitglied des Aufsichtsrates der HKM \u2013 H\u00fcttenwerke Krupp Mannesmann GmbH.",
				"en": "B\u00e4rbel Bas is a German politician. She is a member of the SPD."
			},
			"full_name": "B\u00e4rbel Bas",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "25.jpg",
				"pathToThumb": "25_t.jpg"
			},
			"name": "B\u00e4rbel Bas",
			"party": "SPD",
			"pid": "25",
			"self_bird": "mehlschwalbe",
			"twittering": {
				"twitterId": 19438957,
				"twitterUserName": "baerbelbas"
			}
		},
		"250": {
			"citizen_bird": "kohlmeise",
			"cv": {
				"de": "Steffen Kanitz (* 14. Februar 1984 in Dortmund) ist ein deutscher Politiker (CDU).  \u00dcber Platz 20 der CDU-Landesliste in Nordrhein-Westfalen zog Kanitz bei der Bundestagswahl 2013 in den Deutschen Bundestag ein.  Von 2014 bis 2016 war er zudem als Mitglied des Deutschen Bundestages in der Funktion als Ordentliches Kommissionsmitglied in der Kommission Lagerung hoch radioaktiver Abfallstoffe (Endlagerkommission) gem\u00e4\u00df Standortauswahlgesetz.",
				"en": "Steffen Kanitz is a German politician. He is a member of the CDU."
			},
			"full_name": "Steffen Kanitz",
			"images": {
				"copyright": "Ute-Felicitas Ott",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "250.jpg",
				"pathToThumb": "250_t.jpg"
			},
			"name": "Steffen Kanitz",
			"party": "CDU",
			"pid": "250",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 63244000,
				"twitterUserName": "steffenkanitz"
			}
		},
		"252": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Susanna Karawanskij (* 7. Mai 1980 in Leipzig) ist eine deutsche Politikerin (Die Linke).  Bei der Bundestagswahl 2013 kandidierte sie im Wahlkreis Nordsachsen sowie auf Platz 3 der s\u00e4chsischen Landesliste der Linken. Ihr gelang \u00fcber die Landesliste der Einzug in den Bundestag. Sie ist eine von sieben Parlamentarischen Gesch\u00e4ftsf\u00fchrern der Linksfraktion im Deutschen Bundestag. Seit dem 19. September 2016 ist Karawanskij die Ostbeauftragte ihrer Fraktion.",
				"en": "Susanna Karawanskij is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Susanna Karawanskij",
			"images": {
				"license": "custom-linke",
				"pathToImage": "252.jpg",
				"pathToThumb": "252_t.jpg"
			},
			"name": "Susanna Karawanskij",
			"party": "DIE LINKE",
			"pid": "252",
			"self_bird": "gartenrotschwanz",
			"twittering": {
				"twitterId": 892779458,
				"twitterUserName": "SuzaKarawanskij"
			}
		},
		"254": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "Anja Maria-Antonia Karliczek (geborene Kerssen; * 29. April 1971 in Ibbenb\u00fcren) ist eine deutsche Kauffrau und Politikerin (CDU). Bei der Bundestagswahl 2013 errang sie im Wahlkreis Steinfurt III das Direktmandat.  Sie setzt sich f\u00fcr die Vereinbarkeit von Familie und Beruf und die M\u00fctterrente ein.",
				"en": "Anja Karliczek is a German politician. She is a member of the CDU."
			},
			"full_name": "Anja Karliczek",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "254.jpg",
				"pathToThumb": "254_t.jpg"
			},
			"name": "Anja Karliczek",
			"party": "CDU",
			"pid": "254",
			"self_bird": "zilpzalp",
			"twittering": {
				"twitterId": 2521976694,
				"twitterUserName": "AnjaKarliczek"
			}
		},
		"257": {
			"citizen_bird": "gartenbaumlaeufer",
			"cv": {
				"de": "Gabriele Katzmarek (* 8. Juli 1960 in Marl) ist eine deutsche Politikerin der SPD. Sie ist seit 2013 Abgeordnete f\u00fcr den Wahlkreis Rastatt im Deutschen Bundestag.  Gabriele Katzmarek ist ordentliches Mitglied im Ausschuss f\u00fcr Wirtschaft und Energie, sowie stellvertretendes Mitglied in den Aussch\u00fcssen f\u00fcr Gesundheit und f\u00fcr Bildung und Forschung, zudem im Unterausschuss f\u00fcr Regionale Wirtschaftspolitik und ERP-Wirtschaftspl\u00e4ne. Die Wirtschaftspolitikerin ist Ansprechpartnerin f\u00fcr die Bereiche Industrie 4.0 und Gesundheitswirtschaft.  Als Beiratsmitglied von Jugend gegen AIDS unterst\u00fctzt Gabriele Katzmarek eine von Jugendlichen initiierte und gef\u00fchrte Initiative, die Aufkl\u00e4rungs- und Pr\u00e4ventionsarbeit auf Augenh\u00f6he betreibt.",
				"en": "Gabriele Katzmarek is a German politician. She is a member of the SPD."
			},
			"full_name": "Gabriele Katzmarek",
			"images": {
				"copyright": "Olaf Kosinsky",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "257.jpg",
				"pathToThumb": "257_t.jpg"
			},
			"name": "Gabriele Katzmarek",
			"party": "SPD",
			"pid": "257",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 3017602065,
				"twitterUserName": "GabyKatzmarek"
			}
		},
		"259": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Stefan Kaufmann (* 21. August 1969 in Stuttgart) ist ein deutscher Politiker (CDU).  Stefan Kaufmann ist Obmann der CDU/CSU-Fraktion im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung. Au\u00dferdem ist er stellvertretendes Mitglied im Haushaltsausschuss.  Im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung ist sein Arbeitsschwerpunkt die High-Tech-Strategie und die Innovationspolitik des Bundes. Zudem k\u00fcmmert er sich um den Bereich der Studienfinanzierung. Dies umfasst insbesondere das Bundesausbildungsf\u00f6rderungsgesetz (BAf\u00f6G) sowie das vor kurzem beschlossene Nationale Stipendienprogramm. Des Weiteren ist er Berichterstatter f\u00fcr die europ\u00e4ische Bildungs- wie auch die europ\u00e4ische Forschungszusammenarbeit \u2013 und damit f\u00fcr fast alle EU-Vorhaben, mit denen der Bundestag seit dem Vertrag von Lissabon mehr noch als fr\u00fcher befasst ist. Aktuell sind dies beispielsweise die Weiterentwicklung des Europ\u00e4ischen Forschungsraums, die Begleitung des Forschungsprogramms Horizon 2020 oder der Bau eines Europ\u00e4ischen Kernfusionsreaktors in S\u00fcdfrankreich.",
				"en": "Stefan Kaufmann (* 21. August 1969 in Stuttgart) is a German politician and member of the Christian Democratic Union (CDU).  In 2008, the CDU in Stuttgart appointed him candidate for the Bundestag, again in Stuttgart I, after his predecessor on this appointment had died. In October of the same year he was elected Deputy District Chairman of the CDU in Stuttgart.  In parliament, Kaufmann is a member of the Committee of Education, Research and Technological Impact Assessment. He also serves as deputy member of the Budget Committee, after previously having been a deputy member of the Committee on Petitions and the Committee of Traffic, Construction and Urban Development."
			},
			"full_name": "Dr. Stefan Kaufmann",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "259.jpg",
				"pathToThumb": "259_t.jpg"
			},
			"name": "Stefan Kaufmann",
			"party": "CDU",
			"pid": "259",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 51752254,
				"twitterUserName": "StefanKaufmann"
			}
		},
		"260": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Uwe Adolf Otto Kekeritz (* 9. Oktober 1953 in Haslach/Oy-Mittelberg) ist ein deutscher Politiker (Gr\u00fcne) und seit 2009 Mitglied des Deutschen Bundestages.  2013 gelang ihm auf Listenplatz 6 der Wiedereinzug in den Bundestag. In der 18. Legislaturperiode ist Kekeritz Mitglied und Obmann im Ausschuss f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung, sowie ist Sprecher f\u00fcr Entwicklungspolitik der Gr\u00fcnen Bundestagsfraktion. Au\u00dferdem ist er stellvertretendes Mitglied im Unterausschuss f\u00fcr \u201eZivile Krisenpr\u00e4vention, Konfliktbearbeitung und vernetztes Handeln\u201c.",
				"en": "Uwe Kekeritz is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Uwe Kekeritz",
			"images": {
				"copyright": "B\u00fcro-Kekeritz",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "260.jpg",
				"pathToThumb": "260_t.jpg"
			},
			"name": "Uwe Kekeritz",
			"party": "GR\u00dcNE",
			"pid": "260",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 46480553,
				"twitterUserName": "UweKekeritz"
			}
		},
		"261": {
			"citizen_bird": "rotkehlchen",
			"cv": {
				"de": "Ulrich Wolfgang Kelber (* 29. M\u00e4rz 1968 in Bamberg) ist ein deutscher Politiker (SPD). Er ist seit Dezember 2013 Parlamentarischer Staatssekret\u00e4r, zuvor war er seit 2005 stellvertretender Vorsitzender der SPD-Bundestagsfraktion.  Kelber griff 2000 die Initiative des fr\u00fcheren Bundestagsabgeordneten Norbert Gansel aus den 1970er Jahren als \u201eGl\u00e4serner Bundestagsabgeordneter\u201c auf und erweiterte diese. Als erster Bundestagsabgeordneter ver\u00f6ffentlichte er seine Steuerbescheide, berichtete \u00fcber alle Dienstreisen und Abstimmungen und listet seit 2009 auch alle Gespr\u00e4che mit Lobbyisten auf.",
				"en": "Ulrich Kelber is a German politician. He is a member of the SPD."
			},
			"full_name": "Ulrich Kelber",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "261.jpg",
				"pathToThumb": "261_t.jpg"
			},
			"name": "Ulrich Kelber",
			"party": "SPD",
			"pid": "261",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 20877217,
				"twitterUserName": "UlrichKelber"
			}
		},
		"264": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Roderich Kiesewetter (* 11. September 1963 in Pfullendorf) ist ein deutscher Politiker (CDU) und ehemaliger Generalstabsoffizier (Oberst a.D.) der Bundeswehr. Bei der Bundestagswahl 2009 wurde er im Wahlkreis Aalen \u2013 Heidenheim als Direktkandidat in den Deutschen Bundestag gew\u00e4hlt. Seit 2011 ist er Pr\u00e4sident des Reservistenverbandes.  Seit 2014 ist er Vorsitzender des Bundesfachausschusses f\u00fcr Au\u00dfen-, Sicherheits-, Entwicklungs-, und Menschenrechtspolitik.  Kiesewetter ist unter anderem Mitglied in der Deutschen Gesellschaft f\u00fcr Ausw\u00e4rtige Politik, in der Europa-Union Deutschland, in der S\u00fcdosteuropa-Gesellschaft (ehemaliger Vizepr\u00e4sident), im Lions-Club sowie im Verband der Wissenschaftlichen Katholischen Studentenvereine Unitas. Im Deutschen Bundestag ist er Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag.",
				"en": "Roderich Kiesewetter (born 11 September 1963 in Pfullendorf, Germany) is a politician (CDU), member of the German Bundestag and former Bundeswehr general staff officer.  From April 2014, Kiesewetter served as member of the German Parliamentary Committee investigating the NSA spying scandal; he resigned from the body in January 2015, citing an increased need to focus on his duties in the Committee on Foreign Affairs.  In addition to his committee assignments, Kiesewetter is a member of the Parliamentary Friendship Group for Relations with Bosnia and Herzegovina.  In February 2013, Kiesewetter accompanied President of the Bundestag Norbert Lammert to Morocco for meetings with the President of the House of Representatives, Karim Ghellab, the President of the Assembly of Councillors, Mohamed Cheikh Biadillah, Prime Minister Abdelilah Benkirane, and Foreign Minister Saad-Eddine El Othmani. In early 2015, he joined Germany\u2019s Foreign Minister Frank-Walter Steinmeier on official trips to Morocco, Tunisia and Algeria; later that year, he also accompanied him on a trip to Cuba; it was the first time a German foreign minister had visited the country since German reunification in 1990."
			},
			"full_name": "Roderich Kiesewetter",
			"images": {
				"copyright": "Ingo Sombrutzki",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "264.jpg",
				"pathToThumb": "264_t.jpg"
			},
			"name": "Roderich Kiesewetter",
			"party": "CDU",
			"pid": "264",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 42698498,
				"twitterUserName": "RKiesewetter"
			}
		},
		"265": {
			"citizen_bird": "schneeeule",
			"cv": {
				"de": "Sven-Christian Kindler (* 14. Februar 1985 in Hannover) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). Er ist seit 2009 Mitglied des Deutschen Bundestages und war von 2007 bis 2009 Sprecher der Gr\u00fcnen Jugend Niedersachsen.  Im 17. Bundestag und im 18. Bundestag war er ordentliches Mitglied im Haushaltsausschuss sowie stellvertretendes Mitglied im Finanzausschuss. Seit 2013 ist er Sprecher der nieders\u00e4chsischen Landesgruppe seiner Bundestagsfraktion und seit 2014 deren haushaltspolitischer Sprecher.",
				"en": "Sven-Christian Kindler is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Sven-Christian Kindler",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "265.jpg",
				"pathToThumb": "265_t.jpg"
			},
			"name": "Sven-Christian Kindler",
			"party": "GR\u00dcNE",
			"pid": "265",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 17343047,
				"twitterUserName": "sven_kindler"
			}
		},
		"267": {
			"citizen_bird": "tannenmeise",
			"cv": {
				"de": "Katja Kipping (* 18. Januar 1978 in Dresden) ist eine deutsche Politikerin (Die Linke). Seit 2005 ist sie Mitglied des Deutschen Bundestages, seit 2012 gemeinsam mit Bernd Riexinger Parteivorsitzende der Partei Die Linke.  Im Bundestag ist die erkl\u00e4rte Hartz-IV-Gegnerin die sozialpolitische Sprecherin der Linksfraktion. Sie setzt sich seit Jahren f\u00fcr ein bedingungsloses Grundeinkommen ein. Ihr Wahlkreis ist Dresden I.  Vom 25. November 2009 bis 26. September 2012 war sie Vorsitzende des Ausschusses f\u00fcr Arbeit und Soziales des Deutschen Bundestags.  Im Januar 2012 wurde bekannt, dass Kipping als eine von 27 Bundestagsabgeordneten der Linken unter Beobachtung durch das Bundesamt f\u00fcr Verfassungsschutz stand, was von Politikern aller Fraktionen kritisiert wurde. Siehe auch: Beobachtung der Partei Die Linke durch den Verfassungsschutz.",
				"en": "Katja Kipping (born 18 January 1978) is a German politician who is chairperson of the Left Party and a member of the Bundestag (German Parliament since 2005).  In the Parliament, the self-declared Hartz-IV opponent is the social affairs spokesperson for the Left. For years, she has championed the idea of a national basic income guarantee. Her home constituency is in Dresden.  From 25 November 2009 until 26 September 2012 she was the Chair of the Committee on Labor and Social Affairs in the German Parliament."
			},
			"full_name": "Katja Kipping",
			"images": {
				"copyright": "Bl\u00f6mke/Kosinsky/Tsch\u00f6pe",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "267.jpg",
				"pathToThumb": "267_t.jpg"
			},
			"name": "Katja Kipping",
			"party": "DIE LINKE",
			"pid": "267",
			"self_bird": "ringeltaube",
			"twittering": {
				"twitterId": 28066013,
				"twitterUserName": "katjakipping"
			}
		},
		"268": {
			"citizen_bird": "ringeltaube",
			"cv": {
				"de": "Cansel Kiziltepe (* 8. Oktober 1975 in Berlin-Kreuzberg) ist eine t\u00fcrkischst\u00e4mmige deutsche Politikerin (SPD) und seit 2013 Mitglied des Deutschen Bundestages (MdB).  Im Deutschen Bundestag ist Cansel Kiziltepe Mitglied im Finanzausschuss und seit September 2015 stellvertretende finanzpolitische Sprecherin der SPD-Bundestagsfraktion.  Cansel Kiziltepe ist seit August 2015 Gesch\u00e4ftsf\u00fchrerin des Projekts \u201eSteuermythen\u201c. Dieses Projekt wird von SPD-Bundestagsabgeordneten getragen und hat den Anspruch, auf wissenschaftlicher Basis die steuerpolitische Debatte in Deutschland zu \u00f6ffnen und zu versachlichen.  Cansel Kiziltepe ist Mitglied in der IG Metall, der Arbeiterwohlfahrt, im Berliner Mieterverein, im Verein M\u00f6ckernkiez und Kuratoriumsmitglied der Berliner Krebsgesellschaft.",
				"en": "Cansel Kiziltepe is a German politician. She is a member of the SPD."
			},
			"full_name": "Cansel Kiziltepe",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "268.jpg",
				"pathToThumb": "268_t.jpg"
			},
			"name": "Cansel Kiziltepe",
			"party": "SPD",
			"pid": "268",
			"self_bird": "eichelhaeher",
			"twittering": {
				"twitterId": 23248102,
				"twitterUserName": "CanselK"
			}
		},
		"269": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Arno Klare (* 1. Februar 1952 in Oberhausen) ist ein deutscher Politiker der SPD und seit 2013 Mitglied des Deutschen Bundestags.",
				"en": "Arno Klare is a German politician. He is a member of the SPD."
			},
			"full_name": "Arno Klare",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "269.jpg",
				"pathToThumb": "269_t.jpg"
			},
			"name": "Arno Klare",
			"party": "SPD",
			"pid": "269",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 1028440310,
				"twitterUserName": "arnoklare"
			}
		},
		"270": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Volkmar Klein (* 13. Januar 1960 in Siegen) ist ein deutscher Politiker (CDU). Er war von 1995 bis 2009 Mitglied des Landtags von Nordrhein-Westfalen und ist seit dem 27. September 2009 direkt gew\u00e4hltes Mitglied des Deutschen Bundestages f\u00fcr den Wahlkreis Siegen-Wittgenstein.  Dort ist er ordentliches Mitglied des Haushaltsausschusses. Au\u00dferdem ist er ordentliches Mitglied im Unterausschuss zu Fragen der Europ\u00e4ischen Union sowie im Parlamentarischen Finanzmarktgremium. Seit 2013 ist er stellvertretendes Mitglied im Ausw\u00e4rtigen Ausschuss.  Klein ist Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag und seit 2013 Vorsitzender der Deutsch-Australisch-Neuseel\u00e4ndischen Parlamentariergruppe. In seiner Heimatgemeinde Burbach ist Volkmar Klein Vorsitzender des Heimatvereins Alte Vogtei Burbach e.V.",
				"en": "Volkmar Klein is a German politician. He is a member of the CDU."
			},
			"full_name": "Volkmar Klein",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "270.jpg",
				"pathToThumb": "270_t.jpg"
			},
			"name": "Volkmar Klein",
			"party": "CDU",
			"pid": "270",
			"self_bird": "gruenfink",
			"twittering": {
				"twitterId": 33043294,
				"twitterUserName": "VolkmarKlein"
			}
		},
		"271": {
			"citizen_bird": "weisskopfseeadler",
			"cv": {
				"de": "Maria Anna Klein-Schmeink (* 6. Januar 1958 in Dingden) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen) und seit 2009 Abgeordnete im Deutschen Bundestag.  Dort ist sie seitdem gesundheitspolitische Sprecherin ihrer Fraktion, ordentliches Mitglied im Ausschuss f\u00fcr Gesundheit sowie stellvertretendes Mitglied im Sportausschuss und im Ausschuss f\u00fcr Arbeit und Soziales.",
				"en": "Maria Klein-Schmeink is a German politician. She is a member of the GR\u00dcNE.  Maria Klein-Schmeink ist Mitglied des Vorstandes der Aktion Psychisch Kranke e.V. in Bonn, stellvertretende Vorsitzende im Kuratorium Sport und Natur e.V. in M\u00fcnchen und Mitglied des Beirates der Stiftung Hospizarbeit M\u00fcnster."
			},
			"full_name": "Maria Klein-Schmeink",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "271.jpg",
				"pathToThumb": "271_t.jpg"
			},
			"name": "Maria Klein-Schmeink",
			"party": "GR\u00dcNE",
			"pid": "271",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 18103366,
				"twitterUserName": "MariaKlSchmeink"
			}
		},
		"272": {
			"citizen_bird": "star",
			"cv": {
				"de": "J\u00fcrgen Klimke (* 2. Juli 1948 in Hamburg) ist ein deutscher Politiker (CDU). Von 1982 bis 2002 geh\u00f6rte er der Hamburgischen B\u00fcrgerschaft an, seit 2002 ist er Mitglied des Deutschen Bundestages.  Seit 2009 ist er Obmann im Ausschuss f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung. Sein regionaler Schwerpunkt liegt in S\u00fcdasien und S\u00fcdostasien. In der aktuellen Legislaturperiode geh\u00f6rt Klimke zudem dem Ausw\u00e4rtigen Ausschuss als ordentliches Mitglied an und ist Obmann im Unterausschuss Vereinte Nationen, Internationale Organisationen und Globalisierung. Weiterhin ist Klimke Vorsitzender der Arbeitsgemeinschaft Elbe der CDU/CSU-Fraktion und stellvertretender Vorsitzender der deutschen Delegation in der Parlamentarischen Versammlung der Organisation f\u00fcr Sicherheit und Zusammenarbeit in Europa (OSZE). Im August 2015 wurde Klimke zum ersten Sonderbeauftragten der Parlamentarischen Versammlung der OSZE f\u00fcr die Ostsee-Region ernannt.",
				"en": "J\u00fcrgen Klimke (born 2 July 1948, in Hamburg) is a German politician and member of the conservative party CDU - Christian Democratic Union of Germany (CDU). From 1982 to 2002 he was member of the Parliament of the Free and Federal State of Hamburg. Since 2002, he has been representing Hamburg in the German Federal Parliament (the Bundestag).  Klimke\u2019s regional focus lies in South and Southeast Asia. During the current legislative period, Klimke is member of the Committee on Foreign Affairs and spokesperson of the CDU/CSU group in the Subcommittee for the United Nations, International Organizations and Globalization. Furthermore, Klimke is chairman of the working group on the River Elbe of the CDU/CSU group and deputy chair of the German delegation to the Parliamentary Assembly of the OSCE.  In August 2015, Klimke was appointed as the first Special Representative of the Parliamentary Assembly of the OSCE for the Baltic Sea Area."
			},
			"full_name": "J\u00fcrgen Klimke",
			"images": {
				"copyright": "Deutscher Bundestag / H.-J. M\u00fcller",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "272.jpg",
				"pathToThumb": "272_t.jpg"
			},
			"name": "J\u00fcrgen Klimke",
			"party": "CDU",
			"pid": "272",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 47698123,
				"twitterUserName": "Klimke_CDU"
			}
		},
		"273": {
			"citizen_bird": "gartenbaumlaeufer",
			"cv": {
				"de": "Lars Klingbeil (* 23. Februar 1978 in Soltau) ist ein deutscher Politiker (SPD) aus Munster in Niedersachsen. Klingbeil ist seit 2001 Mitglied des Stadtrates von Munster und seit 2006 Mitglied im Kreistag des Heidekreises. Er war 2005 f\u00fcr knapp neun Monate und ist seit der Bundestagswahl 2009 erneut Abgeordneter im Deutschen Bundestag. Au\u00dferdem war er von 2003 bis 2007 stellvertretender Bundesvorsitzender der Jusos.  Lars Klingbeil geh\u00f6rte dem Verteidigungsausschuss an und arbeitete zus\u00e4tzlich als stellvertretendes Mitglied im Ausschuss f\u00fcr Kultur und Medien und dem Unterausschuss Neue Medien mit. Daneben wurde Klingbeil zum Berichterstatter f\u00fcr neue Medien der SPD-Bundestagsfraktion und zum stellvertretenden Vorsitzenden der SPD-Landesgruppe gew\u00e4hlt. Bei der Bundestagswahl 2013 zog er erneut \u00fcber die Landesliste in den Bundestag ein. Er ist erneut Ordentliches Mitglied im Verteidigungsausschuss.",
				"en": "Lars Klingbeil is a German politician. He is a member of the SPD."
			},
			"full_name": "Lars Klingbeil",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "273.jpg",
				"pathToThumb": "273_t.jpg"
			},
			"name": "Lars Klingbeil",
			"party": "SPD",
			"pid": "273",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 16255941,
				"twitterUserName": "larsklingbeil"
			}
		},
		"275": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Tom Koenigs (Thomas Carl Joerge Koenigs; * 25. Januar 1944 in Damm, Pommern) ist ein deutscher Politiker und Bundestagsabgeordneter der Fraktion B\u00fcndnis 90/Die Gr\u00fcnen. Er ist Obmann seiner Fraktion im Ausschusses f\u00fcr Menschenrechte und humanit\u00e4re Hilfe und ist Ordentliches Mitglied im Unterausschuss Vereinte Nationen, Internationale Organisationen und Globalisierung. Koenigs ist seit November 2013 menschenrechtspolitischer Sprecher der Fraktion B\u00fcndnis 90/Die Gr\u00fcnen.  Im 18. Bundestag ist Koenigs Obmann seiner Fraktion im Ausschuss f\u00fcr Menschenrechte und humanit\u00e4re Hilfe und Ordentliches Mitglied des Unterausschusses Vereinte Nationen, internationale Organisationen und Globalisierung.  Koenigs war von 2009 bis 2013 Vorsitzender des Ausschusses f\u00fcr Menschenrechte und humanit\u00e4re Hilfe und Ordentliches Mitglied im Verteidigungsausschuss.",
				"en": "Tom Koenigs is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Tom Koenigs",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "275.jpg",
				"pathToThumb": "275_t.jpg"
			},
			"name": "Tom Koenigs",
			"party": "GR\u00dcNE",
			"pid": "275",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 399988476,
				"twitterUserName": "KoenigsGruen"
			}
		},
		"276": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Jens Koeppen (* 27. September 1962 in Zeitz) ist ein deutscher Politiker (CDU). Er ist seit 2005 Abgeordneter im Deutschen Bundestag und seit 2014 Vorsitzender des in der 18. Wahlperiode des Deutschen Bundestages neu konstituierten Ausschusses Digitale Agenda.  Koeppen ist ordentliches Mitglied im Ausschuss Digitale Agenda, dessen Vorsitz er innehat. Daneben ist er ordentliches Mitglied im Ausschuss f\u00fcr Wirtschaft und Energie sowie im \u00c4ltestenrat und stellvertretendes Mitglied im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur. Ferner ist er Vorstandsmitglied des Parlamentskreises Mittelstand (PKM).  Als Schriftf\u00fchrer-Obmann forderte Koeppen im Januar 2010 eine angemessene Kleidung der eingeteilten Schriftf\u00fchrer im deutschen Bundestag, insbesondere w\u00e4hrend des Sitzungsdienstes im Plenarsaal.",
				"en": "Jens Koeppen (born 27 September 1962 in Zeitz, Germany) is a German politician (CDU). Since 2005, he has been a member of the Bundestag.  Since 18 September 2005, Koeppen has been a member of the German Bundestag, representing Uckermark \u2013 Barnim I. Between 2005 and 2013, he served as a member of the Committee on the Environment, Nature Conservation and Nuclear Safety.  Following the 2013 federal elections, Koeppen was elected Chairman of the Committee on the Digital Agenda. He also serves as a member of the Committee for Economic Affairs and Energy as well as of the parliament\u2019s Council of Elders, which \u2013 among other duties \u2013 determines daily legislative agenda items and assigns committee chairpersons based on party representation."
			},
			"full_name": "Jens Koeppen",
			"images": {
				"copyright": "Frank N\u00fcrnberger",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "276.jpg",
				"pathToThumb": "276_t.jpg"
			},
			"name": "Jens Koeppen",
			"party": "CDU",
			"pid": "276",
			"self_bird": "schwanzmeise",
			"twittering": {
				"twitterId": 146376562,
				"twitterUserName": "JensKoeppen"
			}
		},
		"277": {
			"citizen_bird": "gartenrotschwanz",
			"cv": {
				"de": "B\u00e4rbel Kofler (* 24. Mai 1967 in Freilassing) ist eine deutsche Politikerin (SPD) und seit 1. M\u00e4rz 2016 Beauftragte der Bundesregierung f\u00fcr Menschenrechtspolitik und Humanit\u00e4re Hilfe.  Am 21. September 2004 r\u00fcckte sie f\u00fcr den verstorbenen Abgeordneten Hans B\u00fcttner in den Deutschen Bundestag nach und ist seitdem Mitglied des Deutschen Bundestages. Dort geh\u00f6rt sie seit Oktober 2007 dem Vorstand der SPD-Bundestagsfraktion an. Bereits seit 2005 ist sie Vorsitzende der Deutsch-Ukrainischen Parlamentariergruppe.",
				"en": "B\u00e4rbel Kofler is a German politician. She is a member of the SPD."
			},
			"full_name": "Dr. B\u00e4rbel Kofler",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "277.jpg",
				"pathToThumb": "277_t.jpg"
			},
			"name": "B\u00e4rbel Kofler",
			"party": "SPD",
			"pid": "277",
			"self_bird": "kiwi",
			"twittering": {
				"twitterId": 193629792,
				"twitterUserName": "BaerbelKofler"
			}
		},
		"278": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Daniela Kolbe (* 22. Februar 1980 in Schleiz, Bezirk Gera, DDR) ist eine deutsche Politikerin (SPD) und seit dem 7. November 2015 die Generalsekret\u00e4rin der SPD Sachsen.  Seit 2009 ist Kolbe Mitglied des Deutschen Bundestages.  Seit 28. Januar 2014 ist sie stellvertretende Sprecherin f\u00fcr Arbeit und Soziales der SPD-Bundestagsfraktion. In der 18. Legislaturperiode ist Kolbe f\u00fcr die SPD-Fraktion ordentliches Mitglied im Ausschuss f\u00fcr Arbeit und Soziales sowie stellvertretendes Mitglied im Innenausschuss und im Petitionsausschuss.  Kolbe ist zudem stellvertretende Vorsitzende des Kuratoriums der Bundeszentrale f\u00fcr politische Bildung sowie Mitglied der Grundwertekommission der SPD.",
				"en": "Daniela Kolbe is a German politician. She is a member of the SPD."
			},
			"full_name": "Daniela Kolbe",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "278.jpg",
				"pathToThumb": "278_t.jpg"
			},
			"name": "Daniela Kolbe",
			"party": "SPD",
			"pid": "278",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": 516783036,
				"twitterUserName": "danielakolbe"
			}
		},
		"28": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Marieluise Beck (* 25. Juni 1952 in Bramsche) ist eine deutsche Politikerin von B\u00fcndnis 90/Die Gr\u00fcnen. Sie ist Mitglied des Bundestags und Sprecherin f\u00fcr Osteuropapolitik ihrer Partei. Sie war von 2002 bis 2005 parlamentarische Staatssekret\u00e4rin bei der Bundesministerin f\u00fcr Familie, Senioren, Frauen und Jugend und von 1998 bis 2005 Ausl\u00e4nderbeauftragte der Bundesregierung.  Beck ist Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag und seit 5. Mai 2015 im Lenkungsausschuss des deutsch-russischen Petersburger Dialogs, und Mitglied der Deutsch-Israelischen Gesellschaft, sowie Vorsitzende des Bremer Hilfsvereins Br\u00fccke der Hoffnung",
				"en": "Marieluise Beck (born 25 June 1952 in Bramsche, Lower Saxony) is a German politician and member of the Alliance '90/The Greens group in the Bundestag. She is also a member of the Parliamentary Assembly of the Council of Europe.  Since 2005, Beck has been member of the Committee on Foreign Affairs at the German parliament, where she focuses as the spokesperson on Eastern European affairs for the Green Party's parliamentary group on matters concerning Russia, Belarus and Western Balkan countries. She serves as the chairperson of the German-Bosnian Parliamentary Friendship Group and as a member of the German-Ukrainian and German-South Caucasus Parliamentary Friendship Groups."
			},
			"full_name": "Marieluise Beck",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "28.jpg",
				"pathToThumb": "28_t.jpg"
			},
			"name": "Marieluise Beck",
			"party": "GR\u00dcNE",
			"pid": "28",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 763549040,
				"twitterUserName": "MarieluiseBeck"
			}
		},
		"282": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Jan Korte (* 5. April 1977 in Osnabr\u00fcck) ist ein deutscher Politiker und Mitglied des Bundestags f\u00fcr Die Linke.  Seit der Fusion mit der WASG im Juni 2007 geh\u00f6rt er dem Bundesvorstand der Partei Die Linke an und wurde auf dem Cottbuser Parteitag am 24. und 25. Mai 2008 in den Bundesvorstand gew\u00e4hlt. 2013 wurde er zum Arbeitskreisleiter und stellvertretenden Fraktionsvorsitzenden gew\u00e4hlt, seit November 2015 ist er pers\u00f6nlicher Stellvertreter der Fraktionsvorsitzenden Dietmar Bartsch und Sahra Wagenknecht.",
				"en": "Jan Korte is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Jan Korte",
			"images": {
				"license": "custom-linke",
				"pathToImage": "282.jpg",
				"pathToThumb": "282_t.jpg"
			},
			"name": "Jan Korte",
			"party": "DIE LINKE",
			"pid": "282",
			"self_bird": "singdrossel",
			"twittering": {
				"twitterId": 4164673821,
				"twitterUserName": "jankortemdb"
			}
		},
		"283": {
			"citizen_bird": "heckenbraunelle",
			"cv": {
				"de": "Hartmut Koschyk (* 16. April 1959 in Forchheim) ist ein deutscher Politiker (CSU). Von Oktober 2009 bis Dezember 2013 war er Parlamentarischer Staatssekret\u00e4r im Bundesfinanzministerium. Zuvor war er von 2005 bis 2009 Parlamentarischer Gesch\u00e4ftsf\u00fchrer der CSU-Landesgruppe im Deutschen Bundestag. Seit dem 8. Januar 2014 ist Koschyk Beauftragter der Bundesregierung f\u00fcr Aussiedlerfragen und nationale Minderheiten.",
				"en": "Hartmut Koschyk is a German politician. He is a member of the CSU."
			},
			"full_name": "Hartmut Koschyk",
			"images": {
				"copyright": "Henning Schacht",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "283.jpg",
				"pathToThumb": "283_t.jpg"
			},
			"name": "Hartmut Koschyk",
			"party": "CSU",
			"pid": "283",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 64377390,
				"twitterUserName": "koschyk"
			}
		},
		"284": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Sylvia Kotting-Uhl (* 29. Dezember 1952 in Karlsruhe) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen), seit 2005 Mitglied des Deutschen Bundestages und seit 2009 atompolitische Sprecherin ihrer Fraktion.  Von 2014 bis 2016 war sie zudem als Mitglied des Deutschen Bundestages in der Funktion als Ordentliches Kommissionsmitglied in der Kommission Lagerung hoch radioaktiver Abfallstoffe (Endlagerkommission) gem\u00e4\u00df Standortauswahlgesetz.  Sylvia Kotting-Uhl ist Mitglied bei BUND, WWF, Europa-Union, Amnesty International und diversen kulturellen und sozialen Einrichtungen Baden-W\u00fcrttembergs.",
				"en": "Sylvia Kotting-Uhl is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Sylvia Kotting-Uhl",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "284.jpg",
				"pathToThumb": "284_t.jpg"
			},
			"name": "Sylvia Kotting-Uhl",
			"party": "GR\u00dcNE",
			"pid": "284",
			"self_bird": "rotkehlchen",
			"twittering": {
				"twitterId": 474013625,
				"twitterUserName": "BabettesChefin"
			}
		},
		"286": {
			"citizen_bird": "moenchsgrasmuecke",
			"cv": {
				"de": "Anette Kramme (* 10. Oktober 1967 in Essen) ist eine deutsche Politikerin (SPD) und seit Dezember 2013 Parlamentarische Staatssekret\u00e4rin bei der Bundesministerin f\u00fcr Arbeit und Soziales.  ",
				"en": "Anette Kramme is a German politician. She is a member of the SPD."
			},
			"full_name": "Anette Kramme",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "286.jpg",
				"pathToThumb": "286_t.jpg"
			},
			"name": "Anette Kramme",
			"party": "SPD",
			"pid": "286",
			"self_bird": "goldammer",
			"twittering": {
				"twitterId": "1461705757",
				"twitterUserName": "AnetteKramme"
			}
		},
		"287": {
			"citizen_bird": "grauschnaepper",
			"cv": {
				"de": "Jutta Krellmann (* 7. Januar 1956 in Johannisberg (Rheingau)) ist eine deutsche Politikerin (Die Linke) und Gewerkschafterin. Seit Oktober 2009 ist sie Mitglied des Deutschen Bundestages.",
				"en": "Jutta Krellmann is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Jutta Krellmann",
			"images": {
				"license": "custom-linke",
				"pathToImage": "287.jpg",
				"pathToThumb": "287_t.jpg"
			},
			"name": "Jutta Krellmann",
			"party": "DIE LINKE",
			"pid": "287",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 600984834,
				"twitterUserName": "JuttaKrellmann"
			}
		},
		"288": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Michael Kretschmer (* 7. Mai 1975 in G\u00f6rlitz) ist ein deutscher Politiker (CDU). Er ist seit 2005 Generalsekret\u00e4r des CDU-Landesverbandes Sachsen und seit 2009 Stellvertretender Fraktionsvorsitzender der CDU/CSU-Bundestagsfraktion, seit September 2013 zudem Vorsitzender der s\u00e4chsischen Landesgruppe.  Seit 2009 ist er Stellvertretender Vorsitzender der CDU/CSU-Bundestagsfraktion mit den Aufgabenbereichen Bildung und Forschung sowie Kunst, Kultur und Medien.  Kretschmer ist Mitglied in der Deutsch-Polnischen, Deutsch-Tschechischen, und Deutsch-Russischen Parlamentariergruppe des Bundestages, die bilaterale Kontakte zu anderen nationalen Parlamenten pflegen.",
				"en": "Michael Kretschmer is a German politician. He is a member of the CDU."
			},
			"full_name": "Michael Kretschmer",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "288.jpg",
				"pathToThumb": "288_t.jpg"
			},
			"name": "Michael Kretschmer",
			"party": "CDU",
			"pid": "288",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 393264968,
				"twitterUserName": "kretsc"
			}
		},
		"29": {
			"citizen_bird": "eichelhaeher",
			"cv": {
				"de": "Volker Beck (* 12. Dezember 1960 in Stuttgart-Bad Cannstatt) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und Menschenrechtler. Seit 1994 ist er Mitglied des Deutschen Bundestags. Er ist innenpolitischer Sprecher der Bundestagsfraktion, Vorsitzender der deutsch-israelischen Parlamentariergruppe des Deutschen Bundestages und Sprecher f\u00fcr Religionspolitik seiner Fraktion.  Beck war von 1994 bis 2002 rechtspolitischer, von 2005 bis 2013 menschenrechtspolitischer sowie von 2013 bis 2016 innenpolitischer Sprecher der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen. Seit 2013 ist er deren religionspolitischer und seit 2016 deren migrationspolitischer Sprecher. Von 2002 bis 2013 war er Erster Parlamentarischer Gesch\u00e4ftsf\u00fchrer seiner Bundestagsfraktion und seit 2014 ist er Vorsitzender der deutsch-israelischen Parlamentariergruppe des Deutschen Bundestages.",
				"en": "Volker Beck (born 12 December 1960 in Stuttgart) is a German politician. He is a sitting member of parliament for the Green Party in the Bundestag since 1994. Beck served as the Green Party Speaker for Legal Affairs from 1994\u20132002, and as the Green Party Chief Whip in the Bundestag till 2013. He was spokesman of the Green Parliamentary Group for interior affairs and religion. In 2014 he was elected as President of the German-Israeli Parliamentary Friendship Group of the German Bundestag."
			},
			"full_name": "Volker Beck",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "29.jpg",
				"pathToThumb": "29_t.jpg"
			},
			"name": "Volker Beck",
			"party": "GR\u00dcNE",
			"pid": "29",
			"self_bird": "ringeltaube",
			"twittering": {
				"twitterId": 16337664,
				"twitterUserName": "Volker_Beck"
			}
		},
		"290": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "G\u00fcnter Krings (* 7. August 1969 in Rheydt) ist ein deutscher Politiker (CDU).  Krings ist seit Dezember 2013 Parlamentarischer Staatssekret\u00e4r beim Bundesminister des Innern. Von 2009 bis Ende 2013 war er stellvertretender Vorsitzender der CDU/CSU-Bundestagsfraktion f\u00fcr die Bereiche Recht, Innen, Sport und Ehrenamt, Vertriebene, Fl\u00fcchtlinge und Aussiedler.  Im Bundestag, der die H\u00e4lfte der Richter des Bundesverfassungsgerichts bestimmt, ist Krings eines von 12 Mitgliedern des Wahlausschusses.",
				"en": "G\u00fcnter Krings is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. G\u00fcnter Krings",
			"images": {
				"copyright": "Martin Kraft",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "290.jpg",
				"pathToThumb": "290_t.jpg"
			},
			"name": "G\u00fcnter Krings",
			"party": "CDU",
			"pid": "290",
			"self_bird": "ringeltaube",
			"twittering": {
				"twitterId": 392312829,
				"twitterUserName": "guenterkrings"
			}
		},
		"291": {
			"citizen_bird": "grauschnaepper",
			"cv": {
				"de": "Oliver Krischer (* 26. Juli 1969 in Z\u00fclpich) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und Abgeordneter des Deutschen Bundestages.  Au\u00dferdem ist er stellvertretendes Mitglied im Ausschuss Wirtschaft und Energie sowie stellvertretendes Mitglied im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur.  Schwerpunkte seiner Arbeit innerhalb der Klima- und Energiepolitik sind Fragen in der Effizienz der Energieerzeugung, Kraft-W\u00e4rme-Kopplung, Strommarktdesign, eine Reformierung des Bergrechts sowie die CCS-Technologie (CO\u00b2-Abscheidung und Speicherung) und das Fracking.  Krischer sitzt f\u00fcr die Gr\u00fcnen im Untersuchungsausschuss zum VW-Abgasskandal. Er wurde am 7. Juli 2016 in dessen konstituierender Sitzung zum stellvertretenden Ausschussvorsitzenden gew\u00e4hlt.",
				"en": "Oliver Krischer (born 26 July 1969 in Z\u00fclpich, Germany) is a German Green politician and member of the German Bundestag.  In the 2009 federal elections, Krischer was elected into the German Bundestag via his nomination on the Green ballot list.  In his first term between 2009 and 2013, Krischer was a member of the Committee on Economic Affairs and Technology and an alternate member of the Committee on the Environment, Conservation and Reactor Safety. He also served as his parliamentary group's spokesperson on energy and resource efficiency. In addition, he was the head of the Working Group for Energy and Climate of the Green Party\u2019s parliamentary group.  In the current parliamentary term starting with the 2013 elections Mr. Krischer is the vice chairman of the Green party in the German parliament and therefor responsible for the topics, energy, environment, climate, reactor safety, animal protection, agriculture and transportation. He is a deputy member of the Committee on Economic Affairs and Energy and the Committee on Transport and Digital Infrastructure. Besides this he is the vice-chairman of the Committee of enquiry on the emissions scandal (Dieselgate) and one of the main scouts in the scandal."
			},
			"full_name": "Oliver Krischer",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "291.jpg",
				"pathToThumb": "291_t.jpg"
			},
			"name": "Oliver Krischer",
			"party": "GR\u00dcNE",
			"pid": "291",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 88630242,
				"twitterUserName": "Oliver_Krischer"
			}
		},
		"293": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "R\u00fcdiger Kruse (* 10. Juni 1961 in Hamburg) ist ein Hamburger Politiker der CDU und seit 2009 Mitglied des Deutschen Bundestages.  Kruse ist ordentliches Mitglied in folgenden Bundestagsaussch\u00fcssen: Haushaltsausschuss, Rechnungspr\u00fcfungsausschuss, parlamentarischer Beirat f\u00fcr nachhaltige Entwicklung. Er ist stellvertretendes Mitglied im Ausschuss f\u00fcr Wirtschaft und Energie.  Seit September 2014 ist Kruse Vorsitzender der Landesgruppe der Hamburger CDU-Abgeordneten.  Kruse ist Mitglied im Kunstbeirat des Deutschen Bundestages, der Europa-Union Parlamentariergruppe Deutscher Bundestag, der Deutsch-Britischen und der Deutsch-Israelischen Parlamentariergruppe. Au\u00dferdem ist er Mitglied in der Kommission \u201eNachhaltig Leben\u201c der CDU Deutschlands, Mitglied im Wirtschaftsrat der CDU, Berlin und Beiratsmitglied im Forum \u00d6kologisch-Soziale Marktwirtschaft.",
				"en": "R\u00fcdiger Kruse (born 10 June 1961 in Hamburg, West Germany), is a German politician who represents the Christian Democratic Union (CDU) in the Bundestag, the German federal parliament.  Kruse sits in the Bundestag as an ordinary member of the Audit Committee, the Budget Committee and a deputy member of the Committee on Environment, Nature Conservation and Nuclear Safety. A member of the Budget Committee since 2009, he serves as his parliamentary group's rapporteur on the budgets of the Federal Chancellery and the Federal Government Commissioner for Culture and the Media. In the Audit Committee, he serves as the rapporteur on the budget of the Federal Ministry of Defense (BMVg). He is also a member of the so-called Confidential Committee (Vertrauensgremium) of the Budget Committee, which provides budgetary supervision for Germany\u2019s three intelligence services, BND, BfV and MAD. Between 2009 and 2013, he was also part of the Parliamentary Friendship Group for Relations with the Baltic States."
			},
			"full_name": "R\u00fcdiger Kruse",
			"images": {
				"copyright": "G\u00f6tz Wrage",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "293.jpg",
				"pathToThumb": "293_t.jpg"
			},
			"name": "R\u00fcdiger Kruse",
			"party": "CDU",
			"pid": "293",
			"self_bird": "goldammer",
			"twittering": {
				"twitterId": 20916357,
				"twitterUserName": "RuedigerKruse"
			}
		},
		"294": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Bettina Irene Kudla (* 20. Juli 1962 in M\u00fcnchen) ist eine deutsche Politikerin der CDU.  Im 18. Deutschen Bundestag geh\u00f6rt sie dem Finanzausschuss an und ist Berichterstatterin ihrer Fraktion f\u00fcr allgemeine Versicherungswirtschaft, Freie Berufe und Sozialversicherung. Ferner geh\u00f6rt sie dem Unterausschuss Europa des Haushaltsausschusses und dem Unterausschuss Kommunales des Innenausschusses an.",
				"en": "Bettina Irene Kudla (born 20 July 1962) is a German politician. She has served as a Member of Federal Parliament for Leipzig from Christian Democratic Union of Germany (CDU) since 27 September 2009. Bettina Kudla joined the Parliament with 33.3% of the vote at the 2009 election and she was elected again with 40% of the vote at the 2013 election.  In parliament, Kudla is a member of the Finance Committee and the Budget Committee\u2019s Sub-Committee on European Union Affairs. On the Finance Committee, she is her parliamentary group\u2019s rapporteur on all issues related to the Federal Central Tax Office (BZSt) and tax advisors."
			},
			"full_name": "Bettina Kudla",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "294.jpg",
				"pathToThumb": "294_t.jpg"
			},
			"name": "Bettina Kudla",
			"party": "CDU",
			"pid": "294",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 3301440772,
				"twitterUserName": "KudlaLeipzig"
			}
		},
		"295": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "Christian K\u00fchn (* 9. April 1979 in T\u00fcbingen), auch oft kurz Chris K\u00fchn, ist ein deutscher Politiker und Mitglied des Deutschen Bundestages. Von 2009 bis 2013 war er Landesvorsitzender des Landesverbands Baden-W\u00fcrttemberg von B\u00fcndnis 90/Die Gr\u00fcnen.  Am 13. Juli 2012 wurde er in T\u00fcbingen als Bundestagskandidat nominiert.  Seit Dezember 2013 ist er Sprecher f\u00fcr Bau- und Wohnungspolitik der Bundestagsfraktion von B\u00fcndnis 90/Die Gr\u00fcnen und vertritt seine Fraktion als ordentliches Mitglied im Ausschuss f\u00fcr Umwelt, Naturschutz, Bau- und Reaktorsicherheit sowie als Obmann im Unterausschuss Kommunales. Daneben ist Chris K\u00fchn stellvertretendes Mitglied in den Aussch\u00fcssen f\u00fcr Recht und Verbraucherschutz, Kultur und Medien sowie Verkehr und digitale Infrastruktur.",
				"en": "Christian K\u00fchn is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Christian K\u00fchn",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "295.jpg",
				"pathToThumb": "295_t.jpg"
			},
			"name": "Christian K\u00fchn",
			"party": "GR\u00dcNE",
			"pid": "295",
			"self_bird": "rotkehlchen",
			"twittering": {
				"twitterId": 1143130812,
				"twitterUserName": "ChrisKuehn_mdb"
			}
		},
		"296": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Stephan K\u00fchn (* 6. September 1979 in Dresden) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und seit 2009 Mitglied des Deutschen Bundestages.  Im Bundestag war er bis Juli 2011 baupolitischer Sprecher der Fraktion B\u00fcndnis 90/Die Gr\u00fcnen. Seit Juli 2011 ist er verkehrspolitischer Sprecher der Fraktion. Dar\u00fcber hinaus ist Stephan K\u00fchn Sprecher der Arbeitsgruppe Ost und war Mitglied des Ausschusses f\u00fcr Verkehr, Bau und Stadtentwicklung und stellvertretendes Mitglied im Innenausschuss. In seiner zweiten Mandatsperiode ist er Sprecher f\u00fcr Verkehrspolitik und ordentliches Mitglied im neu benannten Ausschuss f\u00fcr Verkehr und digitale Infrastruktur sowie stellvertretendes Mitglied im Petitionsausschuss.",
				"en": "Stephan K\u00fchn is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Stephan K\u00fchn",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "296.jpg",
				"pathToThumb": "296_t.jpg"
			},
			"name": "Stephan K\u00fchn",
			"party": "GR\u00dcNE",
			"pid": "296",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": 104100946,
				"twitterUserName": "stephankuehn"
			}
		},
		"298": {
			"citizen_bird": "star",
			"cv": {
				"de": "Roy K\u00fchne (* 27. September 1967 in Magdeburg) ist ein deutscher Politiker (CDU) und seit 2013 Mitglied des Deutschen Bundestages.  Er ist in der 18. Legislaturperiode des Deutschen Bundestags ordentliches Mitglied im Ausschuss f\u00fcr Gesundheit, wo er Berichterstatter f\u00fcr Heil- und Hilfsmittel und verantwortlich f\u00fcr nicht-\u00e4rztliche Gesundheitsberufe ist, und im Ausschuss f\u00fcr Tourismus.",
				"en": "Roy K\u00fchne is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Roy K\u00fchne",
			"images": {
				"copyright": "K\u00fchne",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "298.jpg",
				"pathToThumb": "298_t.jpg"
			},
			"name": "Roy K\u00fchne",
			"party": "CDU",
			"pid": "298",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 1906911697,
				"twitterUserName": "Dr_Roy_Kuehne"
			}
		},
		"299": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Renate Elly K\u00fcnast (* 15. Dezember 1955 in Recklinghausen) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie war von 2001 bis 2005 Bundesministerin f\u00fcr Ern\u00e4hrung, Landwirtschaft und Verbraucherschutz und von 2005 bis 2013 Vorsitzende der Bundestagsfraktion ihrer Partei.  Seit der Bundestagswahl 2002 ist Renate K\u00fcnast Mitglied des Deutschen Bundestages. K\u00fcnast ist seitdem immer \u00fcber die Landesliste Berlin in den Bundestag eingezogen. Im Bundestag ist K\u00fcnast eines von 12 Mitgliedern des Wahlausschusses, der die H\u00e4lfte der Richter des Bundesverfassungsgerichts bestimmt.",
				"en": "Renate Elly K\u00fcnast (born 15 December 1955) is a German politician of Alliance '90/The Greens. She was the Minister of Consumer Protection, Food and Agriculture from 2001 to 2005 and subsequently served as chairwoman of her party's parliamentary group in the Bundestag.  After the 2013 federal elections, the resignation of K\u00fcnast and J\u00fcrgen Trittin as co-leaders of the Green Party's group in parliament cleared the way for the election of Katrin G\u00f6ring-Eckardt and Anton Hofreiter.[9] In 2014, K\u00fcnast was instead elected chairwoman of the Committee on Legal Affairs and Consumer Protection. Already since 2005, she also been serving on the Committee on the Election of Judges (Wahlausschuss), which is in charge of appointing judges to the Federal Constitutional Court of Germany."
			},
			"full_name": "Renate K\u00fcnast",
			"images": {
				"copyright": "Harald Krichel",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "299.jpg",
				"pathToThumb": "299_t.jpg"
			},
			"name": "Renate K\u00fcnast",
			"party": "GR\u00dcNE",
			"pid": "299",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 1425265488,
				"twitterUserName": "RenateKuenast"
			}
		},
		"301": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Markus Kurth (* 14. April 1966 in Beuel) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen).  Seit 2002 ist er Mitglied des Deutschen Bundestages. Dort war er von 2002 bis 2013 sozialpolitischer Sprecher der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen. Seit 2005 war er zus\u00e4tzlich Fraktionssprecher f\u00fcr Behindertenpolitik und Leiter der Fraktionsarbeitsgemeinschaft Gr\u00fcne Politik f\u00fcr Menschen mit Behinderungen. Im Dezember 2013 hat Kurth das Amt des rentenpolitischen Sprechers \u00fcbernommen. Er ist Obmann der Gr\u00fcnen im Ausschuss f\u00fcr Arbeit und Soziales. Im Gesundheitsausschuss ist er stellvertretendes Mitglied.",
				"en": "Markus Kurth is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Markus Kurth",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "301.jpg",
				"pathToThumb": "301_t.jpg"
			},
			"name": "Markus Kurth",
			"party": "GR\u00dcNE",
			"pid": "301",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": 150188536,
				"twitterUserName": "markuskurthmdb"
			}
		},
		"309": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Christian Lange (* 27. Februar 1964 in Saarlouis) ist ein deutscher Politiker (SPD). Er ist Parlamentarischer Staatssekret\u00e4r beim Bundesminister der Justiz und f\u00fcr Verbraucherschutz.  Seit 2013 ist er Parlamentarischer Staatssekret\u00e4r im Bundesministerium der Justiz und f\u00fcr Verbraucherschutz. Nachdem er schon seit 1998 stellvertretender Vorsitzender war, ist er seit 2002 Vorsitzender der Deutsch-Portugiesischen Parlamentariergruppe. Seit 2006 ist er stellvertretender Vorsitzender des Gespr\u00e4chskreises Israel der SPD-Bundestagsfraktion.",
				"en": "Christian Lange is a German politician. He is a member of the SPD."
			},
			"full_name": "Christian Lange",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "309.jpg",
				"pathToThumb": "309_t.jpg"
			},
			"name": "Christian Lange",
			"party": "SPD",
			"pid": "309",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 328939758,
				"twitterUserName": "LangeMdB"
			}
		},
		"313": {
			"citizen_bird": "hausrotschwanz",
			"cv": {
				"de": "Karl Wilhelm Lauterbach (* 21. Februar 1963 in D\u00fcren) ist ein deutscher Mediziner, Politiker (SPD) und seit 2005 Mitglied des Deutschen Bundestages.  Er war in der 17. Legislaturperiode der Sprecher der Arbeitsgruppe Gesundheit der SPD-Bundestagsfraktion, seine Nachfolgerin wurde Hilde Mattheis. Seit Ende 2013 ist er stellvertretender Vorsitzender der SPD-Bundestagsfraktion. Lauterbach ist zudem ordentliches Mitglied im Ausschuss f\u00fcr Gesundheit sowie stellvertretendes Mitglied im Finanzausschuss.  Neben seinem Schwerpunkt, der Gesundheitspolitik, engagiert sich Lauterbach auch in anderen Politikbereichen, etwa in der Bildungs- oder Sozialpolitik. Er wird dem linken Fl\u00fcgel der SPD zugerechnet und ist ein erkl\u00e4rter Gegner des dreigliedrigen Schulsystems.  Lauterbach hat sich schon in seiner Zeit als Wissenschaftler f\u00fcr seine gesundheitspolitischen Vorstellungen eingesetzt, u. a. als Berater der Gesundheitsministerin Ulla Schmidt.",
				"en": "Karl Lauterbach (born 21 February 1963) is a German scientist and politician (SPD). He is professor of health economics and epidemiology at the University of Cologne.  Ahead of the 2013 elections, Peer Steinbr\u00fcck included Lauterbach in his shadow cabinet for the Social Democrats\u2019 campaign to unseat incumbent Angela Merkel as chancellor. During the campaign, he served as shadow minister of health. In the negotiations to form a government following the elections, he led the SPD delegation in the health working group; his co-chair from the CDU/CSU was Jens Spahn. He later became deputy chairman of the SPD parliamentary group under the leadership of Thomas Oppermann.  Lauterbach is a strong advocate of the so-called B\u00fcrgerversicherung, mainly favoured by the Social Democrats. These idea includes the reorganization of the German health system and the incorporation of all people and all income groups into the financing of the health care system."
			},
			"full_name": "Prof. Dr. Karl Lauterbach",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "313.jpg",
				"pathToThumb": "313_t.jpg"
			},
			"name": "Karl Lauterbach",
			"party": "SPD",
			"pid": "313",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": 3292982985,
				"twitterUserName": "Karl_Lauterbach"
			}
		},
		"314": {
			"citizen_bird": "zilpzalp",
			"cv": {
				"de": "Caren Nicole Lay (* 11. Dezember 1972 in Neuwied) ist eine deutsche Politikerin (Die Linke). Sie ist seit 2009 Mitglied des Bundestags und wurde auf dem 3. Bundesparteitag im Juni 2012 zur stellvertretenden Vorsitzenden ihrer Partei gew\u00e4hlt. Bei der Bundestagswahl 2013 war sie eine von acht Spitzenkandidaten der Linken.  Auf dem Bundeskongress im April 2008 wurde sie neben Stefan Liebich und Inga Nitz zu einem der drei Sprecher des Forums gew\u00e4hlt. Nach ihrer Nominierung als Bundesgesch\u00e4ftsf\u00fchrerin im Jahr 2010 gab sie diesen Posten auf. Im Vorfeld des G\u00f6ttinger Bundesparteitages 2012 pl\u00e4dierte sie gemeinsam mit Katja Kipping, Katharina Schwabedissen, Jan van Aken und Thomas Nord f\u00fcr einen \u201eDritten Weg\u201c jenseits des reformorientierten und traditionellen Fl\u00fcgels der Linken.",
				"en": "Caren Lay is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Caren Lay",
			"images": {
				"license": "custom-linke",
				"pathToImage": "314.jpg",
				"pathToThumb": "314_t.jpg"
			},
			"name": "Caren Lay",
			"party": "DIE LINKE",
			"pid": "314",
			"self_bird": "gimpel",
			"twittering": {
				"twitterId": 21404382,
				"twitterUserName": "CarenLay"
			}
		},
		"315": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Monika Lazar (* 13. September 1967 in Leipzig) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen) und seit 2004 Mitglied des Deutschen Bundestages.  Am 21. Dezember 2004 r\u00fcckte sie f\u00fcr die ausgeschiedene Bundestagsabgeordnete Antje Hermenau nach und ist seitdem Mitglied des Deutschen Bundestages. Hier ist sie seit 2005 Sprecherin der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen f\u00fcr Strategien gegen Rechtsextremismus. Sie ist ordentliches Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend. Im 18. Deutschen Bundestag ist Lazar neben ihrer bisherigen Sprecherfunktion Obfrau ihrer Fraktion im Sportausschuss und geh\u00f6rt als ordentliches Mitglied dem Kuratorium der Bundeszentrale f\u00fcr politische Bildung an.",
				"en": "Monika Lazar is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Monika Lazar",
			"images": {
				"copyright": "Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen",
				"license": "CC-BY-2.0",
				"pathToImage": "315.jpg",
				"pathToThumb": "315_t.jpg"
			},
			"name": "Monika Lazar",
			"party": "GR\u00dcNE",
			"pid": "315",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 60630139,
				"twitterUserName": "monikalazar"
			}
		},
		"317": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Sabine Leidig (* 7. August 1961 in Heidelberg) ist eine deutsche Politikerin, Gewerkschafterin, gelernte Biologielaborantin und war von 2002 bis 2009 Gesch\u00e4ftsf\u00fchrerin von Attac Deutschland. Seit September 2009 ist sie Bundestagsabgeordnete f\u00fcr die Partei Die Linke.  Sabine Leidig ist seit 2009 verkehrspolitische Sprecherin der Linksfraktion. In der 17. Legislatur war sie bis zur Vorlage des Abschlussberichtes Mitglied der Enquete-Kommission Wachstum, Wohlstand, Lebensqualit\u00e4t \u2013 Wege zu nachhaltigem Wirtschaften und gesellschaftlichem Fortschritt in der Sozialen Marktwirtschaft. Sie leitete die Projektgruppe 5: Arbeitswelt, Konsumverhalten und Lebensstile.  Sie ist Mitglied im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur des Bundestages und stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft. Weiterhin geh\u00f6rt sie dem Parlamentarischen Beirat f\u00fcr nachhaltige Entwicklung an und ist im Vorstand der fraktions\u00fcbergreifenden Parlamentsgruppe Schienenverkehr.",
				"en": "Sabine Leidig is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Sabine Leidig",
			"images": {
				"license": "custom-linke",
				"pathToImage": "317.jpg",
				"pathToThumb": "317_t.jpg"
			},
			"name": "Sabine Leidig",
			"party": "DIE LINKE",
			"pid": "317",
			"self_bird": "star",
			"twittering": {
				"twitterId": 257461232,
				"twitterUserName": "SabineLeidig"
			}
		},
		"318": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "Katja Isabel Leikert (* 3. M\u00e4rz 1975 in Neustadt an der Weinstra\u00dfe als Katja Isabel R\u00fcb) ist eine deutsche Politikerin (CDU).  In der 18. Legislaturperiode des Deutschen Bundestags ist sie ordentliches Mitglied im Ausschuss f\u00fcr Gesundheit sowie Schriftf\u00fchrerin und stellvertretendes Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend und im Ausw\u00e4rtigen Ausschuss.",
				"en": "Katja Leikert is a German politician. She is a member of the CDU."
			},
			"full_name": "Dr. Katja Leikert",
			"images": {
				"copyright": "Martin Kraft",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "318.jpg",
				"pathToThumb": "318_t.jpg"
			},
			"name": "Katja Leikert",
			"party": "CDU",
			"pid": "318",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 467381196,
				"twitterUserName": "KLeikert"
			}
		},
		"319": {
			"citizen_bird": "mauersegler",
			"cv": {
				"de": "Steffi Lemke (* 19. Januar 1968 in Dessau, DDR) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen) und Mitglied des Deutschen Bundestages. Sie war zudem von 2002 bis 2013 politische Bundesgesch\u00e4ftsf\u00fchrerin ihrer Partei.  Steffi Lemke ist ordentliches Mitglied im Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit. Sie ist stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft.  Seit 2013 ist Steffi Lemke eine von vier parlamentarischen Gesch\u00e4ftsf\u00fchrerinnen der Bundestagsfraktion von B\u00fcndnis 90/Die Gr\u00fcnen.",
				"en": "Steffi Lemke is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Steffi Lemke",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "319.jpg",
				"pathToThumb": "319_t.jpg"
			},
			"name": "Steffi Lemke",
			"party": "GR\u00dcNE",
			"pid": "319",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 17229513,
				"twitterUserName": "SteffiLemke"
			}
		},
		"32": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Maik Beermann (* 19. Oktober 1981 in Nienburg/Weser) ist ein deutscher Politiker (CDU). Er wurde bei der Bundestagswahl 2013 in den 18. Deutschen Bundestag gew\u00e4hlt.  Beermann ist ordentliches Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend und dem Ausschuss f\u00fcr Digitale Agenda. Er ist stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft[1] sowie im Ausschuss f\u00fcr Verkehr und Digitale Infrastruktur. Au\u00dferdem ist er Stellvertreter im parlamentarischen Beirat f\u00fcr nachhaltige Entwicklung und Mitglied der Jungen Gruppe der CDU/CSU-Bundestagsfraktion.",
				"en": "Maik Beermann is a German politician. He is a member of the CDU."
			},
			"full_name": "Maik Beermann",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "32.jpg",
				"pathToThumb": "32_t.jpg"
			},
			"name": "Maik Beermann",
			"party": "CDU",
			"pid": "32",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 569257486,
				"twitterUserName": "MaikBeermann"
			}
		},
		"320": {
			"citizen_bird": "tuerkentaube",
			"cv": {
				"de": "Steffen-Claudio Lemme (* 1. April 1965 in Erfurt) ist ein deutscher Politiker (SPD) und Gewerkschafter. Seit Oktober 2009 ist er Mitglied des Deutschen Bundestages.  Seit 2006 war er Landesvorsitzender des DGB Th\u00fcringen, sowie Vorsitzender der DGB-Region Mittel-/Nordth\u00fcringen und stellvertretender Bezirksvorsitzender des DGB-Bezirks Hessen-Th\u00fcringen. Im November 2009 wurde Renate Licht zur neuen Landesvorsitzenden gew\u00e4hlt.",
				"en": "Steffen-Claudio Lemme is a German politician. He is a member of the SPD."
			},
			"full_name": "Steffen-Claudio Lemme",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "320.jpg",
				"pathToThumb": "320_t.jpg"
			},
			"name": "Steffen-Claudio Lemme",
			"party": "SPD",
			"pid": "320",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 26102710,
				"twitterUserName": "SCLemme"
			}
		},
		"321": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Philipp David Lengsfeld (* 21. M\u00e4rz 1972 in Ost-Berlin) ist ein deutscher Politiker. Seit 2013 ist er Bundestagsabgeordneter (CDU).  Im Bundestag ist er Mitglied der Deutsch-Koreanischen Parlamentariergruppe.",
				"en": "Philipp Lengsfeld is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Philipp Lengsfeld",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "321.jpg",
				"pathToThumb": "321_t.jpg"
			},
			"name": "Philipp Lengsfeld",
			"party": "CDU",
			"pid": "321",
			"self_bird": "gartengrasmuecke",
			"twittering": {
				"twitterId": 2166255624,
				"twitterUserName": "PLengsfeld"
			}
		},
		"323": {
			"citizen_bird": "gruenfink",
			"cv": {
				"de": "Andreas Lenz (* 23. April 1981 in Ebersberg) ist ein deutscher Politiker (CSU) aus Jakobneuharting.  In der 18. Wahlperiode arbeitet Lenz als ordentliches Mitglied im Ausschuss f\u00fcr Wirtschaft und Energie sowie im Parlamentarischen Beirat f\u00fcr nachhaltige Entwicklung und als stellvertretendes Mitglied im Finanzausschuss, im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung sowie im Unterausschuss Regionale Wirtschaftspolitik und ERP-Wirtschaftspl\u00e4ne.",
				"en": "Andreas Lenz is a German politician. She is a member of the CSU."
			},
			"full_name": "Dr. Andreas Lenz",
			"images": {
				"copyright": "MdB B\u00fcro Andreas Lenz",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "323.jpg",
				"pathToThumb": "323_t.jpg"
			},
			"name": "Andreas Lenz",
			"party": "CSU",
			"pid": "323",
			"self_bird": "saatkraehe",
			"twittering": {
				"twitterId": "3635730797",
				"twitterUserName": "DerLenzMdB"
			}
		},
		"325": {
			"citizen_bird": "kohlmeise",
			"cv": {
				"de": "Michael Gerhard Leutert (* 8. August 1974 in Schlema) ist ein deutscher Politiker (Die Linke).  In der 18. Wahlperiode seit 2013 ist Leutert im Haushaltsausschuss f\u00fcr die Etats des Verteidigungsministeriums, des Au\u00dfenministeriums, des Bundesministeriums f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung, des Bundesministeriums f\u00fcr Familie, Senioren, Frauen und Jugend und des Bundesrechnungshofs zust\u00e4ndig. Zudem ist er Obmann im Rechnungspr\u00fcfungsausschuss und Vorsitzender der deutsch-mexikanischen Parlamentariergruppe.",
				"en": "Michael Leutert is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Michael Leutert",
			"images": {
				"license": "custom-linke",
				"pathToImage": "325.jpg",
				"pathToThumb": "325_t.jpg"
			},
			"name": "Michael Leutert",
			"party": "DIE LINKE",
			"pid": "325",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 93612750,
				"twitterUserName": "MichaelLeutert"
			}
		},
		"326": {
			"citizen_bird": "bachstelze",
			"cv": {
				"de": "Ursula Gertrud von der Leyen (geb. Albrecht; * 8. Oktober 1958) ist eine deutsche Politikerin der CDU und seit dem 17. Dezember 2013 Bundesministerin der Verteidigung im Kabinett Merkel III.  Am 17. August 2005 wurde sie von Angela Merkel in das Kompetenzteam f\u00fcr die Bundestagswahl 2005 der CDU/CSU f\u00fcr die Bereiche Familie und Gesundheit berufen. Am 22. November 2005 wurde Ursula von der Leyen als Bundesministerin f\u00fcr Familie, Senioren, Frauen und Jugend in der von Bundeskanzlerin Angela Merkel gef\u00fchrten Bundesregierung, dem Kabinett Merkel I, vereidigt.  Bei der Bundestagswahl 2009 wurde von der Leyen \u00fcber die Landesliste Niedersachsen in den Deutschen Bundestag gew\u00e4hlt. Anschlie\u00dfend wurde sie am 28. Oktober 2009 im Kabinett Merkel II erneut als Familienministerin vereidigt.  Nach dem R\u00fccktritt des Bundesministers f\u00fcr Arbeit und Soziales, Franz Josef Jung, am 27. November 2009 wurde von der Leyen am 30. November 2009 zu dessen Nachfolgerin ernannt.  Nachdem sie bei der Bundestagswahl 2013 wieder in den Bundestag gew\u00e4hlt wurde, hat von der Leyen am 17. Dezember 2013 nach dreimonatigen Koalitionsverhandlungen das Amt der Verteidigungsministerin in der gro\u00dfen Koalition von CDU/CSU und SPD \u00fcbernommen und ist damit die erste Frau an der Spitze des Verteidigungsministeriums.",
				"en": "Ursula Gertrud von der Leyen (n\u00e9e Albrecht; born 8 October 1958) is a German politician who has been the Minister of Defence since 2013, and is the first woman in German history to hold that office. A doctor by profession, she previously also served as the Minister of Labour and Social Affairs from 2009 to 2013 and as the Minister of Senior Citizens, Women and Youth from 2005 to 2009.   In 2013, Ursula von der Leyen was appointed as Germany's first female defence minister. By placing a major party figure such as von der Leyen at the head of the Defence Ministry, Merkel was widely seen as reinvigorating the scandal-ridden ministry\u2019s morale and prestige. Along with Finance Minister Wolfgang Sch\u00e4uble and Interior Minister Thomas de Maiziere, von der Leyen is one of only three ministers to remain with Merkel since she became chancellor in 2005.  Von der Leyen chairs the EPP Defence Ministers Meeting, which gathers EPP defence ministers ahead of meetings of the Council of the European Union."
			},
			"full_name": "Dr. Ursula von der Leyen",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "326.jpg",
				"pathToThumb": "326_t.jpg"
			},
			"name": "Ursula von der Leyen",
			"party": "CDU",
			"pid": "326",
			"self_bird": "dohle",
			"twittering": {
				"twitterId": 45194009,
				"twitterUserName": "vdleyen"
			}
		},
		"328": {
			"citizen_bird": "bachstelze",
			"cv": {
				"de": "Stefan Liebich (* 30. Dezember 1972 in Wismar) ist ein deutscher Politiker (Die Linke). Er ist seit 2009 Mitglied des Deutschen Bundestages und war zuvor seit 1995 Mitglied des Berliner Abgeordnetenhauses.  Er ist Mitglied des Ausw\u00e4rtigen Ausschusses des Bundestages, in der 18. Wahlperiode als Obmann der Fraktion Die Linke.",
				"en": "Stefan Liebich is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Stefan Liebich",
			"images": {
				"license": "custom-linke",
				"pathToImage": "328.jpg",
				"pathToThumb": "328_t.jpg"
			},
			"name": "Stefan Liebich",
			"party": "DIE LINKE",
			"pid": "328",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 1065899292,
				"twitterUserName": "berlinliebich"
			}
		},
		"329": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Ingbert Liebing (* 11. Mai 1963 in Flensburg) ist ein deutscher Politiker (CDU). Er ist seit 2005 Mitglied des Deutschen Bundestages und seit November 2014 Landesvorsitzender der CDU Schleswig-Holstein. Nach der Bundestagswahl 2009 wurde Liebing stellvertretender Vorsitzender der Arbeitsgemeinschaft Kommunalpolitik der CDU/CSU-Bundestagsfraktion und stellvertretender Vorsitzender im Unterausschuss Kommunalpolitik im Deutschen Bundestag. Dem Ausschuss f\u00fcr Umwelt, Naturschutz und Reaktorsicherheit und dem Ausschuss f\u00fcr Tourismus geh\u00f6rte er als ordentliches Mitglied an.  Seit 2013 ist Liebing in seiner Bundestagsfraktion Vorsitzender der Arbeitsgemeinschaft Kommunalpolitik und kommunalpolitischer Sprecher, Mitglied im Fraktionsvorstand sowie Obmann im Unterausschuss Kommunalpolitik im Bundestag. Zudem sitzt Liebing seitdem im Ausschuss f\u00fcr Wirtschaft und Energie. Als stellvertretendes Mitglied geh\u00f6rt er dem Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit, Innenausschuss und dem Ausschuss f\u00fcr Tourismus an.  Liebing ist Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag.",
				"en": "Ingbert Liebing (born May 11, 1963) is a German politician and CDU member of the Bundestag, where he represents Nordfriesland \u2013 Dithmarschen Nord. Since November 2014, he has been serving as chairman of the CDU state party group of Schleswig-Holstein.  Liebing has been a member of the German Bundestag since the 2005 national elections. Between 2005 and 2013, he served on the Committee on the Environment, Nature Conservation, Building and Nuclear Safety as well as on the Committee on Tourism. On the environment committee, he was his parliamentary group's rapporteur on marine conservation. In addition, he was the deputy chairman of the German delegation to the Baltic Sea Parliamentary Conference (BSPC) from 2005 to 2009.  Since 2009, Liebing has been a member of the Sub-Committee on Municipal Policy, serving as the committee\u2019s deputy chairman between 2009 and 2013. He also serves on the Committee on Economic Affairs and Energy. In 2013, he was elected as chairman of the Regional Government Association of the CDU/CSU (KPV)."
			},
			"full_name": "Ingbert Liebing",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "329.jpg",
				"pathToThumb": "329_t.jpg"
			},
			"name": "Ingbert Liebing",
			"party": "CDU",
			"pid": "329",
			"self_bird": "tannenmeise",
			"twittering": {
				"twitterId": 2785989037,
				"twitterUserName": "IngbertLiebing"
			}
		},
		"33": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Herbert Behrens (* 30. Mai 1954 in Osterholz-Scharmbeck) ist ein deutscher Gewerkschafter und Politiker der Partei Die Linke. Er zog 2009 als einer von sechs nieders\u00e4chsischen Kandidaten seiner Partei in den Bundestag ein. In der 17. Wahlperiode war er ordentliches Mitglied im Ausschuss f\u00fcr Verkehr, Bau und Stadtentwicklung, im Unterausschuss Neue Medien, sowie stellvertretendes Mitglied im Ausschuss f\u00fcr Gesundheit und im Ausschuss f\u00fcr Kultur und Medien. Daneben arbeitete er in der Enquete-Kommission Internet und digitale Gesellschaft mit. Bei der Wahl im September 2013 zog Behrens erneut \u00fcber die nieders\u00e4chsische Landesliste f\u00fcr die Partei Die Linke in den Bundestag ein.  Er ist Obmann der Linksfraktion im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur, Mitglied im Ausschuss Digitale Agenda und Mitglied im Rechnungspr\u00fcfungsausschuss. Behrens ist auch Mitglied im Ausschuss f\u00fcr niederdeutsche Sprache und Mitglied der Ostseeparlamentarierkonferenz.",
				"en": "Herbert Behrens is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Herbert Behrens",
			"images": {
				"license": "custom-linke",
				"pathToImage": "33.jpg",
				"pathToThumb": "33_t.jpg"
			},
			"name": "Herbert Behrens",
			"party": "DIE LINKE",
			"pid": "33",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 573861390,
				"twitterUserName": "HerbertBehrens"
			}
		},
		"331": {
			"citizen_bird": "eichelhaeher",
			"cv": {
				"de": "Andrea Lindholz (* 25. September 1970 in Bonn als Andrea Milnik) ist eine deutsche Politikerin (CSU).  Am 22. September 2013 zog Andrea Lindholz f\u00fcr den Bundeswahlkreises Stadt und Landkreis Aschaffenburg in den Deutschen Bundestag ein und trat die Nachfolge von Norbert Geis an.  Im Bundestag ist sie Ordentliches Mitglied im Innenausschuss, im NSA-Untersuchungsausschuss sowie im Ausschuss f\u00fcr die Angelegenheiten der Europ\u00e4ischen Union. Als stellvertretendes Mitglied sitzt sie im Ausschuss f\u00fcr Kultur und Medien.",
				"en": "Andrea Lindholz is a German politician. She is a member of the CSU."
			},
			"full_name": "Andrea Lindholz",
			"images": {
				"copyright": "Lindholz",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "331.jpg",
				"pathToThumb": "331_t.jpg"
			},
			"name": "Andrea Lindholz",
			"party": "CSU",
			"pid": "331",
			"self_bird": "gartenrotschwanz",
			"twittering": {
				"twitterId": 1446031532,
				"twitterUserName": "AndreaLindholz"
			}
		},
		"332": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Tobias Lindner (* 11. Januar 1982 in Karlsruhe) ist deutscher Volkswirt und Politiker (B\u00fcndnis 90/Die Gr\u00fcnen).  Im Juni 2011 r\u00fcckte Lindner f\u00fcr Ulrike H\u00f6fken als Mitglied des Deutschen Bundestages \u00fcber die Landesliste Rheinland-Pfalz nach. Seither war Lindner in der 17. Wahlperiode Mitglied im Haushaltsausschuss und Rechnungspr\u00fcfungsausschuss sowie stellvertretendes Mitglied im Verteidigungsausschuss. Am 27. M\u00e4rz 2012 wurde Lindner zum Sprecher f\u00fcr Wirtschaftspolitik der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen gew\u00e4hlt und war fortan Mitglied im Ausschuss f\u00fcr Wirtschaft und Technologie. Des Weiteren \u00fcbernahm er das Amt des stellvertretenden Vorsitzenden der Deutsch-Irischen Parlamentariergruppe und des stellvertretenden Vorsitzenden der rheinland-pf\u00e4lzischen Landesgruppe der Gr\u00fcnen-Bundestagsabgeordneten. Am 1. Dezember 2012 wurde Lindner auf der Landesdelegiertenversammlung seiner Partei auf Platz zwei der Landesliste f\u00fcr die Bundestagswahl 2013 gew\u00e4hlt. Im September 2013 wurde er schlie\u00dflich \u00fcber die Landesliste erneut in den neuen Bundestag gew\u00e4hlt.  In der 18. Wahlperiode des Deutschen Bundestages ist Lindner Obmann der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen im Haushaltsausschuss, Ordentliches Mitglied im Verteidigungsausschuss und im Rechnungspr\u00fcfungsausschuss. Zudem ist er stellvertretendes Mitglied der Parlamentarischen Versammlung der NATO. In der rheinland-pf\u00e4lzischen Landesgruppe der Gr\u00fcnen-Bundestagsabgeordneten ist er weiterhin stellvertretender Vorsitzender.",
				"en": "Tobias Lindner (born January 11, 1982 in Karlsruhe) is a German economist and politician from the political party B\u00fcndnis 90/Die Gr\u00fcnen. Lindner is the current member for the electorate of Germersheim, Rhineland-Palatinate in the Bundestag.  In June 2011, Lindner became a member of the German Parliament when he filled in the seat of Ulrike H\u00f6fken who had previously resigned. Since then, Lindner has been serving on the budget committee and the audit committee. He is also a member of the defence committee. In addition, he served as deputy chairman of the German-Irish Parliamentary Friendship Group.  In the 2013 federal elections, Lindner was reelected. On the budget committee, he is his parliamentary group's rapporteur on the budgets of the Federal Ministry of Finance, the Federal Foreign Office, the Federal Ministry of Defence, the German Bundesrat and the Federal Court of Auditors. In addition, he serves as deputy chairman of the German-Canadian Parliamentary Friendship Group."
			},
			"full_name": "Dr. Tobias Lindner",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "332.jpg",
				"pathToThumb": "332_t.jpg"
			},
			"name": "Tobias Lindner",
			"party": "GR\u00dcNE",
			"pid": "332",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 21197524,
				"twitterUserName": "tobiaslindner"
			}
		},
		"335": {
			"citizen_bird": "fitis",
			"cv": {
				"de": "Burkhard Lischka (* 1. Februar 1965 in Marsberg) ist ein deutscher Politiker (SPD) und seit 2009 Mitglied des Deutschen Bundestages. Zuvor war er von 2006 bis 2009 Staatssekret\u00e4r im Ministerium der Justiz des Landes Sachsen-Anhalt.  Bei den Bundestagswahlen 2009 und 2013 war Lischka Direktkandidat der SPD im Wahlkreis Magdeburg und zog jeweils \u00fcber die SPD-Landesliste in den Bundestag ein, 2013 als Spitzenkandidat der sachsen-anhaltischen SPD. Lischka war von 2011 bis 2014 Sprecher f\u00fcr Recht und Verbraucherschutz und ist seit September 2014 als Mitglied des Bundestags-Innenausschusses innenpolitischer Sprecher der SPD-Bundestagsfraktion. Er ist zudem SPD-Obmann im Parlamentarischen Kontrollgremium zur Kontrolle der Nachrichtendienste (PKGr) und stellvertretendes Mitglied des NSA-Untersuchungsausschusses. Seit 2011 ist er au\u00dferdem Mitglied im Vorstand der SPD-Bundestagsfraktion.",
				"en": "Burkhard Lischka is a German politician. He is a member of the SPD."
			},
			"full_name": "Burkhard Lischka",
			"images": {
				"copyright": "Daniela Laske",
				"license": "custom-spd",
				"pathToImage": "335.jpg",
				"pathToThumb": "335_t.jpg"
			},
			"name": "Burkhard Lischka",
			"party": "SPD",
			"pid": "335",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 36665085,
				"twitterUserName": "LischkaB"
			}
		},
		"338": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Hiltrud Lotze (* 13. November 1958 in Bauhaus, Hessen) ist eine deutsche Politikerin der Sozialdemokratischen Partei Deutschlands.  Bei der Bundestagswahl am 22. September 2013 kandidierte sie auf einem Listenplatz der SPD, \u00fcber den ihr der Einzug in den Bundestag gelang. Sie ist Mitglied des Ausschusses f\u00fcr Kultur und Medien und des Ausschusses f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit.",
				"en": "Hiltrud Lotze is a German politician. She is a member of the SPD."
			},
			"full_name": "Hiltrud Lotze",
			"images": {
				"copyright": "Dbtg / Renate Blanke",
				"license": "custom-spd",
				"pathToImage": "338.jpg",
				"pathToThumb": "338_t.jpg"
			},
			"name": "Hiltrud Lotze",
			"party": "SPD",
			"pid": "338",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 38272947,
				"twitterUserName": "HiltrudLotze"
			}
		},
		"339": {
			"citizen_bird": "hausrotschwanz",
			"cv": {
				"de": "Gesine L\u00f6tzsch (* 7. August 1961 in Berlin-Lichtenberg) ist eine deutsche Politikerin der Partei Die Linke. Seit 2005 ist sie stellvertretende Vorsitzende der Linksfraktion im Deutschen Bundestag. Von 2010 bis 2012 war sie eine der beiden Parteivorsitzenden.  L\u00f6tzsch zog seit 2002 f\u00fcr den Wahlkreises Berlin-Lichtenberg in den Bundestag ein.  Gesine L\u00f6tzsch und Klaus Ernst wurden Ende Januar 2010 vom Parteivorstand der Die Linke f\u00fcr eine Doppelspitze der Partei vorgeschlagen. Am 10. April 2012 trat sie wegen einer Erkrankung ihres Mannes von diesem Amt zur\u00fcck.  Am 15. Januar 2014 wurde Gesine L\u00f6tzsch zur Vorsitzenden des Haushaltsausschusses des Deutschen Bundestages gew\u00e4hlt.  Nach dem R\u00fccktritt von Evrim Sommer wurde L\u00f6tzsch wieder zur Bezirksvorsitzenden des Lichtenberger Kreisverbands der Linken gew\u00e4hlt.",
				"en": "Gesine L\u00f6tzsch (German pronunciation: [ge\u02c8zin\u0259 l\u00f8\u02d0t\u0283]; born August 7, 1961) is a German politician of the left-wing party Die Linke (\"The Left\"). In 2010, with Klaus Ernst, she was elected president of the party.  In 2002, as a candidate of the Party of Democratic Socialism, L\u00f6tzsch was elected to the German parliament (the Bundestag) for the constituency Berlin Lichtenberg, which she continues to represent today. For her first term, she and Petra Pau were the only PDS deputies in the chamber."
			},
			"full_name": "Dr. Gesine L\u00f6tzsch",
			"images": {
				"license": "custom-linke",
				"pathToImage": "339.jpg",
				"pathToThumb": "339_t.jpg"
			},
			"name": "Gesine L\u00f6tzsch",
			"party": "DIE LINKE",
			"pid": "339",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": 24178198,
				"twitterUserName": "Team_GLoetzsch"
			}
		},
		"34": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Manfred Behrens (* 28. Oktober 1956 in Magdeburg) ist ein deutscher Politiker (CDU).  Am 27. September 2009 gewann Behrens bei der Bundestagswahl das Direktmandat im Bundestagswahlkreis B\u00f6rde \u2013 Jerichower Land und wurde Mitglied des Deutschen Bundestages. Er ist in der 17. Wahlperiode ordentliches Mitglied des Innenausschusses sowie stellvertretendes Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend.  Bei der Bundestagswahl 2013 gewann er erneut das Direktmandat f\u00fcr seinen Wahlkreis und sitzt nun im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur und ist weiterhin stellvertretendes Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend.",
				"en": "Manfred Behrens is a German politician. He is a member of the CDU."
			},
			"full_name": "Manfred Behrens",
			"images": {
				"copyright": "Renate Blanke/Dt. Bundestag",
				"license": "CC-BY-3.0 de",
				"pathToImage": "34.jpg",
				"pathToThumb": "34_t.jpg"
			},
			"name": "Manfred Behrens",
			"party": "CDU",
			"pid": "34",
			"self_bird": "saatkraehe",
			"twittering": {
				"twitterId": 61577387,
				"twitterUserName": "Manfredbehrens"
			}
		},
		"341": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Jan-Marco Luczak (* 2. Oktober 1975 in Berlin) ist ein Berliner Rechtsanwalt und Politiker (CDU).  Seit 2009 ist Jan-Marco Luczak Abgeordneter im Deutschen Bundestag. Seine inhaltlichen Schwerpunkte liegen im Bereich der Rechts-, Wirtschafts- und Europapolitik. Er ist Mitglied im Ausschuss f\u00fcr Recht und Verbraucherschutz, dessen stellvertretender Vorsitzender er seit Beginn der 18. Wahlperiode ist. Luczak geh\u00f6rt zudem dem Unterausschuss Europarecht sowie dem Ausschuss f\u00fcr die Wahl der Bundesverfassungsrichter an. Daneben ist er stellvertretendes Mitglied im Innen- und Umweltausschuss. Au\u00dferdem geh\u00f6rt Luczak dem H\u00f6rfunkrat des Deutschlandradios an und ist stellvertretendes Mitglied im Beirat der Bundesnetzagentur.  Luczak ist unter anderem Mitglied in der Deutsch-Franz\u00f6sischen sowie der Deutsch-Polnischen Parlamentariergruppe.",
				"en": "Jan-Marco Luczak (born 2 October 1975) is a German lawyer and a politician (CDU). He represents the Tempelhof-Sch\u00f6neberg constituency at the Bundestag.  As a member of the Committee on Legal Affairs, the Sub-Committee on European Law and as an alternate member of the Committee on Internal Affairs, the majority of his work focuses on legal affairs and economic policy. Since 2014, he has been deputy chairman of the Committee on Legal Affairs. In addition, he has been serving on the Committee on the Election of Judges (Wahlausschuss), which is in charge of appointing judges to the Federal Constitutional Court of Germany.  Within the parliament Luczak is additionally a member of the German Parliamentary Society. He is also a member of the German-French, the German-Polish and the German-American Parliamentary Friendship Groups."
			},
			"full_name": "Dr. Jan-Marco Luczak",
			"images": {
				"copyright": "Deutscher Bundestag/M\u00fcller",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "341.jpg",
				"pathToThumb": "341_t.jpg"
			},
			"name": "Jan-Marco Luczak",
			"party": "CDU",
			"pid": "341",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": "22908417",
				"twitterUserName": "JM_Luczak"
			}
		},
		"344": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Thomas Lutze (* 23. August 1969 in Elsterwerda) ist ein deutscher Politiker der Partei Die Linke. Er ist seit Oktober 2009 Mitglied des Deutschen Bundestages.  Lutze ist ordentliches Mitglied des Ausschusses f\u00fcr Wirtschaft und Energie und des Ausschusses f\u00fcr Verkehr und digitale Infrastruktur sowie Mitglied des Ausschusses f\u00fcr Tourismus und stellvertretender Vorsitzender der Deutsch-Koreanischen Parlamentariergruppe. F\u00fcr die Linksfraktion des Deutschen Bundestages bekleidet er die Funktion des Obmanns im Ausschuss f\u00fcr Wirtschaft und Energie.  Als Mitglied des Bundestages engagiert Lutze sich vor allem im Bereich der Innenpolitik, schwerpunktm\u00e4\u00dfig in den Bereichen Verkehr, Bau und Stadtentwicklung. Weitere Bereiche in denen, teilweise seit den 1990ern, ein deutliches Engagement wahrnehmbar sind: Abschaltung von Cattenom, Forderung Castor-Transporte zu stoppen, und Bek\u00e4mpfung von Jugendarmut.",
				"en": "Thomas Lutze is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Thomas Lutze",
			"images": {
				"license": "custom-linke",
				"pathToImage": "344.jpg",
				"pathToThumb": "344_t.jpg"
			},
			"name": "Thomas Lutze",
			"party": "DIE LINKE",
			"pid": "344",
			"self_bird": "gartenrotschwanz",
			"twittering": {
				"twitterId": "3667108397",
				"twitterUserName": "thlutze"
			}
		},
		"346": {
			"citizen_bird": "dohle",
			"cv": {
				"de": "Yvonne Magwas (* 28. November 1979 in Rodewisch) ist eine deutsche Politikerin (CDU).  Bei den Kommunalwahlen in Sachsen 2003 wurde sie Stadtr\u00e4tin in Auerbach und Kreisr\u00e4tin im Vogtlandkreis und \u00fcbte die beiden \u00c4mter bis 2009 bzw. 2005 aus. Bei der Bundestagswahl 2013 wurde Magwas \u00fcber Platz 12 der CDU-Landesliste Sachsen zum Mitglied des Deutschen Bundestages gew\u00e4hlt. Sie ist ordentliches Mitglied im Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit sowie im Ausschuss f\u00fcr Kultur und Medien. Die Bundesregierung entsendete sie 2014 in den H\u00f6rfunkrat des Deutschlandradios und ist auch dessen Mitglied im Finanz- und Wirtschaftsausschuss.",
				"en": "Yvonne Magwas (born in November 28, 1979 in Rodewisch in Eastern Germany (GDR)) is a German politician of the CDU  In the local elections in 2003, she was elected Councillor in Auerbach and district councillor in the Vogtland and practiced the two functions until 2009 or rather until 2005. In the general election in 2013 she was elected as a member of the German Bundestag \u2013 being on 12th position of CDU regional list. She is a full member of the Committee for the Environment, Nature Conservation, Building and Nuclear Safety and the Committee on Culture and Media. There she is rapporteur on rental policy, housing and urban development in the Committee on Environment, Nature Conservation, Building and Nuclear Safety. She is also member of the German Taiwanese friendship group in the German parliament. The federal government dispatched her to the Council for Radio and Broadcasting Association in Germany where she is also member of the Financial and Economic Committee."
			},
			"full_name": "Yvonne Magwas",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "346.jpg",
				"pathToThumb": "346_t.jpg"
			},
			"name": "Yvonne Magwas",
			"party": "CDU",
			"pid": "346",
			"self_bird": "saatkraehe",
			"twittering": {
				"twitterId": "3852561082",
				"twitterUserName": "YvonneMagwas"
			}
		},
		"348": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Nicole Maisch (* 20. April 1981 in Hanau) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Am 20. Februar 2007 r\u00fcckte sie \u00fcber die Landesliste Hessen f\u00fcr den ausgeschiedenen Abgeordneten Matthias Berninger in den Bundestag nach und ist seitdem Mitglied des Deutschen Bundestages.  Im Bundestag ist Nicole Maisch Sprecherin f\u00fcr Verbraucherpolitik und Tierschutz der Fraktion B\u00fcndnis 90/Die Gr\u00fcnen. Sie ist Ordentliches Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft und dessen Obfrau ihrer Fraktion, sowie des Ausschusses f\u00fcr Recht und Verbraucherschutz.",
				"en": "Nicole Maisch is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Nicole Maisch",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "348.jpg",
				"pathToThumb": "348_t.jpg"
			},
			"name": "Nicole Maisch",
			"party": "GR\u00dcNE",
			"pid": "348",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 18723339,
				"twitterUserName": "NicoleMaisch"
			}
		},
		"350": {
			"citizen_bird": "tuerkentaube",
			"cv": {
				"de": "Birgit Malecha-Nissen (* 9. April 1958 in Essen) ist eine deutsche Geologin und Politikerin der SPD.  Am 22. September 2013 gelang ihr bei der Bundestagswahl 2013 der Einzug in den Bundestag \u00fcber einen Listenplatz der SPD.",
				"en": "Birgit Malecha-Nissen is a German politician. She is a member of the SPD."
			},
			"full_name": "Dr. Birgit Malecha-Nissen",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "350.jpg",
				"pathToThumb": "350_t.jpg"
			},
			"name": "Birgit Malecha-Nissen",
			"party": "SPD",
			"pid": "350",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 1230819176,
				"twitterUserName": "malechanissen"
			}
		},
		"351": {
			"citizen_bird": "hausrotschwanz",
			"cv": {
				"de": "Gisela Manderla (* 11. Februar 1958 in D\u00fcsseldorf-Kaiserswerth) ist eine deutsche Politikerin (CDU).  Bei der Bundestagswahl 2013 wurde Manderla \u00fcber die CDU-Landesliste Nordrhein-Westfalen zum Mitglied des Deutschen Bundestages gew\u00e4hlt. Sie ist Ordentliches Mitglied im Verteidigungsausschuss und stellvertretendes Mitglied des Gesundheitsausschusses. Sie ist Pr\u00e4sidiumsmitglied der Deutschen Gesellschaft f\u00fcr Wehrtechnik und Mitglied der Deutsch-Slowenischen und der Deutsch-Australischen Parlamentariergruppe. Ihr Wahlkreis in K\u00f6ln ist Nippes, Ehrenfeld und Chorweiler.  Sie ist Schirmherrin des K\u00f6lner Segelverbandes f\u00fcr Behinderte (SFEHB) und stellvertretende Vorsitzende des Bundes Katholischer Unternehmer (BKU), Di\u00f6zesangruppe K\u00f6ln.",
				"en": "Gisela Manderla is a German politician. She is a member of the CDU."
			},
			"full_name": "Gisela Manderla",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "351.jpg",
				"pathToThumb": "351_t.jpg"
			},
			"name": "Gisela Manderla",
			"party": "CDU",
			"pid": "351",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 1095781218,
				"twitterUserName": "ManderlaGisela"
			}
		},
		"353": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Matern Freiherr Marschall von Bieberstein (* 3. August 1962 in Freiburg im Breisgau) ist ein deutscher Politiker (CDU). In der \u00d6ffentlichkeit ist er unter dem Namen Matern von Marschall bekannt. Seit Oktober 2013 ist er Mitglied des Bundestages f\u00fcr den Wahlkreis Freiburg.  Im Deutschen Bundestag ist Matern von Marschall in zwei Aussch\u00fcssen ordentliches Mitglied. Im Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit ist er der Berichterstatter der CDU/CSU-Bundestagsfraktion f\u00fcr die Schwerpunkte Nachhaltigkeit, Umweltforschung und umweltbezogenen Verbraucherschutz. Im Ausschuss f\u00fcr die Angelegenheiten der Europ\u00e4ischen Union ist von Marschall f\u00fcr die Themenfelder Klimapolitik, Umweltschutz, Kultur und Medien sowie Fragen zur Informationsgesellschaft zust\u00e4ndig. Dar\u00fcber hinaus hat er die L\u00e4nderberichterstattung f\u00fcr Portugal, Spanien und die T\u00fcrkei inne. Als ordentliches Mitglied im Parlamentarischen Beirat f\u00fcr nachhaltige Entwicklung (PBNE) liegt von Marschalls Schwerpunkt bei der Gr\u00fcnen Gentechnik.  Am 30. Januar 2014 stimmte er mit vier weiteren Abgeordneten der CDU/CSU-Fraktion mit der Opposition gegen eine Zulassung der Genmaissorte 1507. Matern von Marschall und seine Abgeordnetenkollegen Josef G\u00f6ppel, Hubert H\u00fcppe, Hans-Georg von der Marwitz und Martin Patzelt stie\u00dfen damit eine innerparteiliche Diskussion an, die zu einer Neupositionierung der CDU/CSU f\u00fchrte.",
				"en": "Matern von Marschall is a German politician. He is a member of the CDU."
			},
			"full_name": "Matern von Marschall",
			"images": {
				"copyright": "J\u00fcrgen Brandel",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "353.jpg",
				"pathToThumb": "353_t.jpg"
			},
			"name": "Matern von Marschall",
			"party": "CDU",
			"pid": "353",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": "1227428528",
				"twitterUserName": "Von_Marschall"
			}
		},
		"356": {
			"citizen_bird": "ringeltaube",
			"cv": {
				"de": "Andreas Mattfeldt (* 28. September 1969 in V\u00f6lkersen) ist ein deutscher Politiker (CDU).  Bei den Bundestagswahlen 2009 und 2013 gewann er im Wahlkreis Osterholz \u2013 Verden das Direktmandat. Seit 2011 ist er au\u00dferdem Mitglied im Kreistag Verden und Kreisausschussmitglied und au\u00dferdem Mitglied im CDU Bezirksvorstand. Im Deutschen Bundestag ist er Mitglied im Haushalts- und Petitionsausschuss (Obmann f\u00fcr die CDU/CSU-Fraktion). Dar\u00fcber hinaus geh\u00f6rt er dem Unterausschuss Rechnungspr\u00fcfung des Haushaltsausschusses an und ist stellvertretendes Mitglied im Finanzausschuss.  Innerhalb der CDU/CSU-Fraktion ist Mattfeldt Initiator und F\u00fchrer der \u201eMattfeldt-Gruppe\u201c, eines Zusammenschlusses von Unions-Abgeordneten, deren Ziel die Verhinderung von Gesetzen ist, welche Hydraulic Fracturing in Deutschland erm\u00f6glichen.  Mattfeldt geh\u00f6rte bei der Abstimmung \u00fcber die Erteilung eines Verhandlungsmandates an die Bundesregierung zur Aufnahme von Verhandlungen f\u00fcr ein drittes Hilfspaket im Zusammenhang mit der Griechischen Depression zu den Abgeordneten der Gro\u00dfen Koalition, die gegen ein solches Mandat stimmten.",
				"en": "Andreas Mattfeldt is a German politician. He is a member of the CDU."
			},
			"full_name": "Andreas Mattfeldt",
			"images": {
				"copyright": "Die Hoffotografen/Christine Blohmann",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "356.jpg",
				"pathToThumb": "356_t.jpg"
			},
			"name": "Andreas Mattfeldt",
			"party": "CDU",
			"pid": "356",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 23447001,
				"twitterUserName": "AMattfeldt"
			}
		},
		"357": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Hilde Mattheis geb. Gudelius (* 6. Oktober 1954 in Finnentrop) ist eine baden-w\u00fcrttembergische Politikerin der SPD und Abgeordnete des Deutschen Bundestags. Sie ist Vorsitzende des Forums Demokratische Linke 21.  Hilde Mattheis ist seit 2002 Mitglied des Deutschen Bundestags und hier seit 2005 stellvertretende Sprecherin der Arbeitsgruppe \u201eGesundheit\u201c sowie Vorsitzende der Arbeitsgruppe \u201eVerteilungsgerechtigkeit und soziale Integration\u201c der SPD-Bundestagsfraktion. Von Dezember 2005 bis November 2013 geh\u00f6rte sie au\u00dferdem dem SPD-Fraktionsvorstand an. Seit der 18. Legislaturperiode ist sie Sprecherin der Arbeitsgruppe \u201eGesundheit\u201c.  Die Schwerpunkte ihrer politischen Arbeit im Bundestag sind Gesundheitspolitik, Pflege, Gleichstellungspolitik und Verteilungsgerechtigkeit.  Sie ist in der 18. Legislaturperiode des Deutschen Bundestags ordentliches Mitglied im Ausschuss f\u00fcr Gesundheit und stellvertretendes Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend.",
				"en": "Hilde Mattheis is a German politician. She is a member of the SPD."
			},
			"full_name": "Hilde Mattheis",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "357.jpg",
				"pathToThumb": "357_t.jpg"
			},
			"name": "Hilde Mattheis",
			"party": "SPD",
			"pid": "357",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": 115614153,
				"twitterUserName": "HildeMattheis"
			}
		},
		"360": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Michael G\u00fcnther Meister (* 9. Juni 1961 in Lorsch) ist ein deutscher Politiker (CDU). Er ist seit Dezember 2013 Parlamentarischer Staatssekret\u00e4r beim Bundesminister f\u00fcr Finanzen.  Seit 1994 ist er Mitglied des Deutschen Bundestages. Dort war er von 1996 bis 1998 Stellvertretender Vorsitzender der Enqu\u00eate-Kommission Zukunft der Medien in Wirtschaft und Gesellschaft \u2013 Deutschlands Weg in die Informationsgesellschaft. Von 2002 bis 2004 war Meister Vorsitzender der Arbeitsgruppe Finanzen der CDU/CSU-Bundestagsfraktion. Nach dem R\u00fccktritt von Friedrich Merz wurde er im Dezember 2004 zum Stellvertretenden Vorsitzenden der CDU/CSU-Fraktion gew\u00e4hlt und war dort zust\u00e4ndig f\u00fcr die Bereiche Wirtschaft, Mittelstand, Finanzen und Haushalt. Anl\u00e4sslich seiner Ernennung zum Staatssekret\u00e4r endete sein Amt im Dezember 2013.",
				"en": "Michael Meister is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Michael Meister",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "360.jpg",
				"pathToThumb": "360_t.jpg"
			},
			"name": "Michael Meister",
			"party": "CDU",
			"pid": "360",
			"self_bird": "gartengrasmuecke",
			"twittering": {
				"twitterId": 34214048,
				"twitterUserName": "meister_schafft"
			}
		},
		"361": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Peter Meiwald (* 31. M\u00e4rz 1966 in Oldenburg) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). \u00dcber die Landesliste Niedersachsen wurde er bei der Bundestagswahl 2013 in den Deutschen Bundestag gew\u00e4hlt.  Im 18. Bundestag ist er ordentliches Mitglied des Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit und der Obmann seiner Fraktion sowie im Petitionsausschuss des Deutschen Bundestages. Des Weiteren ist er stellvertretendes Mitglied im Ausschuss f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung und Parlamentarischer Beirat f\u00fcr nachhaltige Entwicklung.",
				"en": "Peter Meiwald is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Peter Meiwald",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "361.jpg",
				"pathToThumb": "361_t.jpg"
			},
			"name": "Peter Meiwald",
			"party": "GR\u00dcNE",
			"pid": "361",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 19464497,
				"twitterUserName": "PeterMeiwald"
			}
		},
		"364": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Jan Metzler (* 5. Juli 1981 in Worms) ist ein deutscher Politiker (CDU). Am 22. September 2013 kandidierte er als Direktkandidat f\u00fcr den Bundestagswahlkreis Worms.  Er ist Mitglied im Bundesvorstand des Jungen Wirtschaftsrates, stellvertretender Landesvorsitzender des Jungen Wirtschaftsrates im Wirtschaftsrat Rheinland-Pfalz und Mitglied der Deutsch-Ukrainischen Gesellschaft f\u00fcr Wirtschaft und Wissenschaft",
				"en": "Jan Metzler (born 5 July 1981 in Worms, West Germany) is a German politician who represents the Christian Democratic Union (CDU) in the Bundestag.  He was selected as the CDU candidate for Worms in November 2012."
			},
			"full_name": "Jan Metzler",
			"images": {
				"copyright": "Martin Kraft",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "364.jpg",
				"pathToThumb": "364_t.jpg"
			},
			"name": "Jan Metzler",
			"party": "CDU",
			"pid": "364",
			"self_bird": "zaunkoenig",
			"twittering": {
				"twitterId": 995653004,
				"twitterUserName": "JanMetzler"
			}
		},
		"369": {
			"citizen_bird": "star",
			"cv": {
				"de": "Irene Mihalic (* 17. November 1976 in Waldbr\u00f6l) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen) aus Gelsenkirchen. Sie ist seit 2013 Mitglied des Bundestages.  Irene Mihalic war anf\u00e4nglich Sprecherin f\u00fcr innere Sicherheit in der Bundestagsfraktion von B\u00fcndnis 90/Die Gr\u00fcnen. Seit April 2016 ist sie Sprecherin f\u00fcr Innenpolitik.  Sie ist Obfrau im Innenausschuss. Sie war Obfrau ihrer Fraktion im Untersuchungsausschuss zur Edathy-Aff\u00e4re, der im Juli 2014 eingesetzt wurde und im Dezember 2015 seinen abschlie\u00dfenden Bericht abgeliefert hat.  Zudem ist sie Obfrau im zweiten NSU-Untersuchungsausschuss des Bundestages, der im November 2015 seine Arbeit aufgenommen hat.  Sie ist stellvertretende Koordinatorin des Arbeitskreises III der Bundestagsfraktion sowie stellvertretendes Mitglied im Ausschuss f\u00fcr Recht und Verbraucherschutz.",
				"en": "Irene Mihalic is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Irene Mihalic",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "369.jpg",
				"pathToThumb": "369_t.jpg"
			},
			"name": "Irene Mihalic",
			"party": "GR\u00dcNE",
			"pid": "369",
			"self_bird": "haussperling",
			"twittering": {
				"twitterId": 143000381,
				"twitterUserName": "IreneMihalic"
			}
		},
		"372": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Susanne Mittag (* 25. Juli 1958 in Cleveland, Ohio, USA) ist eine deutsche Politikerin (SPD) und wurde bei der Bundestagswahl am 22. September 2013 \u00fcber die nieders\u00e4chsische Landesliste ihrer Partei in den Deutschen Bundestag gew\u00e4hlt. Sie trat im Bundestagswahlkreis Delmenhorst \u2013 Wesermarsch \u2013 Oldenburg-Land (Wahlkreis 28) an.  Sie ist ordentliches Mitglied im Innenausschuss, stellvertretende Vorsitzende des NSA-Untersuchungsausschusses und stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft.",
				"en": "Susanne Mittag is a German politician. She is a member of the SPD."
			},
			"full_name": "Susanne Mittag",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "372.jpg",
				"pathToThumb": "372_t.jpg"
			},
			"name": "Susanne Mittag",
			"party": "SPD",
			"pid": "372",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": "939034891",
				"twitterUserName": "susanne_mittag"
			}
		},
		"373": {
			"citizen_bird": "hausrotschwanz",
			"cv": {
				"de": "Cornelia M\u00f6hring (* 9. Januar 1960 in Hamburg) ist eine deutsche Politikerin (Die Linke). 2009 zog sie erstmals als Abgeordnete in den Bundestag ein. Von 2009 bis 2011 war sie frauenpolitische Sprecherin der Linksfraktion im Bundestag und wurde im November 2011 zur Ersten stellvertretenden Fraktionsvorsitzenden gew\u00e4hlt. Mit Beginn der 18. Legislatur ist sie erneut Mitglied im Deutschen Bundestag. Sie wurde von ihrer Fraktion erneut als frauenpolitische Sprecherin und als stellvertretende Vorsitzende gew\u00e4hlt.  Cornelia M\u00f6hring war in der vergangenen Legislaturperiode stellvertretendes Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend, sowie in den Aussch\u00fcssen Gesundheit und Arbeit und Soziales. Mit Beginn der 18. Legislatur ist sie Vollmitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend.",
				"en": "Cornelia M\u00f6hring is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Cornelia M\u00f6hring",
			"images": {
				"license": "custom-linke",
				"pathToImage": "373.jpg",
				"pathToThumb": "373_t.jpg"
			},
			"name": "Cornelia M\u00f6hring",
			"party": "DIE LINKE",
			"pid": "373",
			"self_bird": "zilpzalp",
			"twittering": {
				"twitterId": 2596171296,
				"twitterUserName": "Conni_Moehring"
			}
		},
		"375": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Karsten M\u00f6ring (* 30. August 1949 in Schneverdingen) ist ein deutscher Politiker (CDU).",
				"en": "Karsten M\u00f6ring is a German politician. He is a member of the CDU."
			},
			"full_name": "Karsten M\u00f6ring",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "375.jpg",
				"pathToThumb": "375_t.jpg"
			},
			"name": "Karsten M\u00f6ring",
			"party": "CDU",
			"pid": "375",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 1099159964,
				"twitterUserName": "KarstenMoering"
			}
		},
		"376": {
			"citizen_bird": "grauschnaepper",
			"cv": {
				"de": "Marlene Mortler (geborene Hengelein; * 16. Oktober 1955 in Lauf an der Pegnitz) ist eine deutsche Politikerin (CSU) und seit Januar 2014 Drogenbeauftragte der Bundesregierung.  2002 wurde Mortler Mitglied des Deutschen Bundestages. Hier arbeitete sie vor allem in den Aussch\u00fcssen f\u00fcr Ern\u00e4hrung, Landwirtschaft und Verbraucherschutz sowie f\u00fcr Tourismus. Von 2004 bis 2005 war sie agrar- und verbraucherpolitische Sprecherin der CSU-Landesgruppe. Von 2005 bis 2009 war sie Vorsitzende des Bundestagsausschusses f\u00fcr Tourismus, von 2009 bis Januar 2014 tourismuspolitische Sprecherin der CDU/CSU-Fraktion. Seit Februar 2010 ist sie Mitglied der Deutsch-Maltesischen Parlamentariergruppe, seit Oktober 2011 Mitglied im CSU-Parteivorstand, seit November 2011 Landesvorsitzende der Arbeitsgemeinschaft Landwirtschaft der CSU.  Im Bundestag 2013 ist sie ordentliches Mitglied im Bundestagsausschuss Ern\u00e4hrung und Landwirtschaft, agrarpolitische Sprecherin der CSU-Landesgruppe sowie stellvertretendes Mitglied im Ausschuss f\u00fcr Tourismus.  Mortler ist im Bundestag zudem Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag.",
				"en": "Marlene Mortler (born 16 October 1955) is a German politician (CSU), and the Drug Commissioner of the Federal Government since January 2014.  In 2002, Mortler became a member of the German Bundestag. Here she has worked primarily in the Committees on Food, Agriculture and Consumer Protection, as well as for tourism. From 2004 to 2005, she was agricultural and consumer policy spokesperson for the CSU state group. From 2005 to 2009, she was president of the parliamentary committee for tourism, and Mortler is tourism spokeswoman of the CDU / CSU parliamentary group. Since February 2010, she is member of the German-Maltese and German-Hungarian Parliamentary Group, and since October 2011 a member of the CSU Board and since November 2011 state chairman of the Working Group on Agriculture of the CSU.  In the Bundestag, she is a full member of the Committee on Food and Agriculture and Deputy Member of the Tourism Committee.  Mortler in the Bundestag is also a member of the European Union Parliamentarians German Bundestag."
			},
			"full_name": "Marlene Mortler",
			"images": {
				"copyright": "Henning Schacht",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "376.jpg",
				"pathToThumb": "376_t.jpg"
			},
			"name": "Marlene Mortler",
			"party": "CSU",
			"pid": "376",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 21579707,
				"twitterUserName": "marlenemortler"
			}
		},
		"379": {
			"citizen_bird": "gartenrotschwanz",
			"cv": {
				"de": "Niema Movassat (* 22. August 1984 in Wuppertal) ist Mitglied des Deutschen Bundestages f\u00fcr die Partei DIE LINKE.  Am 27. September 2009 wurde Movassat \u00fcber die Landesliste Nordrhein-Westfalen in den 17. Bundestag gew\u00e4hlt und ist seitdem Mitglied des Deutschen Bundestages. Er war der j\u00fcngste Abgeordnete seiner Fraktion, bis Yvonne Ploetz im Februar 2010 f\u00fcr den zur\u00fcckgetretenen Oskar Lafontaine nachr\u00fcckte. Movassat ist Mitglied und seit 2013 Obmann im Ausschuss f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung sowie Stellvertreter im Ausw\u00e4rtigen Ausschuss. Zudem ist er Sprecher f\u00fcr Weltern\u00e4hrung f\u00fcr die Fraktion DIE LINKE. Von 2009 bis 2013 war er Obmann seiner Fraktion im Unterausschuss Gesundheit in Entwicklungsl\u00e4ndern; diesen Unterausschuss gibt es seit der neuesten Legislaturperiode nicht mehr.  Bei der Bundestagswahl am 22. September 2013 wurde er wieder in den Bundestag gew\u00e4hlt.",
				"en": "Niema Movassat is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Niema Movassat",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "379.jpg",
				"pathToThumb": "379_t.jpg"
			},
			"name": "Niema Movassat",
			"party": "DIE LINKE",
			"pid": "379",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 111054843,
				"twitterUserName": "NiemaMovassat"
			}
		},
		"380": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Bettina M\u00fcller (* 7. Juni 1959 in Alzenau-Wasserlos) ist eine deutsche Rechtsanw\u00e4ltin, Politikerin und Abgeordnete im Deutschen Bundestag (SPD).  Sie ist in der 18. Legislaturperiode des Deutschen Bundestags ordentliches Mitglied im Ausschuss f\u00fcr Gesundheit sowie Schriftf\u00fchrer und stellvertretendes Mitglied im Ausschuss f\u00fcr Recht und Verbraucherschutz.",
				"en": "Bettina M\u00fcller is a German politician. She is a member of the SPD."
			},
			"full_name": "Bettina M\u00fcller",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "380.jpg",
				"pathToThumb": "380_t.jpg"
			},
			"name": "Bettina M\u00fcller",
			"party": "SPD",
			"pid": "380",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 2443895924,
				"twitterUserName": "BetMueller"
			}
		},
		"383": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Gerhard \u201eGerd\u201c M\u00fcller (* 25. August 1955 in Krumbach (Schwaben)) ist seit 1978 ein deutscher Politiker (CSU). Er ist seit Dezember 2013 Bundesminister f\u00fcr wirtschaftliche Zusammenarbeit und Entwicklung.  Seit 1994 ist M\u00fcller Mitglied des Deutschen Bundestages f\u00fcr den Wahlkreis Oberallg\u00e4u (Kempten, Lindau, Oberallg\u00e4u und Westallg\u00e4u). Bis 2005 war er, unter anderem, au\u00dfen- und europapolitischer Sprecher und Stellvertretender Vorsitzender der CSU-Landesgruppe im Deutschen Bundestag. Zudem war er von 2002 bis 2005 im Bundestag in der Position als stellvertretender Vorsitzender des Ausschusses f\u00fcr Fremdenverkehr und Tourismus.  Am 22. November 2005 wurde M\u00fcller als Parlamentarischer Staatssekret\u00e4r beim Bundesministerium f\u00fcr Ern\u00e4hrung, Landwirtschaft und Verbraucherschutz in die von Bundeskanzlerin Angela Merkel gef\u00fchrte Bundesregierung berufen. Wie dem Kabinett Merkel I geh\u00f6rte er auch dem Kabinett Merkel II an.",
				"en": "Gerd M\u00fcller (born 25 August 1955 as Gerhard M\u00fcller in the town of Krumbach, district of G\u00fcnzburg, Bavaria, Germany) is a German politician and member of the Christian Social Union in Bavaria. He is the Federal Minister of Economic Cooperation and Development.  Since 1994, M\u00fcller has been a member of the German Parliament (Bundestag) following the federal election that year. He served on the committees for foreign and defense policy and was the spokesman on Europe, foreign, and development policy for the CSU group.  Since 2013, M\u00fcller has been Federal Minister of Economic Cooperation and Development in the third cabinet of Chancellor Angela Merkel; he was succeeded as Parliamentary State Secretary by Peter Bleser.  In February 2014, M\u00fcller accompanied German President Joachim Gauck on a state visit to India \u2013 where they met with Prime Minister Manmohan Singh and Sonia Gandhi, among others \u2013 and to Myanmar.  In October 2014, M\u00fcller co-chaired the Berlin Conference on the Syrian Refugee Situation along with Foreign Minister Frank-Walter Steinmeier and the UN High Commissioner for Refugees, Ant\u00f3nio Guterres. In November 2014, he and German Environment Minister Barbara Hendricks hosted a conference which raised $10 billion for the Green Climate Fund\u2019s projects to fight global warming.  In August 2015, M\u00fcller was part of Chancellor Merkel\u2019s delegation to the first joint cabinet meeting of the governments of Germany and Brazil in Bras\u00edlia. In January 2016, he participated in the first joint cabinet meeting of the governments of Germany and Turkey in Berlin."
			},
			"full_name": "Dr. Gerd M\u00fcller",
			"images": {
				"copyright": "Conrad Nutschan",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "383.jpg",
				"pathToThumb": "383_t.jpg"
			},
			"name": "Gerd M\u00fcller",
			"party": "CSU",
			"pid": "383",
			"self_bird": "dohle",
			"twittering": {
				"twitterId": "43111409",
				"twitterUserName": "Gerdbillen"
			}
		},
		"384": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Norbert M\u00fcller (* 10. Februar 1986 in Wriezen) ist ein deutscher Politiker (Die Linke). Norbert M\u00fcller ist seit Dezember 2014 Mitglied des Deutschen Bundestages. Er ist Kinder- und Jugendpolitischer Sprecher der Linksfraktion im Deutschen Bundestag und seit Januar 2016 Vorsitzender der Kinderkommission des Deutschen Bundestages.  Zur Bundestagswahl 2013 trat Norbert M\u00fcller auf Platz 6 der Landesliste der Partei Die Linke in Brandenburg an und kandierte im Bundestagswahlkreis 61. Dabei erzielte er 20,0 % der Erststimmen.",
				"en": "Norbert M\u00fcller is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Norbert M\u00fcller",
			"images": {
				"license": "custom-linke",
				"pathToImage": "384.jpg",
				"pathToThumb": "384_t.jpg"
			},
			"name": "Norbert M\u00fcller",
			"party": "DIE LINKE",
			"pid": "384",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 4265444913,
				"twitterUserName": "Norbert_MdB"
			}
		},
		"385": {
			"citizen_bird": "elster",
			"cv": {
				"de": "Stefan M\u00fcller (* 3. September 1975 in Neustadt an der Aisch) ist ein deutscher Politiker (CSU).  Er ist stets als direkt gew\u00e4hlter Abgeordneter des Wahlkreises Erlangen in den Bundestag eingezogen.  Seit Dezember 2013 ist M\u00fcller Parlamentarischer Staatssekret\u00e4r bei der Bundesministerin f\u00fcr Bildung und Forschung im Kabinett Merkel III.  M\u00fcller ist Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag und des Kuratoriums der Stiftung Lesen.",
				"en": "Stefan M\u00fcller is a German politician. He is a member of the CSU."
			},
			"full_name": "Stefan M\u00fcller",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "385.jpg",
				"pathToThumb": "385_t.jpg"
			},
			"name": "Stefan M\u00fcller",
			"party": "CSU",
			"pid": "385",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 17960911,
				"twitterUserName": "smuellermdb"
			}
		},
		"386": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Beate M\u00fcller-Gemmeke (geb. M\u00fcller; *\u00a07. Oktober 1960 in Frankfurt am Main) ist eine deutsche Politikerin der Gr\u00fcnen. Sie ist seit 2009 Mitglied des Deutschen Bundestags.  M\u00fcller-Gemmeke ist Mitglied im Bundestagsausschuss f\u00fcr Arbeit und Soziales und Sprecherin f\u00fcr Arbeitnehmerrechte ihrer Fraktion. Seit 2013 ist sie stellvertretendes Mitglied im Petitionsausschuss.",
				"en": "Beate M\u00fcller-Gemmeke is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Beate M\u00fcller-Gemmeke",
			"images": {
				"copyright": "Steffen Pr\u00f6\u00dfdorf",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "386.jpg",
				"pathToThumb": "386_t.jpg"
			},
			"name": "Beate M\u00fcller-Gemmeke",
			"party": "GR\u00dcNE",
			"pid": "386",
			"self_bird": "schwanzmeise",
			"twittering": {
				"twitterId": 53426644,
				"twitterUserName": "GrueneBeate"
			}
		},
		"389": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "\u00d6zcan Mutlu (* 10. Januar 1968 in Kelkit, T\u00fcrkei) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). Er ist Mitglied des 18. Deutschen Bundestages.  Mit der Wahl zum Mitglied des Deutschen Bundestages wurde er auch zum Sprecher f\u00fcr Bildungspolitik und Sprecher f\u00fcr Sportpolitik der Fraktion B\u00fcndnis 90/Die Gr\u00fcnen im Deutschen Bundestag und er ist zugleich Berichterstatter seiner Fraktion f\u00fcr das Thema B\u00fcrgerbeteiligung. Er ist Ordentliches Mitglied und Sprecher seiner Fraktion im Sportausschuss[8] und im Ausschuss f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung.",
				"en": "\u00d6zcan Mutlu (born January 10, 1968) in Kelkit, G\u00fcm\u00fc\u015fhane Province, Turkey, is a Turkish-German Green Party politician.  He is one of the eleven politicians of Turkish descent who entered the federal parliament, including seven women. In parliament, he has since been serving on the Committee on Education, Research and Technology Assessment. On the committee, he is his parliamentary group\u2019s rapporteur on citizen participation."
			},
			"full_name": "\u00d6zcan Mutlu",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "389.jpg",
				"pathToThumb": "389_t.jpg"
			},
			"name": "\u00d6zcan Mutlu",
			"party": "GR\u00dcNE",
			"pid": "389",
			"self_bird": "tannenmeise",
			"twittering": {
				"twitterId": 73088949,
				"twitterUserName": "OezcanMutlu"
			}
		},
		"392": {
			"citizen_bird": "gartenbaumlaeufer",
			"cv": {
				"de": "Alexander Soranto Neu (* 19. M\u00e4rz 1969 in Harmonie, Gemeinde Eitorf) ist ein deutscher Politiker (Die Linke). Seit der Bundestagswahl 2013 ist er Mitglied des Deutschen Bundestages.  Neu ist Obmann im Verteidigungsausschuss des Deutschen Bundestages und stellvertretendes Mitglied im Ausw\u00e4rtigen Ausschuss.  Weiterhin ist er Mitglied des Aufsichtsrates des Zentrum f\u00fcr Internationale Friedenseins\u00e4tze gemeinn\u00fctzige GmbH (ZIF)",
				"en": "Alexander Neu is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Dr. Alexander S. Neu",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "392.jpg",
				"pathToThumb": "392_t.jpg"
			},
			"name": "Alexander S. Neu",
			"party": "DIE LINKE",
			"pid": "392",
			"self_bird": "kiwi",
			"twittering": {
				"twitterId": 1182878557,
				"twitterUserName": "AlexanderSNeu"
			}
		},
		"393": {
			"citizen_bird": "heckenbraunelle",
			"cv": {
				"de": "Andreas Nick (* 26. April 1967 in Koblenz-Moselwei\u00df) ist ein deutscher Politiker (CDU). Er wurde am 22. September 2013 in seinem Bundestagswahlkreis Montabaur in den Deutschen Bundestag gew\u00e4hlt.  Seit Januar 2014 ist er Mitglied im Ausw\u00e4rtigen Ausschuss; dort ist er federf\u00fchrender Berichterstatter seiner Fraktion f\u00fcr Globalisierungsthemen (UN/internationale Organisationen, Welthandel, Klimaschutz) sowie als Regionalberichterstatter f\u00fcr die T\u00fcrkei, Ungarn, S\u00fcdamerika und Australien zust\u00e4ndig. Er ist ebenfalls Mitglied des Unterausschusses Vereinte Nationen, Internationale Organisationen und Globalisierung sowie stellvertretendes Mitglied der Parlamentarischen Versammlung der OSZE. Im Februar 2014 wurde er zus\u00e4tzlich als Mitglied in den neuen Ausschuss Digitale Agenda berufen, wo er als Berichterstatter unter anderem f\u00fcr Fragen des Breitbandausbaus, der Digitalisierung der Finanzwirtschaft und der Internet Governance zust\u00e4ndig ist. Au\u00dferdem ist Andreas Nick stellvertretendes Mitglied im Finanzausschuss des Deutschen Bundestages.  Im Herbst 2014 wurde er von CDU-Generalsekret\u00e4r Peter Tauber als Mitglied in den neu gebildeten Bundesfachausschuss Au\u00dfen-, Sicherheits-, Entwicklungs- und Menschenrechtspolitik der CDU Deutschlands unter Vorsitz von Roderich Kiesewetter berufen. Au\u00dferdem geh\u00f6rt er dem Netzwerk Digitalisierung unter Vorsitz von Nadine Sch\u00f6n an.",
				"en": "Andreas Nick is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Andreas Nick",
			"images": {
				"copyright": "Bianca Richter",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "393.jpg",
				"pathToThumb": "393_t.jpg"
			},
			"name": "Andreas Nick",
			"party": "CDU",
			"pid": "393",
			"self_bird": "buntspecht",
			"twittering": {
				"twitterId": 49450039,
				"twitterUserName": "DrAndreasNick"
			}
		},
		"395": {
			"citizen_bird": "ringeltaube",
			"cv": {
				"de": "Ulrike \u201eUlli\u201c Nissen (* 16. Juni 1959 in Essen) ist eine deutsche Politikerin (SPD). Sie ist seit 2013 Mitglied des Deutschen Bundestages.  Sie ist ordentliches Mitglied im Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit sowie stellvertretendes Mitglied im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur sowie im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft[1]. Sie ist au\u00dferdem Schriftf\u00fchrerin im Bundestag.",
				"en": "Ulli Nissen is a German politician. She is a member of the SPD."
			},
			"full_name": "Ulli Nissen",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "395.jpg",
				"pathToThumb": "395_t.jpg"
			},
			"name": "Ulli Nissen",
			"party": "SPD",
			"pid": "395",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 382044198,
				"twitterUserName": "UlliNissen"
			}
		},
		"397": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Thomas Nord (* 19. Oktober 1957 in Berlin[1]) ist ein deutscher Politiker (Die Linke). Von 2005 bis 2012 war er Landesvorsitzender der PDS in Brandenburg. Seit 2009 ist er Mitglied des Bundestages, seit 2012 Mitglied des Bundesvorstandes der Linkspartei und seit Mai 2014 deren Bundesschatzmeister.  Im Bundestag arbeitet er dort im Ausschuss f\u00fcr die Angelegenheiten der Europ\u00e4ischen Union. Er ist stellvertretendes Mitglied im Ausschuss f\u00fcr Kultur und Medien sowie im Verteidigungsausschuss.[7] Er ist f\u00fcr die Linksfraktion stellvertretender Vorsitzender der deutsch-polnischen und der deutsch-franz\u00f6sischen Parlamentariergruppe.",
				"en": "Thomas Nord (born 19 October 1957 in Berlin) is a German politician (DIE LINKE) and Member of the German Federal Parliament.  In the 18th Legislature he is a member of the Committee of Economy and Energy, substitute member of the Committee on the Affairs of the European Union and a member of the Subcommittee on Regional Economic Policy. He is chairman of the German-Polish Parliamentary Group and Deputy Chairman of the German-French Parliamentary Friendship Group. He is also a national spokesperson for the Group of Brandenburger Deputies of DIE LINKE."
			},
			"full_name": "Thomas Nord",
			"images": {
				"license": "custom-linke",
				"pathToImage": "397.jpg",
				"pathToThumb": "397_t.jpg"
			},
			"name": "Thomas Nord",
			"party": "DIE LINKE",
			"pid": "397",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 1421571306,
				"twitterUserName": "NordMdb"
			}
		},
		"398": {
			"citizen_bird": "mauersegler",
			"cv": {
				"de": "Konstantin von Notz (* 21. Januar 1971 in M\u00f6lln) ist ein deutscher Jurist und Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). Er ist seit 2009 Mitglied des Deutschen Bundestags.  In der 17. Wahlperiode war er Mitglied des Bundestags-Innenausschusses und geh\u00f6rte als Obmann der Bundestagsfraktion von B\u00fcndnis 90/Die Gr\u00fcnen der Enquete-Kommission Internet und digitale Gesellschaft an. Er war stellvertretendes Mitglied im Rechtsausschuss und im Unterausschuss Neue Medien und vertrat die Bundestagsfraktion von B\u00fcndnis 90/Die Gr\u00fcnen im Gremium nach dem Zollfahndungsdienstgesetzes und der IuK-Kommission, einer Unterkommission des \u00c4ltestenrates des Deutschen Bundestages. In der 18. Wahlperiode ist er erneut ordentliches Mitglied im Bundestags-Innenausschuss sowie Obmann im NSA-Untersuchungsausschuss und im Ausschuss Digitale Agenda. Stellvertretendes Mitglied ist er im Ausschuss f\u00fcr Recht und Verbraucherschutz sowie im Wahlausschuss.",
				"en": "Konstantin von Notz (born 21 January 1971 in M\u00f6lln) is a German lawyer and politician. He is a member of the Alliance '90/The Greens party. Since 2009 he has been an MP of the German Bundestag. He was the speaker for the Interior and Internet Policies of the Green Party faction in the Bundestag from 2009 to 2013. In 2013 he was elected as Vice Faction Leader and again as Speaker for Internet Policies. He is also currently the party chairman within the German Parliamentary Committee investigating the NSA spying scandal and the Digital Agenda Committee in the Bundestag. Von Notz is also a member of Transparency International in Germany."
			},
			"full_name": "Dr. Konstantin von Notz",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "398.jpg",
				"pathToThumb": "398_t.jpg"
			},
			"name": "Konstantin von Notz",
			"party": "GR\u00dcNE",
			"pid": "398",
			"self_bird": "stieglitz",
			"twittering": {
				"twitterId": 17752770,
				"twitterUserName": "KonstantinNotz"
			}
		},
		"399": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Omid Nouripour (* 18. Juni 1975 in Teheran, Iran) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und Mitglied des Deutschen Bundestages (MdB).  Am 1. September 2006 r\u00fcckte er f\u00fcr den ausgeschiedenen Abgeordneten Joschka Fischer \u00fcber die Landesliste Hessen in den Bundestag nach. Dort war er zun\u00e4chst Mitglied im Europaausschuss und von 2008 bis zur Bundestagswahl 2009 Mitglied im Haushalts- und Verteidigungsausschuss.  Von 2009 bis 2013 war Nouripour sicherheitspolitischer Sprecher der Fraktion B\u00fcndnis 90/Die Gr\u00fcnen. Seit der Bundestagswahl 2013 ist er au\u00dfenpolitischer Sprecher seiner Fraktion. Zudem ist er ordentliches Mitglied im Ausw\u00e4rtigen Ausschuss und im Ausschuss f\u00fcr Menschenrechte und humanit\u00e4re Hilfe sowie stellvertretendes Mitglied im Verteidigungsausschuss und im Unterausschuss f\u00fcr Ausw\u00e4rtige Kulturpolitik.",
				"en": "Omid Nouripour (born June 18, 1975) is a German politician of the Alliance '90/The Greens who currently serves as a member of the Bundestag, representing the state of Hesse. He is his party's spokesman on foreign affairs, and is a member of the Committee on Foreign Affairs and a deputy member of the Defence Committee. He is a member of the board of trustees of the pro-American Atlantik-Br\u00fccke organisation, a member of the pro-EU Europa-Union Deutschland caucus in the Bundestag, and a member of the board of trustees of the pro-NATO German Atlantic Association.."
			},
			"full_name": "Omid Nouripour",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "399.jpg",
				"pathToThumb": "399_t.jpg"
			},
			"name": "Omid Nouripour",
			"party": "GR\u00dcNE",
			"pid": "399",
			"self_bird": "haussperling",
			"twittering": {
				"twitterId": 16674128,
				"twitterUserName": "nouripour"
			}
		},
		"4": {
			"citizen_bird": "gruenfink",
			"cv": {
				"de": "Peter Altmaier (* 18. Juni 1958 in Ensdorf, Saar), ist ein deutscher Politiker (CDU). Seit dem 17. Dezember 2013 ist er Bundesminister f\u00fcr besondere Aufgaben und Chef des Bundeskanzleramtes im Kabinett Merkel III. Ab dem 22. Mai 2012 war er Nachfolger von Norbert R\u00f6ttgen als Bundesumweltminister im Kabinett Merkel II. Zuvor war Altmaier Parlamentarischer Gesch\u00e4ftsf\u00fchrer der CDU/CSU-Bundestagsfraktion.  Altmaier ist Mitglied der Europa-Union Parlamentariergruppe Deutscher Bundestag.",
				"en": "Peter Altmaier (born 18 June 1958) is a German politician who has served as the Chief of Staff of the German Chancellery and as Federal Minister for Special Affairs since December 2013. Previously he was Federal Minister for the Environment, Nature Conservation and Nuclear Safety from May 2012 to December 2013. Altmaier is widely seen as one of Chancellor Angela Merkel's most trusted advisors and respected for his \"compromising style\".  In October 2015, Merkel put Altmaier in charge of coordinating Germany's response to the refugee crisis. Since early 2017, he has been a member of the German government's cabinet committee on Brexit at which ministers will discuss organizational and structural issues related to the United Kingdom's departure from the European Union."
			},
			"full_name": "Peter Altmaier",
			"images": {
				"copyright": "kuxma",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "4.jpg",
				"pathToThumb": "4_t.jpg"
			},
			"name": "Peter Altmaier",
			"party": "CDU",
			"pid": "4",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 378693834,
				"twitterUserName": "peteraltmaier"
			}
		},
		"40": {
			"citizen_bird": "saatkraehe",
			"cv": {
				"de": "Peter Beyer (* 25. Dezember 1970 in Ratingen) ist ein deutscher Politiker (CDU) und Rechtsanwalt (Fachanwalt f\u00fcr gewerblichen Rechtsschutz).  Am 22. September 2013 wurde Beyer bei der Bundestagswahl als Abgeordneter in den 18. deutschen Bundestag wiedergew\u00e4hlt. Er ist dort Mitglied des Ausw\u00e4rtigen Ausschusses sowie stellvertretendes Mitglied des Verteidigungsausschusses, des Unterausschusses Vereinte Nationen, Internationale Organisationen und Globalisierung, der Parlamentarischen Versammlung der OSZE.",
				"en": "Peter Beyer LL.M. (born 25 December 1970 in Ratingen, Germany) is a German politician (CDU).  In the federal election on September 22, 2013, Beyer was re-elected as a Member of the Bundestag. Currently, he is a member of the Bundestag's Committee on Foreign Affairs where he serves as Parliamentary Special Rapporteur on Transatlantic Relations.  Beyer describes himself as \"committed Atlanticist\" and \"advocate of a strong transatlantic relationship\". He is a member of the Atlantik-Br\u00fccke (engl. Atlantic Bridge), and of the Deutsche Gesellschaft f\u00fcr ausw\u00e4rtige Politik (engl. German Council on Foreign Relations), both private Think Tanks."
			},
			"full_name": "Peter Beyer",
			"images": {
				"copyright": "Frank N\u00fcrnberger",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "40.jpg",
				"pathToThumb": "40_t.jpg"
			},
			"name": "Peter Beyer",
			"party": "CDU",
			"pid": "40",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 69814084,
				"twitterUserName": "PeterbeyerCDU"
			}
		},
		"400": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Helmut Fedor Nowak (* 19. April 1941 in Beuthen, Oberschlesien) ist ein deutscher Politiker (CDU).  Helmut Nowak ist Mitglied der Landsmannschaft der Oberschlesier, Kreisgruppe K\u00f6ln und nach Herbert Czaja erstes Mitglied der Landsmannschaft, das den Sprung in den Deutschen Bundestag geschafft hat.",
				"en": "Helmut Nowak is a German politician. He is a member of the CDU."
			},
			"full_name": "Helmut Nowak",
			"images": {
				"copyright": "Helmut Nowak / Jutta Hartmann",
				"license": "unknown-bundestag",
				"pathToImage": "400.jpg",
				"pathToThumb": "400_t.jpg"
			},
			"name": "Helmut Nowak",
			"party": "CDU",
			"pid": "400",
			"self_bird": "buchfink",
			"twittering": {
				"twitterId": 1120117190,
				"twitterUserName": "Helmut_Nowak"
			}
		},
		"401": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Georg N\u00fc\u00dflein (* 10. April 1969 in Krumbach/Schwaben) ist ein deutscher Politiker (CSU). Seit 2002 ist er Mitglied des Deutschen Bundestages und seit Januar 2014 Stellvertretender Vorsitzender der CDU/CSU-Bundestagsfraktion.  Seine Zust\u00e4ndigkeiten umfassen die Gesundheits- und Umweltpolitik.",
				"en": "Georg N\u00fc\u00dflein is a German politician. He is a member of the CSU."
			},
			"full_name": "Dr. Georg N\u00fc\u00dflein",
			"images": {
				"copyright": "Henning Schacht",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "401.jpg",
				"pathToThumb": "401_t.jpg"
			},
			"name": "Georg N\u00fc\u00dflein",
			"party": "CSU",
			"pid": "401",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 1630247738,
				"twitterUserName": "georgnuesslein"
			}
		},
		"402": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Julia Obermeier (* 7. September 1984 in M\u00fcnchen als Julia Bartz) ist eine deutsche Politikerin der CSU und Mitglied des Deutschen Bundestages.  Bei der Bundestagswahl 2013 zog Obermeier \u00fcber Platz 39 der CSU-Landesliste in den Deutschen Bundestag ein. Sie ist Mitglied im Verteidigungsausschuss und stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft. Zudem ist sie ordentliches Mitglied der Parlamentarischen Versammlung des Europarates. Dar\u00fcber hinaus ist sie stellvertretendes Mitglied der Parlamentarischen Versammlung der NATO und der Interparlamentarische Konferenz f\u00fcr die Gemeinsame Au\u00dfen- und Sicherheitspolitik und die Gemeinsame Sicherheits- und Verteidigungspolitik.  Obermeier wurde 2015 von der CDU/CSU-Bundestagsfraktion in den Stiftungsrat Treuh\u00e4nderische Stiftung zur Unterst\u00fctzung besonderer H\u00e4rtef\u00e4lle in der Bundeswehr und der ehemaligen NVA (Deutsche H\u00e4rtefallstiftung) berufen.",
				"en": "Julia Obermeier (born Julia Bartz 7 September 1984) is a German politician (CSU). She had been a member of the German parliament (Bundestag) since 2013, and is a member of the assembly's defence committee. Since 2015 she has also been a member of the Parliamentary Assembly of the Council of Europe.  She is a member of the Bundestag Defence Committee and an alternate member of the Food and Agriculture Committee. She is a member of the Parliamentary Assembly of the Council of Europe. She is a deputy member of the NATO Parliamentary Assembly and of the Interparliamentary Conference for Joint Foreign and Security Policy and for Joint Security and Defence Policy."
			},
			"full_name": "Julia Obermeier",
			"images": {
				"copyright": "Helmut Fromberger",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "402.jpg",
				"pathToThumb": "402_t.jpg"
			},
			"name": "Julia Obermeier",
			"party": "CSU",
			"pid": "402",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 2175692210,
				"twitterUserName": "Julia_Obermeier"
			}
		},
		"403": {
			"citizen_bird": "heckenbraunelle",
			"cv": {
				"de": "Wilfried Oellers (* 16. September 1975 in M\u00f6nchengladbach) ist ein deutscher Politiker (CDU) und Mitglied des Deutschen Bundestages.  Er ist ordentliches Mitglied im Ausschuss f\u00fcr Arbeit und Soziales, dort Berichterstatter f\u00fcr Arbeitsrecht, Arbeitsgerichtsgesetz, Tarifrecht und europ\u00e4isches Arbeitsrecht. Dar\u00fcber hinaus ist er stellvertretendes Mitglied in den Aussch\u00fcssen Wirtschaft und Energie sowie Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft.",
				"en": "Wilfried Oellers is a German politician. He is a member of the CDU."
			},
			"full_name": "Wilfried Oellers",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "403.jpg",
				"pathToThumb": "403_t.jpg"
			},
			"name": "Wilfried Oellers",
			"party": "CDU",
			"pid": "403",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 906847944,
				"twitterUserName": "WilfriedOellers"
			}
		},
		"404": {
			"citizen_bird": "haussperling",
			"cv": {
				"de": "Thomas Ludwig Albert Oppermann (* 27. April 1954 in Freckenhorst) ist ein deutscher Politiker (SPD) und seit dem 16.\u00a0Dezember 2013 Fraktionsvorsitzender der SPD-Bundestagsfraktion. Er war von November 2007 bis Dezember 2013 Erster Parlamentarischer Gesch\u00e4ftsf\u00fchrer der SPD-Bundestagsfraktion und Vorsitzender des Parlamentarischen Kontrollgremiums und von 1998 bis 2003 nieders\u00e4chsischer Minister f\u00fcr Wissenschaft und Kultur.  Oppermann geh\u00f6rt dem Netzwerk Berlin und dem Seeheimer Kreis an und ist stellvertretender Vorsitzender der deutsch-israelischen Parlamentariergruppe. Er geh\u00f6rte dem Jahrgang 1992 des \u201eYoung Leaders Programm\u201c des Netzwerkes Atlantik-Br\u00fccke an.",
				"en": "Thomas Ludwig Albert Oppermann[2] (born 27 April 1954 in Freckenhorst, West Germany) is a German politician with the Social Democratic Party (SPD). From 1998-2003, he was the Minister for Science and Culture in the German state of Lower Saxony. He has been the First Secretary of the SPD Parliamentary Group in the Bundestag from November 2007 to December 2013. Currently, he serves as Chairman of the SPD Parliamentary Group."
			},
			"full_name": "Thomas Oppermann",
			"images": {
				"copyright": "Gerrit Sievert",
				"license": "custom-spd",
				"pathToImage": "404.jpg",
				"pathToThumb": "404_t.jpg"
			},
			"name": "Thomas Oppermann",
			"party": "SPD",
			"pid": "404",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 293066780,
				"twitterUserName": "ThomasOppermann"
			}
		},
		"405": {
			"citizen_bird": "haussperling",
			"cv": {
				"de": "Florian O\u00dfner (* 5. Juli 1980 in Vilsbiburg) ist ein deutscher Politiker (CSU) und Mitglied des Deutschen Bundestages.  Bei der Bundestagswahl 2013 gewann O\u00dfner das Direktmandat im Bundestagswahlkreis Landshut. Er ist im Bundestag ordentliches Mitglied im Ausschuss f\u00fcr Verkehr und digitale Infrastruktur und stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft sowie im Ausschuss f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit. Er vertritt die CSU in der Kommission Lagerung hoch radioaktiver Abfallstoffe (Endlagerkommission) gem\u00e4\u00df Standortauswahlgesetz. Er ist Mitglied in den Parlamentariergruppen zwischen Deutschland und den Vereinigten Staaten, Schweden sowie \u00d6sterreich.",
				"en": "Florian O\u00dfner is a German politician. He is a member of the CSU."
			},
			"full_name": "Florian O\u00dfner",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "405.jpg",
				"pathToThumb": "405_t.jpg"
			},
			"name": "Florian O\u00dfner",
			"party": "CSU",
			"pid": "405",
			"self_bird": "weisskopfseeadler",
			"twittering": {
				"twitterId": "1294532364",
				"twitterUserName": "Florian_Ossner"
			}
		},
		"406": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Friedrich Ostendorff (* 12. Januar 1953 in Dortmund) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). Er war von 2002 bis 2005 und ist wiederum seit 2009 Mitglied des Deutschen Bundestages.  Seit Juni 2011 war er Stellvertretender Vorsitzender des Bundestagsausschusses f\u00fcr Ern\u00e4hrung, Landwirtschaft und Verbraucherschutz. Im 18. Bundestag ist er Ordentliches Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft.  Weiterhin ist er Stellvertretendes Mitglied im Ausschuss f\u00fcr Umwelt, Naturschutz und Reaktorsicherheit, sowie im Haushaltsausschuss.  Ostendorff ist Stellvertretender Vorsitzender der Deutsch-Japanischen Parlamentariergruppe.",
				"en": "Friedrich Ostendorff is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Friedrich Ostendorff",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "406.jpg",
				"pathToThumb": "406_t.jpg"
			},
			"name": "Friedrich Ostendorff",
			"party": "GR\u00dcNE",
			"pid": "406",
			"self_bird": "gruenfink",
			"twittering": {
				"twitterId": 862714045,
				"twitterUserName": "FOstendorff"
			}
		},
		"407": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Tim Ostermann (* 16. Juni 1979 in B\u00fcnde) ist ein deutscher Politiker (CDU) und Mitglied des 18. Deutschen Bundestages.  Schwartze ist ordentliches Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend und im Petitionsausschuss. Er ist ebenso stellvertretendes Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft.",
				"en": "Tim Ostermann is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Tim Ostermann",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "407.jpg",
				"pathToThumb": "407_t.jpg"
			},
			"name": "Tim Ostermann",
			"party": "CDU",
			"pid": "407",
			"self_bird": "singdrossel",
			"twittering": {
				"twitterId": "702800400223772672",
				"twitterUserName": "OstermannMdB"
			}
		},
		"409": {
			"citizen_bird": "elster",
			"cv": {
				"de": "Cem \u00d6zdemir (* 21. Dezember 1965 in Urach) ist ein deutscher Politiker und seit November 2008 Bundesvorsitzender der Partei B\u00fcndnis 90/Die Gr\u00fcnen. 1994 waren Cem \u00d6zdemir und Leyla Onur die ersten Bundestagsabgeordneten mit t\u00fcrkischen Eltern.  \u00d6zdemir war kurzzeitig Mitglied der Atlantik-Br\u00fccke, trat aber wegen des Vorsitzenden Friedrich Merz aus und sprach in dem Kontext von \u201eCDU-nahen Machenschaften\u201c; er ist Mitglied des Beirats der Atlantischen Initiative. 2007 war er an der Gr\u00fcndung der europ\u00e4ischen Denkfabrik European Council on Foreign Relations beteiligt.  Er ist offizieller Unterst\u00fctzer des B\u00fcndnisses Freiheit statt Angst f\u00fcr Datenschutz und gegen staatliche \u00dcberwachung.[29] Au\u00dferdem ist er Gr\u00fcndungskurator der Amadeu Antonio Stiftung. Au\u00dferdem ist \u00d6zdemir ehrenamtliches Jurymitglied bei \u201eTop 100\u201c, einer Auszeichnung f\u00fcr die innovativsten Unternehmen im deutschen Mittelstand.",
				"en": "Cem \u00d6zdemir (born 21 December 1965) is a German politician. Since 2008 he is co-chair of the German political party Alliance '90/The Greens, together with Simone Peter. He is a member of the German Bundestag since 2013 and he was a member of the German Bundestag between 1994 and 2002 and of the European Parliament between 2004 and 2009. He is standing to be one of the top two candidates for the Greens for the 2017 German federal election.  He serves as deputy chairman of the German-Chinese Parliamentary Friendship Group."
			},
			"full_name": "Cem \u00d6zdemir",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "409.jpg",
				"pathToThumb": "409_t.jpg"
			},
			"name": "Cem \u00d6zdemir",
			"party": "GR\u00dcNE",
			"pid": "409",
			"self_bird": "haussperling",
			"twittering": {
				"twitterId": 19108766,
				"twitterUserName": "cem_oezdemir"
			}
		},
		"41": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Steffen Bilger (* 16. Februar 1979 in Schongau) ist ein deutscher Politiker (CDU).  Im 17. Bundestag war er ordentliches Mitglied im Ausschuss f\u00fcr Verkehr, Bau und Stadtentwicklung, Mitglied im Parlamentarischen Beirat f\u00fcr nachhaltige Entwicklung und stellvertretendes Mitglied in den Aussch\u00fcssen Gesundheit und Umwelt, Naturschutz und Reaktorsicherheit. Au\u00dferdem war er ordentliches Mitglied in der Enquete-Kommission Wachstum, Wohlstand, Lebensqualit\u00e4t.  Im 18. Bundestag ist er ordentliches Mitglied im Parlamentarischen Beirat f\u00fcr nachhaltige Entwicklung, Ausschuss f\u00fcr Verkehr und digitale Infrastruktur sowie Vorsitzender des Kuratoriums der Bundeszentrale f\u00fcr politische Bildung.  Innerhalb der Arbeitsgruppe Verkehr der CDU/CSU-Bundestagsfraktion ist Bilger u. a. Berichterstatter f\u00fcr Alternative Antriebe. Er leitet auch den Parlamentskreis Elektromobilit\u00e4t des Deutschen Bundestages. Au\u00dferdem ist er Vorsitzender der Jungen Gruppe, des Zusammenschlusses aller Unionsabgeordneten unter 35 Jahren.",
				"en": "Steffen Bilger is a German politician. He is a member of the CDU."
			},
			"full_name": "Steffen Bilger",
			"images": {
				"copyright": "Andreas Essig",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "41.jpg",
				"pathToThumb": "41_t.jpg"
			},
			"name": "Steffen Bilger",
			"party": "CDU",
			"pid": "41",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 23925041,
				"twitterUserName": "SteffenBilger"
			}
		},
		"410": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Mahmut \u00d6zdemir (* 23. Juni 1987 in Duisburg) ist ein deutscher Politiker (SPD). Er wurde bei der Bundestagswahl 2013 ins Parlament gew\u00e4hlt.  In Bundestagsaussch\u00fcssen ist er ordentliches Mitglied im Kuratorium der Bundeszentrale f\u00fcr politische Bildung (seit dem 6. Mai 2014), im Unterausschuss Kommunales (seit dem 18. Februar 2014), und im Innenausschuss (seit dem 13. Januar 2014).",
				"en": "Mahmut \u00d6zdemir (born June 23, 1987 in Duisburg, Germany) is a Turkish-German politician for the Social Democratic Party of Germany (SPD). He has been a member of the Bundestag since being elected in September 2013."
			},
			"full_name": "Mahmut \u00d6zdemir",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "410.jpg",
				"pathToThumb": "410_t.jpg"
			},
			"name": "Mahmut \u00d6zdemir",
			"party": "SPD",
			"pid": "410",
			"self_bird": "gimpel",
			"twittering": {
				"twitterId": 1399764944,
				"twitterUserName": "oezdemir_spd"
			}
		},
		"411": {
			"citizen_bird": "kleiber",
			"cv": {
				"de": "Aydan \u00d6zo\u011fuz (* 31. Mai 1967 in Hamburg) ist eine deutsche Politikerin (SPD) und seit 2009 Mitglied des Deutschen Bundestages. \u00d6zo\u011fuz ist seit Dezember 2011 eine der sechs stellvertretenden Bundesvorsitzenden der SPD. Im Juni 2013 wurde sie zur ersten Bundesvorsitzenden der SPD-Arbeitsgemeinschaft Migration und Vielfalt gew\u00e4hlt. Seit Dezember 2013 ist sie Nachfolgerin von Maria B\u00f6hmer als Staatsministerin bei der Bundeskanzlerin als Beauftragte der Bundesregierung f\u00fcr Migration, Fl\u00fcchtlinge und Integration.  Im 17. Bundestag wurde \u00d6zo\u011fuz Mitglied im Ausschuss f\u00fcr Familie, Senioren, Frauen und Jugend und Mitglied der Enquete-Kommission Internet und digitale Gesellschaft.  Nach der Bundestagswahl 2013 wurde sie im Kabinett Merkel III Nachfolgerin von Maria B\u00f6hmer als Staatsministerin bei der Bundeskanzlerin als Beauftragte der Bundesregierung f\u00fcr Migration, Fl\u00fcchtlinge und Integration.",
				"en": "Aydan \u00d6zo\u011fuz (born 31 May 1967) is a German politician. She is a member of the Bundestag for SPD, and was elected deputy chairperson of the party in 2011. Since 2013, she serves as Minister of State in the German Chancellery and Commissioner for Immigration, Refugees and Integration."
			},
			"full_name": "Aydan \u00d6zo\u011fuz",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "411.jpg",
				"pathToThumb": "411_t.jpg"
			},
			"name": "Aydan \u00d6zo\u011fuz",
			"party": "SPD",
			"pid": "411",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 83342791,
				"twitterUserName": "oezoguz"
			}
		},
		"413": {
			"citizen_bird": "tannenmeise",
			"cv": {
				"de": "Sylvia Maria Pantel (* 1. Januar 1961 in D\u00fcsseldorf) ist eine deutsche Politikerin (CDU).  Sie gewann bei der Wahl zum Deutschen Bundestag 2013 den Bundestagswahlkreis D\u00fcsseldorf II und wurde direkt in den Bundestag gew\u00e4hlt.",
				"en": "Sylvia Pantel is a German politician. She is a member of the CDU."
			},
			"full_name": "Sylvia Pantel",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "413.jpg",
				"pathToThumb": "413_t.jpg"
			},
			"name": "Sylvia Pantel",
			"party": "CDU",
			"pid": "413",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 1082464339,
				"twitterUserName": "SylviaPantel"
			}
		},
		"414": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Markus Paschke (* 8. Juni 1963 in Bremen) ist ein deutscher Politiker (SPD). Seit 2013 ist Paschke Mitglied des Deutschen Bundestages. Er zog \u00fcber die Landesliste seiner Partei in das Parlament ein.",
				"en": "Markus Paschke is a German politician. He is a member of the SPD."
			},
			"full_name": "Markus Paschke",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "414.jpg",
				"pathToThumb": "414_t.jpg"
			},
			"name": "Markus Paschke",
			"party": "SPD",
			"pid": "414",
			"self_bird": "stieglitz",
			"twittering": {
				"twitterId": 3163499392,
				"twitterUserName": "MarkusPaschke"
			}
		},
		"417": {
			"citizen_bird": "ringeltaube",
			"cv": {
				"de": "Petra Angelika Pau (* 9. August 1963 in Ost-Berlin) ist eine deutsche Politikerin (Die Linke). Seit 2006 ist sie Vizepr\u00e4sidentin des Deutschen Bundestages.  Sie ist Obfrau der Linken in den NSU-Untersuchungsaussch\u00fcssen des Bundestages (2012/13 im ersten und seit 2015 im zweiten).",
				"en": "Petra Pau (born 9 August 1963) is a member of The Left in the German parliament, the Bundestag. First elected in 1998, from 2002 to 2005 she was one of just two party representatives, having been directly elected as the representative of Berlin Marzahn \u2013 Hellersdorf. She is currently one of the vice presidents of the Bundestag and belongs to the reform-oriented members of her party, actively supporting parliamentary representative democracy."
			},
			"full_name": "Petra Pau",
			"images": {
				"license": "custom-linke",
				"pathToImage": "417.jpg",
				"pathToThumb": "417_t.jpg"
			},
			"name": "Petra Pau",
			"party": "DIE LINKE",
			"pid": "417",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": 560467206,
				"twitterUserName": "PetraPauMaHe"
			}
		},
		"418": {
			"citizen_bird": "kleiber",
			"cv": {
				"de": "Lisa Paus (* 19. September 1968 in Rheine) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen), sie zog 2009 \u00fcber die Berliner Landesliste in den Deutschen Bundestag ein.",
				"en": "Elisabeth Paus is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Lisa Paus",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "418.jpg",
				"pathToThumb": "418_t.jpg"
			},
			"name": "Lisa Paus",
			"party": "GR\u00dcNE",
			"pid": "418",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 569166756,
				"twitterUserName": "lisapaus"
			}
		},
		"419": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Christian Peter Petry (* 15. M\u00e4rz 1965 in Neunkirchen, Saarland) ist ein deutscher Politiker (SPD) und Mitglied des 18. Deutschen Bundestags.",
				"en": "Christian Petry is a German politician. He is a member of the SPD."
			},
			"full_name": "Christian Petry",
			"images": {
				"copyright": "Daniel Bittner",
				"license": "custom-spd",
				"pathToImage": "419.jpg",
				"pathToThumb": "419_t.jpg"
			},
			"name": "Christian Petry",
			"party": "SPD",
			"pid": "419",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 64159881,
				"twitterUserName": "CPetryMdB"
			}
		},
		"42": {
			"citizen_bird": "eichelhaeher",
			"cv": {
				"de": "Karin Binder (* 28. August 1957 in Stuttgart) ist eine deutsche Politikerin (Die Linke). Sie ist Mitglied des Deutschen Bundestages.",
				"en": "Karin Binder is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Karin Binder",
			"images": {
				"license": "custom-linke",
				"pathToImage": "42.jpg",
				"pathToThumb": "42_t.jpg"
			},
			"name": "Karin Binder",
			"party": "DIE LINKE",
			"pid": "42",
			"self_bird": "goldammer",
			"twittering": {
				"twitterId": 22142658,
				"twitterUserName": "karinbinder"
			}
		},
		"420": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Harald Alfred Petzold (* 28. M\u00e4rz 1962 in Seebad Heringsdorf) ist ein deutscher Politiker (Die Linke).",
				"en": "Harald Petzold is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Harald Petzold",
			"images": {
				"license": "custom-linke",
				"pathToImage": "420.jpg",
				"pathToThumb": "420_t.jpg"
			},
			"name": "Harald Petzold",
			"party": "DIE LINKE",
			"pid": "420",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": 377925526,
				"twitterUserName": "HPetzold"
			}
		},
		"422": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Joachim Pfeiffer (* 25. April 1967 in Mutlangen) ist ein deutscher Politiker (CDU). Er ist seit 2002 Mitglied des Deutschen Bundestages.",
				"en": "Joachim Pfeiffer is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Joachim Pfeiffer",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "422.jpg",
				"pathToThumb": "422_t.jpg"
			},
			"name": "Joachim Pfeiffer",
			"party": "CDU",
			"pid": "422",
			"self_bird": "goldammer",
			"twittering": {
				"twitterId": 39108206,
				"twitterUserName": "jopf"
			}
		},
		"423": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Sibylle Pfeiffer geb. Schmidt (* 16. Oktober 1951 in Wetzlar) ist eine deutsche Politikerin (CDU) und Mitglied des Deutschen Bundestages.",
				"en": "Sibylle Pfeiffer is a German politician. She is a member of the CDU."
			},
			"full_name": "Sibylle Pfeiffer",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "423.jpg",
				"pathToThumb": "423_t.jpg"
			},
			"name": "Sibylle Pfeiffer",
			"party": "CDU",
			"pid": "423",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 574528123,
				"twitterUserName": "SibyllePfeiffer"
			}
		},
		"425": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Detlev Pilger (* 29. April 1955 in Koblenz) ist ein deutscher Lehrer und Politiker der SPD. Seit dem 22. Oktober 2013 geh\u00f6rt er dem Deutschen Bundestag an.",
				"en": "Detlev Pilger is a German politician. He is a member of the SPD."
			},
			"full_name": "Detlev Pilger",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "425.jpg",
				"pathToThumb": "425_t.jpg"
			},
			"name": "Detlev Pilger",
			"party": "SPD",
			"pid": "425",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 535373865,
				"twitterUserName": "DetlevPilger"
			}
		},
		"43": {
			"citizen_bird": "gartenrotschwanz",
			"cv": {
				"de": "Lothar Binding (* 1. April 1950 in Sandershausen) ist ein deutscher Politiker (SPD).",
				"en": "Lothar Binding is a German politician. He is a member of the SPD."
			},
			"full_name": "Lothar Binding",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "43.jpg",
				"pathToThumb": "43_t.jpg"
			},
			"name": "Lothar Binding",
			"party": "SPD",
			"pid": "43",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 2617281050,
				"twitterUserName": "Lothar_Binding"
			}
		},
		"431": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Achim Post (* 2. Mai 1959 in Rahden im Kreis Minden-L\u00fcbbecke) ist ein deutscher Politiker (SPD). Er war bis Oktober 2012 stellvertretender Bundesgesch\u00e4ftsf\u00fchrer der Sozialdemokratischen Partei Deutschlands und ist seit Oktober 2012 Generalsekret\u00e4r der Sozialdemokratischen Partei Europas (SPE). Er ist \u00fcber die Landesliste im September 2013 in den 18. Bundestag gew\u00e4hlt worden.",
				"en": "Achim Post is a German politician. He is a member of the SPD."
			},
			"full_name": "Achim Post",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "431.jpg",
				"pathToThumb": "431_t.jpg"
			},
			"name": "Achim Post",
			"party": "SPD",
			"pid": "431",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 1117749912,
				"twitterUserName": "Achim_P"
			}
		},
		"432": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Florian Post (* 27. Mai 1981 in Neustadt a.d. Waldnaab (Oberpfalz)) ist Diplom-Kaufmann Univ. und Politiker (SPD).",
				"en": "Florian Post is a German politician. He is a member of the SPD."
			},
			"full_name": "Florian Post",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "432.jpg",
				"pathToThumb": "432_t.jpg"
			},
			"name": "Florian Post",
			"party": "SPD",
			"pid": "432",
			"self_bird": "star",
			"twittering": {
				"twitterId": 741532130,
				"twitterUserName": "FlorianPost"
			}
		},
		"433": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Brigitte Pothmer (* 10. Februar 1955 in Prisser) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen).",
				"en": "Brigitte Pothmer is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Brigitte Pothmer",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "433.jpg",
				"pathToThumb": "433_t.jpg"
			},
			"name": "Brigitte Pothmer",
			"party": "GR\u00dcNE",
			"pid": "433",
			"self_bird": "tannenmeise",
			"twittering": {
				"twitterId": 68394879,
				"twitterUserName": "BrigittePothmer"
			}
		},
		"434": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Wilhelm Priesmeier (* 25. Juli 1954 in Rahden) ist ein deutscher Politiker (SPD). Seit 2002 geh\u00f6rt er dem Deutschen Bundestag an.",
				"en": "Wilhelm Priesmeier is a German politician. He is a member of the SPD."
			},
			"full_name": "Dr. Wilhelm Priesmeier",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "434.jpg",
				"pathToThumb": "434_t.jpg"
			},
			"name": "Wilhelm Priesmeier",
			"party": "SPD",
			"pid": "434",
			"self_bird": "zaunkoenig",
			"twittering": {
				"twitterId": 62457691,
				"twitterUserName": "W_Priesmeier"
			}
		},
		"435": {
			"citizen_bird": "dohle",
			"cv": {
				"de": "Florian Pronold (* 28. Dezember 1972 in Passau) ist ein deutscher Politiker (SPD) und seit 2002 Mitglied des Deutschen Bundestages. Seit dem 11. Juli 2009 ist er Vorsitzender der SPD in Bayern. Seit dem 17. Dezember 2013 ist er Parlamentarischer Staatssekret\u00e4r im Bundesministerium f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit mit der Zust\u00e4ndigkeit f\u00fcr Bau und Stadtentwicklung.",
				"en": "Florian Pronold is a German politician. He is a member of the SPD."
			},
			"full_name": "Florian Pronold",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "435.jpg",
				"pathToThumb": "435_t.jpg"
			},
			"name": "Florian Pronold",
			"party": "SPD",
			"pid": "435",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 19257571,
				"twitterUserName": "FlorianPronold"
			}
		},
		"438": {
			"citizen_bird": "fitis",
			"cv": {
				"de": "Martin Hermann Rabanus (* 2. September 1971 in Fulda) ist ein deutscher Politiker der SPD, der dem deutschen Bundestag seit 2013 angeh\u00f6rt.",
				"en": "Martin Rabanus is a German politician. He is a member of the SPD."
			},
			"full_name": "Martin Rabanus",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "438.jpg",
				"pathToThumb": "438_t.jpg"
			},
			"name": "Martin Rabanus",
			"party": "SPD",
			"pid": "438",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 46356949,
				"twitterUserName": "MartinRabanus"
			}
		},
		"441": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Alexander Gamal Radwan (* 30. August 1964 in M\u00fcnchen) ist ein deutscher Politiker (CSU) und Mitglied des Deutschen Bundestages.",
				"en": "Alexander Radwan is a German politician. He is a member of the CSU."
			},
			"full_name": "Alexander Radwan",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "441.jpg",
				"pathToThumb": "441_t.jpg"
			},
			"name": "Alexander Radwan",
			"party": "CSU",
			"pid": "441",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 2831760211,
				"twitterUserName": "AlexanderRadwan"
			}
		},
		"444": {
			"citizen_bird": "star",
			"cv": {
				"de": "Mechthild Rawert (* 3. November 1957 in Coesfeld) ist eine deutsche Politikerin (SPD).",
				"en": "Mechthild Rawert is a German politician. She is a member of the SPD."
			},
			"full_name": "Mechthild Rawert",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "444.jpg",
				"pathToThumb": "444_t.jpg"
			},
			"name": "Mechthild Rawert",
			"party": "SPD",
			"pid": "444",
			"self_bird": "gartengrasmuecke",
			"twittering": {
				"twitterId": 37438947,
				"twitterUserName": "MechthildRawert"
			}
		},
		"445": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Stefan Rebmann (* 20. Juni 1962 in Heidelberg) ist ein deutscher Politiker (SPD).",
				"en": "Stefan Rebmann is a German politician. He is a member of the SPD."
			},
			"full_name": "Stefan Rebmann",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "445.jpg",
				"pathToThumb": "445_t.jpg"
			},
			"name": "Stefan Rebmann",
			"party": "SPD",
			"pid": "445",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": 313213943,
				"twitterUserName": "RebmannMdB"
			}
		},
		"446": {
			"citizen_bird": "gruenfink",
			"cv": {
				"de": "Eckhardt Harald Rehberg (* 3. April 1954 in Ribnitz-Damgarten) ist ein deutscher Politiker (CDU).",
				"en": "Eckhardt Rehberg is a German politician. He is a member of the CDU."
			},
			"full_name": "Eckhardt Rehberg",
			"images": {
				"copyright": "Andre Illing",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "446.jpg",
				"pathToThumb": "446_t.jpg"
			},
			"name": "Eckhardt Rehberg",
			"party": "CDU",
			"pid": "446",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 39485791,
				"twitterUserName": "EckhardtRehberg"
			}
		},
		"448": {
			"citizen_bird": "mauersegler",
			"cv": {
				"de": "Gerold Reichenbach (* 28. September 1953 in Geinsheim am Rhein) ist ein deutscher Politiker (SPD). Er ist seit 2002 Mitglied des Deutschen Bundestages.",
				"en": "Gerold Reichenbach is a German politician. He is a member of the SPD."
			},
			"full_name": "Gerold Reichenbach",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "448.jpg",
				"pathToThumb": "448_t.jpg"
			},
			"name": "Gerold Reichenbach",
			"party": "SPD",
			"pid": "448",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 277587918,
				"twitterUserName": "g_reichenbach"
			}
		},
		"45": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Matthias W. Birkwald (* 28. September 1961 in M\u00fcnster) ist ein deutscher Politiker (DIE LINKE). Seit Oktober 2009 ist er Mitglied des Deutschen Bundestages.",
				"en": "Matthias Birkwald is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Matthias W. Birkwald",
			"images": {
				"license": "custom-linke",
				"pathToImage": "45.jpg",
				"pathToThumb": "45_t.jpg"
			},
			"name": "Matthias W. Birkwald",
			"party": "DIE LINKE",
			"pid": "45",
			"self_bird": "tannenmeise",
			"twittering": {
				"twitterId": 748851762,
				"twitterUserName": "MWBirkwald"
			}
		},
		"450": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Martina Renner (* 11. M\u00e4rz 1967 in Mainz) ist eine deutsche Politikerin (Die Linke). Sie war seit 2009 Mitglied des Th\u00fcringer Landtags und wurde 2013 in den Deutschen Bundestag gew\u00e4hlt.",
				"en": "Martina Renner is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Martina Renner",
			"images": {
				"license": "custom-linke",
				"pathToImage": "450.jpg",
				"pathToThumb": "450_t.jpg"
			},
			"name": "Martina Renner",
			"party": "DIE LINKE",
			"pid": "450",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 68920005,
				"twitterUserName": "MartinaRenner"
			}
		},
		"455": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Andreas Rimkus (* 24. Dezember 1962 in D\u00fcsseldorf) ist ein deutscher Politiker (SPD). Seit 2011 ist er Vorsitzender des SPD-Unterbezirks D\u00fcsseldorf. Seit der Wahl zum 18. Deutschen Bundestag 2013 ist er Abgeordneter und stellvertretender, verkehrspolitischer Sprecher der SPD-Bundestagsfraktion. Zuvor war er von 2009 bis 2013 Mitglied des Rats der Landeshauptstadt D\u00fcsseldorf.",
				"en": "Andreas Rimkus is a German politician. He is a member of the SPD."
			},
			"full_name": "Andreas Rimkus",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "455.jpg",
				"pathToThumb": "455_t.jpg"
			},
			"name": "Andreas Rimkus",
			"party": "SPD",
			"pid": "455",
			"self_bird": "tannenmeise",
			"twittering": {
				"twitterId": 30838821,
				"twitterUserName": "SPDuesseldorf"
			}
		},
		"456": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "S\u00f6nke Rix (* 3. Dezember 1975 in Eckernf\u00f6rde) ist ein deutscher Politiker (SPD).",
				"en": "S\u00f6nke Rix is a German politician. He is a member of the SPD."
			},
			"full_name": "S\u00f6nke Rix",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "456.jpg",
				"pathToThumb": "456_t.jpg"
			},
			"name": "S\u00f6nke Rix",
			"party": "SPD",
			"pid": "456",
			"self_bird": "dohle",
			"twittering": {
				"twitterId": 36327895,
				"twitterUserName": "SoenkeRix"
			}
		},
		"458": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Dennis Rohde (* 24. Juni 1986 in Oldenburg) ist ein deutscher Politiker (SPD). Er ist Mitglied des 18. Deutschen Bundestages.",
				"en": "Dennis Rohde is a German politician. He is a member of the SPD."
			},
			"full_name": "Dennis Rohde",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "458.jpg",
				"pathToThumb": "458_t.jpg"
			},
			"name": "Dennis Rohde",
			"party": "SPD",
			"pid": "458",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 18160122,
				"twitterUserName": "DennisRohde"
			}
		},
		"460": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Martin Rosemann (* 20. Oktober 1976 in Saulgau) ist promovierter Volkswirt und deutscher Politiker (SPD). Seit dem Jahr 2013 ist er Bundestagsabgeordneter des Wahlkreises Hechingen-T\u00fcbingen. Dr. Martin Rosemann ist Mitglied im Ausschuss f\u00fcr Arbeit und Soziales sowie stellvertretendes Mitglied im Europaausschuss. F\u00fcr die SPD-Fraktion ist er federf\u00fchrender Berichterstatter f\u00fcr den Bereich Rentenpolitik.",
				"en": "Martin Rosemann is a German politician. He is a member of the SPD."
			},
			"full_name": "Dr. Martin Rosemann",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "460.jpg",
				"pathToThumb": "460_t.jpg"
			},
			"name": "Martin Rosemann",
			"party": "SPD",
			"pid": "460",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 28784085,
				"twitterUserName": "MartinRosemann"
			}
		},
		"461": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Peter Ren\u00e9 R\u00f6spel (* 9. Juli 1964 in Hagen) ist ein deutscher Politiker (SPD).",
				"en": "Ren\u00e9 R\u00f6spel is a German politician. He is a member of the SPD."
			},
			"full_name": "Ren\u00e9 R\u00f6spel",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "461.jpg",
				"pathToThumb": "461_t.jpg"
			},
			"name": "Ren\u00e9 R\u00f6spel",
			"party": "SPD",
			"pid": "461",
			"self_bird": "gartenrotschwanz",
			"twittering": {
				"twitterId": 213663314,
				"twitterUserName": "reneroespel"
			}
		},
		"462": {
			"citizen_bird": "saatkraehe",
			"cv": {
				"de": "Ernst Dieter Rossmann (* 4. Februar 1951 in Elmshorn) ist ein deutscher Politiker (SPD).",
				"en": "Ernst Dieter Rossmann is a German politician. He is a member of the SPD."
			},
			"full_name": "Dr. Ernst Dieter Rossmann",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "462.jpg",
				"pathToThumb": "462_t.jpg"
			},
			"name": "Ernst Dieter Rossmann",
			"party": "SPD",
			"pid": "462",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 38645930,
				"twitterUserName": "edrossmann"
			}
		},
		"463": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Tabea R\u00f6\u00dfner (* 7. Dezember 1966 in Sassenberg) ist eine rheinland-pf\u00e4lzische Politikerin von B\u00fcndnis 90/Die Gr\u00fcnen und seit 2009 Mitglied des Bundestages. Als Landesvorstandssprecherin war sie von November 2001 bis November 2006 eine der beiden Vorsitzenden des rheinland-pf\u00e4lzischen Landesverbandes. Von 1991 bis 2009 arbeitete sie als freie Redakteurin und Autorin im \u00f6ffentlich-rechtlichen wie privaten Rundfunk, unter anderem bei der Kindernachrichtensendung logo!.",
				"en": "Tabea R\u00f6\u00dfner is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Tabea R\u00f6\u00dfner",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "463.jpg",
				"pathToThumb": "463_t.jpg"
			},
			"name": "Tabea R\u00f6\u00dfner",
			"party": "GR\u00dcNE",
			"pid": "463",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 53351462,
				"twitterUserName": "TabeaRoessner"
			}
		},
		"464": {
			"citizen_bird": "haussperling",
			"cv": {
				"de": "Claudia Benedikta Roth (* 15. Mai 1955 in Ulm) ist eine deutsche Politikerin und war von 2001 bis 2002 sowie von 2004 bis 2013 eine von zwei Bundesvorsitzenden der Partei B\u00fcndnis 90/Die Gr\u00fcnen, 2008 bis 2013 neben Cem \u00d6zdemir. Seit dem 22.\u00a0Oktober 2013 ist sie Vizepr\u00e4sidentin des Deutschen Bundestages.",
				"en": "Claudia Roth is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Claudia Roth",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "464.jpg",
				"pathToThumb": "464_t.jpg"
			},
			"name": "Claudia Roth",
			"party": "GR\u00dcNE",
			"pid": "464",
			"self_bird": "goldammer",
			"twittering": {
				"twitterId": "70627400",
				"twitterUserName": "GreenClaudia"
			}
		},
		"465": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Michael Roth (* 24. August 1970 in Heringen) ist ein deutscher Politiker (SPD). Er ist seit dem 17. Dezember 2013 Staatsminister f\u00fcr Europa im Ausw\u00e4rtigen Amt der Bundesrepublik Deutschland im Kabinett Merkel III. Seit Januar 2014 ist er auch Beauftragter der Bundesregierung f\u00fcr die deutsch-franz\u00f6sische Zusammenarbeit.",
				"en": "Michael Roth is a German politician. He is a member of the SPD."
			},
			"full_name": "Michael Roth",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "465.jpg",
				"pathToThumb": "465_t.jpg"
			},
			"name": "Michael Roth",
			"party": "SPD",
			"pid": "465",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 1114675538,
				"twitterUserName": "MiRo_SPD"
			}
		},
		"467": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Erwin Josef R\u00fcddel (* 21. Dezember 1955 in Bonn) ist ein deutscher Politiker (CDU). Er ist seit 2009 Mitglied des Deutschen Bundestages.",
				"en": "Erwin R\u00fcddel is a German politician. He is a member of the CDU."
			},
			"full_name": "Erwin R\u00fcddel",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "467.jpg",
				"pathToThumb": "467_t.jpg"
			},
			"name": "Erwin R\u00fcddel",
			"party": "CDU",
			"pid": "467",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": 33071479,
				"twitterUserName": "Erwin_Rueddel"
			}
		},
		"468": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Corinna R\u00fcffer (* 11. Oktober 1975 in Osnabr\u00fcck) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie ist seit Oktober 2013 Mitglied des Deutschen Bundestages.",
				"en": "Corinna R\u00fcffer is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Corinna R\u00fcffer",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "468.jpg",
				"pathToThumb": "468_t.jpg"
			},
			"name": "Corinna R\u00fcffer",
			"party": "GR\u00dcNE",
			"pid": "468",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": 1524735194,
				"twitterUserName": "crueffer"
			}
		},
		"470": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Susann R\u00fcthrich MdB, (* 21. Juli 1977 in Mei\u00dfen) ist eine deutsche Politikerin (SPD).",
				"en": "Susann R\u00fcthrich is a German politician. She is a member of the SPD."
			},
			"full_name": "Susann R\u00fcthrich",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "470.jpg",
				"pathToThumb": "470_t.jpg"
			},
			"name": "Susann R\u00fcthrich",
			"party": "SPD",
			"pid": "470",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 1461997548,
				"twitterUserName": "SusannRuethrich"
			}
		},
		"474": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Manuel Sarrazin (* 6. Februar 1982 in Dortmund) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen) und seit 2008 Mitglied des Deutschen Bundestages.",
				"en": "Manuel Sarrazin is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Manuel Sarrazin",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "474.jpg",
				"pathToThumb": "474_t.jpg"
			},
			"name": "Manuel Sarrazin",
			"party": "GR\u00dcNE",
			"pid": "474",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 17922403,
				"twitterUserName": "ManuelSarrazin"
			}
		},
		"475": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Annette Sawade (* 23. April 1953 in Nordhausen) ist eine deutsche Politikerin (SPD) und seit 2012 Mitglied des Deutschen Bundestages.",
				"en": "Annette Sawade is a German politician. She is a member of the SPD."
			},
			"full_name": "Annette Sawade",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "475.jpg",
				"pathToThumb": "475_t.jpg"
			},
			"name": "Annette Sawade",
			"party": "SPD",
			"pid": "475",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 1192966249,
				"twitterUserName": "AnnetteSawade"
			}
		},
		"481": {
			"citizen_bird": "weisskopfseeadler",
			"cv": {
				"de": "Ulle Schauws (* 30. April 1966 in H\u00fcls) ist eine deutsche Politikerin der B\u00fcndnis 90/Die Gr\u00fcnen.",
				"en": "Ursula Schauws is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Ulle Schauws",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "481.jpg",
				"pathToThumb": "481_t.jpg"
			},
			"name": "Ulle Schauws",
			"party": "GR\u00dcNE",
			"pid": "481",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": 17363943,
				"twitterUserName": "ulle_schauws"
			}
		},
		"484": {
			"citizen_bird": "bachstelze",
			"cv": {
				"de": "Andreas Franz Scheuer (* 26. September 1974 in Passau) ist ein deutscher Politiker (CSU) und seit 2002 Mitglied des Deutschen Bundestages. Er war im Kabinett Merkel II von 2009 bis 2013 Parlamentarischer Staatssekret\u00e4r beim Bundesminister f\u00fcr Verkehr, Bau und Stadtentwicklung. Seit dem 15. Dezember 2013 ist er Generalsekret\u00e4r der CSU.",
				"en": "Andreas Scheuer is a German politician. He is a member of the CSU."
			},
			"full_name": "Andreas Scheuer",
			"images": {
				"copyright": "Freud",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "484.jpg",
				"pathToThumb": "484_t.jpg"
			},
			"name": "Andreas Scheuer",
			"party": "CSU",
			"pid": "484",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 50315139,
				"twitterUserName": "AndiScheuer"
			}
		},
		"485": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Gerhard Schick, MdB, (* 18. April 1972 in Hechingen) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen).",
				"en": "Gerhard Schick is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Dr. Gerhard Schick",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "485.jpg",
				"pathToThumb": "485_t.jpg"
			},
			"name": "Gerhard Schick",
			"party": "GR\u00dcNE",
			"pid": "485",
			"self_bird": "mehlschwalbe",
			"twittering": {
				"twitterId": 4110374301,
				"twitterUserName": "SchickGerhard"
			}
		},
		"487": {
			"citizen_bird": "gartenrotschwanz",
			"cv": {
				"de": "Udo Schiefner (* 7. August 1959 in Kempen, Niederrhein) ist ein deutscher Politiker der SPD. Seit dem 22. Oktober 2013 geh\u00f6rt er dem Deutschen Bundestag an.",
				"en": "Udo Schiefner is a German politician. He is a member of the SPD."
			},
			"full_name": "Udo Schiefner",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "487.jpg",
				"pathToThumb": "487_t.jpg"
			},
			"name": "Udo Schiefner",
			"party": "SPD",
			"pid": "487",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": "3823042877",
				"twitterUserName": "UdoSchiefner"
			}
		},
		"489": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Jana Schimke (* 6. September 1979 in Cottbus) ist eine deutsche Politikerin (CDU).",
				"en": "Jana Schimke is a German politician. She is a member of the CDU."
			},
			"full_name": "Jana Schimke",
			"images": {
				"copyright": "Karoline Wolf",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "489.jpg",
				"pathToThumb": "489_t.jpg"
			},
			"name": "Jana Schimke",
			"party": "CDU",
			"pid": "489",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": 728990858,
				"twitterUserName": "JanaSchimke"
			}
		},
		"49": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Maria B\u00f6hmer (* 23. April 1950 in Mainz) ist eine deutsche Politikerin (CDU). Vom November 2005 bis zum Dezember 2013 war sie Staatsministerin bei der Bundeskanzlerin und Beauftragte der Bundesregierung f\u00fcr Migration, Fl\u00fcchtlinge und Integration. Ihre Nachfolgerin in dieser Funktion ist Aydan \u00d6zo\u011fuz. Im Kabinett Merkel\u00a0III ist sie seit Dezember 2013 Staatsministerin im Ausw\u00e4rtigen Amt und f\u00fcr Ausw\u00e4rtige Kultur- und Bildungspolitik zust\u00e4ndig.",
				"en": "Maria B\u00f6hmer is a German politician. She is a member of the CDU."
			},
			"full_name": "Prof. Dr. Maria B\u00f6hmer",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "49.jpg",
				"pathToThumb": "49_t.jpg"
			},
			"name": "Maria B\u00f6hmer",
			"party": "CDU",
			"pid": "49",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": "2898896728",
				"twitterUserName": "MariaBoehmer"
			}
		},
		"491": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Tankred Schipanski (* 30. Dezember 1976 in Leipzig) ist ein deutscher Politiker (CDU). Seit der Bundestagswahl 2009 ist er direkt gew\u00e4hlter Abgeordneter des Bundestagswahlkreises 192 (Gotha \u2013 Ilm-Kreis).",
				"en": "Tankred Schipanski is a German politician. He is a member of the CDU."
			},
			"full_name": "Tankred Schipanski",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "491.jpg",
				"pathToThumb": "491_t.jpg"
			},
			"name": "Tankred Schipanski",
			"party": "CDU",
			"pid": "491",
			"self_bird": "buchfink",
			"twittering": {
				"twitterId": 39938182,
				"twitterUserName": "TSchipanski"
			}
		},
		"492": {
			"citizen_bird": "eichelhaeher",
			"cv": {
				"de": "Michael Schlecht (* 25. Juni 1951 in Hildesheim) ist ein deutscher Politiker (Die Linke).",
				"en": "Michael Schlecht is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Michael Schlecht",
			"images": {
				"license": "custom-linke",
				"pathToImage": "492.jpg",
				"pathToThumb": "492_t.jpg"
			},
			"name": "Michael Schlecht",
			"party": "DIE LINKE",
			"pid": "492",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 151173601,
				"twitterUserName": "mschlecht"
			}
		},
		"496": {
			"citizen_bird": "gimpel",
			"cv": {
				"de": "Dagmar Schmidt (* 13. M\u00e4rz 1973 in Gie\u00dfen) ist eine deutsche Historikerin und Politikerin (SPD).",
				"en": "Dagmar Schmidt is a German politician. She is a member of the SPD."
			},
			"full_name": "Dagmar Schmidt",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "496.jpg",
				"pathToThumb": "496_t.jpg"
			},
			"name": "Dagmar Schmidt",
			"party": "SPD",
			"pid": "496",
			"self_bird": "kiwi",
			"twittering": {
				"twitterId": 95905207,
				"twitterUserName": "dieschmidt"
			}
		},
		"5": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Luise Amtsberg (* 17. Oktober 1984 in Greifswald, geborene von Jackowski) ist eine deutsche Politikerin von B\u00fcndnis 90/Die Gr\u00fcnen und seit der Bundestagswahl 2013 Abgeordnete des Deutschen Bundestages.",
				"en": "Luise Amtsberg is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Luise Amtsberg",
			"images": {
				"copyright": "SoenkeWe",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "5.jpg",
				"pathToThumb": "5_t.jpg"
			},
			"name": "Luise Amtsberg",
			"party": "GR\u00dcNE",
			"pid": "5",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 126058461,
				"twitterUserName": "Luise_Amtsberg"
			}
		},
		"501": {
			"citizen_bird": "goldammer",
			"cv": {
				"de": "Ronja Schmitt (* 3. Mai 1989 in Esslingen am Neckar) ist eine deutsche Politikerin der CDU und seit Dezember 2014 Mitglied des Deutschen Bundestages.",
				"en": "Ronja Schmitt is a German politician. She is a member of the CDU."
			},
			"full_name": "Ronja Kemmer",
			"images": {
				"copyright": "ToKo",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "501.jpg",
				"pathToThumb": "501_t.jpg"
			},
			"name": "Ronja Kemmer",
			"party": "CDU",
			"pid": "501",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 2405039875,
				"twitterUserName": "RonjaSchmitt"
			}
		},
		"502": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Carsten Schneider (* 23. Januar 1976 in Erfurt) ist ein deutscher Politiker (SPD). Er ist seit 1998 Mitglied des Deutschen Bundestages und war von 2005 bis 2013 (16. Bundestag und 17. Bundestag) haushaltspolitischer Sprecher der SPD-Bundestagsfraktion. Seit dem 19. Dezember 2013 ist er einer der sieben stellvertretenden Vorsitzenden der SPD-Bundestagsfraktion (Liste hier).",
				"en": "Carsten Schneider is a German politician. He is a member of the SPD."
			},
			"full_name": "Carsten Schneider",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "502.jpg",
				"pathToThumb": "502_t.jpg"
			},
			"name": "Carsten Schneider",
			"party": "SPD",
			"pid": "502",
			"self_bird": "haussperling",
			"twittering": {
				"twitterId": 96783215,
				"twitterUserName": "schneidercar"
			}
		},
		"503": {
			"citizen_bird": "moenchsgrasmuecke",
			"cv": {
				"de": "Patrick Schnieder (* 1. Mai 1968 in Kyllburg) ist deutscher Politiker (CDU) und seit 2009 Mitglied des Deutschen Bundestages. Schnieder war von 1999 bis 2009 B\u00fcrgermeister in der Verbandsgemeinde Arzfeld, bevor er in den Bundestag wechselte. Er hat an der Rheinischen Friedrich-Wilhelms-Universit\u00e4t in Bonn Rechtswissenschaften studiert und ist Volljurist.",
				"en": "Patrick Schnieder is a German politician. He is a member of the CDU."
			},
			"full_name": "Patrick Schnieder",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "503.jpg",
				"pathToThumb": "503_t.jpg"
			},
			"name": "Patrick Schnieder",
			"party": "CDU",
			"pid": "503",
			"self_bird": "schwanzmeise",
			"twittering": {
				"twitterId": 368334555,
				"twitterUserName": "PSchnieder"
			}
		},
		"506": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Nadine Sch\u00f6n (* 5. Juni 1983 in Lebach als Nadine M\u00fcller) ist eine deutsche Politikerin (CDU).",
				"en": "Nadine Sch\u00f6n is a German politician. She is a member of the CDU."
			},
			"full_name": "Nadine Sch\u00f6n",
			"images": {
				"copyright": "Margarete Singer",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "506.jpg",
				"pathToThumb": "506_t.jpg"
			},
			"name": "Nadine Sch\u00f6n",
			"party": "CDU",
			"pid": "506",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 41840905,
				"twitterUserName": "NadineSchoen"
			}
		},
		"507": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Kristina Schr\u00f6der (geb. K\u00f6hler; * 3. August 1977 in Wiesbaden) ist eine deutsche Politikerin (CDU). Vom 30. November 2009 bis Mitte Dezember 2013 war sie Bundesministerin f\u00fcr Familie, Senioren, Frauen und Jugend.",
				"en": "Kristina Schr\u00f6der is a German politician. She is a member of the CDU."
			},
			"full_name": "Dr. Kristina Schr\u00f6der",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "507.jpg",
				"pathToThumb": "507_t.jpg"
			},
			"name": "Kristina Schr\u00f6der",
			"party": "CDU",
			"pid": "507",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": 18761526,
				"twitterUserName": "schroeder_k"
			}
		},
		"511": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Swen Schulz (* 1. M\u00e4rz 1968 in Hamburg) ist ein deutscher Politiker (SPD).",
				"en": "Swen Schulz is a German politician. He is a member of the SPD."
			},
			"full_name": "Swen Schulz",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "511.jpg",
				"pathToThumb": "511_t.jpg"
			},
			"name": "Swen Schulz",
			"party": "SPD",
			"pid": "511",
			"self_bird": "goldammer",
			"twittering": {
				"twitterId": 21791639,
				"twitterUserName": "swenschulz"
			}
		},
		"512": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Kordula Anna Paula Schulz-Asche (* 31. Dezember 1956 in Berlin-Spandau) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie ist Mitglied des Deutschen Bundestages.",
				"en": "Kordula Schulz-Asche is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Kordula Schulz-Asche",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "512.jpg",
				"pathToThumb": "512_t.jpg"
			},
			"name": "Kordula Schulz-Asche",
			"party": "GR\u00dcNE",
			"pid": "512",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 17870195,
				"twitterUserName": "K_SA"
			}
		},
		"514": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Uwe Schummer (* 14. November 1957 in Adelaide, Australien) ist ein deutscher Politiker (CDU) aus Willich. Seit 2014 ist er Beauftragter der CDU/CSU-Bundestagsfraktion f\u00fcr Menschen mit Behinderungen.",
				"en": "Uwe Schummer is a German politician. He is a member of the CDU."
			},
			"full_name": "Uwe Schummer",
			"images": {
				"copyright": "Swen Siewert",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "514.jpg",
				"pathToThumb": "514_t.jpg"
			},
			"name": "Uwe Schummer",
			"party": "CDU",
			"pid": "514",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 409203792,
				"twitterUserName": "UweSchummer"
			}
		},
		"516": {
			"citizen_bird": "fitis",
			"cv": {
				"de": "Armin Schuster (* 20. Mai 1961 in Andernach) ist ein deutscher Politiker (CDU). Er ist seit 2009 Mitglied des Deutschen Bundestages.",
				"en": "Armin Schuster is a German politician. He is a member of the CDU."
			},
			"full_name": "Armin Schuster",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "516.jpg",
				"pathToThumb": "516_t.jpg"
			},
			"name": "Armin Schuster",
			"party": "CDU",
			"pid": "516",
			"self_bird": "weisskopfseeadler",
			"twittering": {
				"twitterId": 395912134,
				"twitterUserName": "armin_schuster"
			}
		},
		"517": {
			"citizen_bird": "gruenfink",
			"cv": {
				"de": "Frank Schwabe (* 12. November 1970 in Waltrop) ist ein deutscher Politiker (SPD) und Mitglied des Deutschen Bundestages.",
				"en": "Frank Schwabe is a German politician. He is a member of the SPD."
			},
			"full_name": "Frank Schwabe",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "517.jpg",
				"pathToThumb": "517_t.jpg"
			},
			"name": "Frank Schwabe",
			"party": "SPD",
			"pid": "517",
			"self_bird": "zaunkoenig",
			"twittering": {
				"twitterId": 30050484,
				"twitterUserName": "FrankSchwabe"
			}
		},
		"518": {
			"citizen_bird": "fitis",
			"cv": {
				"de": "Stefan Schwartze (* 23. Mai 1974 in Bad Oeynhausen) ist ein deutscher Politiker (SPD) und Bundestagsabgeordneter im 18. Deutschen Bundestag.",
				"en": "Stefan Schwartze is a German politician. He is a member of the SPD."
			},
			"full_name": "Stefan Schwartze",
			"images": {
				"copyright": "Annette Koroll Fotos",
				"license": "custom-spd",
				"pathToImage": "518.jpg",
				"pathToThumb": "518_t.jpg"
			},
			"name": "Stefan Schwartze",
			"party": "SPD",
			"pid": "518",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 1117526742,
				"twitterUserName": "s_schwartze"
			}
		},
		"519": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Andreas Schwarz (* 3. M\u00e4rz 1965 in Berleburg) ist ein deutscher Politiker der SPD und seit 2013 Mitglied des Deutschen Bundestages.",
				"en": "Andreas Schwarz is a German politician. He is a member of the SPD."
			},
			"full_name": "Andreas Schwarz",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "519.jpg",
				"pathToThumb": "519_t.jpg"
			},
			"name": "Andreas Schwarz",
			"party": "SPD",
			"pid": "519",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": 2226815116,
				"twitterUserName": "Schwarz_MdB"
			}
		},
		"520": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Rita Schwarzel\u00fchr-Sutter MdB, (* 13. Oktober 1962 in Waldshut) ist eine deutsche Wirtschaftswissenschaftlerin und Politikerin (SPD). Seit dem 17. Dezember 2013 ist sie Parlamentarische Staatssekret\u00e4rin im Kabinett Merkel III bei der Bundesministerin f\u00fcr Umwelt, Naturschutz, Bau und Reaktorsicherheit.",
				"en": "Rita Schwarzel\u00fchr-Sutter is a German politician. She is a member of the SPD."
			},
			"full_name": "Rita Schwarzel\u00fchr-Sutter",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "520.jpg",
				"pathToThumb": "520_t.jpg"
			},
			"name": "Rita Schwarzel\u00fchr-Sutter",
			"party": "SPD",
			"pid": "520",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 50932086,
				"twitterUserName": "rischwasu"
			}
		},
		"521": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Christina Schwarzer (* 12. September 1976 in Berlin) ist eine deutsche Politikerin (CDU).",
				"en": "Christina Schwarzer is a German politician. She is a member of the CDU."
			},
			"full_name": "Christina Schwarzer",
			"images": {
				"copyright": "Tobias Koch/www.tobiaskoch.net",
				"license": "CC-BY-3.0 unported",
				"pathToImage": "521.jpg",
				"pathToThumb": "521_t.jpg"
			},
			"name": "Christina Schwarzer",
			"party": "CDU",
			"pid": "521",
			"self_bird": "saatkraehe",
			"twittering": {
				"twitterId": 359327259,
				"twitterUserName": "TinaSchwarzer"
			}
		},
		"525": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Patrick Ernst Sensburg (* 25. Juni 1971 in Paderborn) ist ein deutscher Bundestagsabgeordneter und Professor an der Fachhochschule f\u00fcr \u00f6ffentliche Verwaltung Nordrhein-Westfalen in M\u00fcnster.",
				"en": "Patrick Ernst Sensburg is a German politician. He is a member of the CDU."
			},
			"full_name": "Prof. Dr. Patrick Sensburg",
			"images": {
				"copyright": "MK Hoffe",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "525.jpg",
				"pathToThumb": "525_t.jpg"
			},
			"name": "Patrick Sensburg",
			"party": "CDU",
			"pid": "525",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 43537931,
				"twitterUserName": "PatrickSensburg"
			}
		},
		"526": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Bernd Siebert (* 17. Oktober 1949 in Gudensberg) ist ein deutscher Politiker (CDU).",
				"en": "Bernd Siebert is a German politician. He is a member of the CDU."
			},
			"full_name": "Bernd Siebert",
			"images": {
				"copyright": "Florian Punck",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "526.jpg",
				"pathToThumb": "526_t.jpg"
			},
			"name": "Bernd Siebert",
			"party": "CDU",
			"pid": "526",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 24674518,
				"twitterUserName": "BerndSiebert"
			}
		},
		"529": {
			"citizen_bird": "weisskopfseeadler",
			"cv": {
				"de": "Johannes Singhammer (* 9. Mai 1953 in M\u00fcnchen) ist ein deutscher Politiker (CSU) und seit 22. Oktober 2013 Vizepr\u00e4sident des Deutschen Bundestages.",
				"en": "Johannes Singhammer is a German politician. He is a member of the CSU."
			},
			"full_name": "Johannes Singhammer",
			"images": {
				"copyright": "Henning Schacht",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "529.jpg",
				"pathToThumb": "529_t.jpg"
			},
			"name": "Johannes Singhammer",
			"party": "CSU",
			"pid": "529",
			"self_bird": "buchfink",
			"twittering": {
				"twitterId": 542415865,
				"twitterUserName": "JohannesSingham"
			}
		},
		"530": {
			"citizen_bird": "tannenmeise",
			"cv": {
				"de": "Petra Sitte (* 1. Dezember 1960 in Dresden) ist eine deutsche Politikerin (Die Linke). Sie ist seit 2013 erste parlamentarische Gesch\u00e4ftsf\u00fchrerin der Linksfraktion im Deutschen Bundestag.",
				"en": "Petra Sitte is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Dr. Petra Sitte",
			"images": {
				"license": "custom-linke",
				"pathToImage": "530.jpg",
				"pathToThumb": "530_t.jpg"
			},
			"name": "Petra Sitte",
			"party": "DIE LINKE",
			"pid": "530",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 293522998,
				"twitterUserName": "Petra_Sitte_MdB"
			}
		},
		"531": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Tino Sorge (* 4. M\u00e4rz 1975 in Ilmenau) ist ein deutscher Politiker (CDU).",
				"en": "Tino Sorge is a German politician. He is a member of the CDU."
			},
			"full_name": "Tino Sorge",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "531.jpg",
				"pathToThumb": "531_t.jpg"
			},
			"name": "Tino Sorge",
			"party": "CDU",
			"pid": "531",
			"self_bird": "eichelhaeher",
			"twittering": {
				"twitterId": 110385973,
				"twitterUserName": "TinoSorge"
			}
		},
		"532": {
			"citizen_bird": "kleiber",
			"cv": {
				"de": "Jens Spahn (* 16. Mai 1980 in Ahaus) ist ein deutscher Politiker (CDU). Seit dem 3. Juli 2015 ist er Parlamentarischer Staatssekret\u00e4r beim Bundesminister der Finanzen.",
				"en": "Jens Spahn is a German politician. He is a member of the CDU."
			},
			"full_name": "Jens Spahn",
			"images": {
				"copyright": "Stephan Baumann",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "532.jpg",
				"pathToThumb": "532_t.jpg"
			},
			"name": "Jens Spahn",
			"party": "CDU",
			"pid": "532",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 299650387,
				"twitterUserName": "jensspahn"
			}
		},
		"538": {
			"citizen_bird": "stieglitz",
			"cv": {
				"de": "Frank Steffel (* 2. M\u00e4rz 1966 in Berlin) ist ein deutscher Politiker (CDU) und Unternehmer. Der Diplom-Kaufmann war von Mai 2001 bis Mai 2003 Fraktionsvorsitzender der CDU im Abgeordnetenhaus von Berlin. Seit dem 27. September 2009 ist Steffel Mitglied des Deutschen Bundestages.",
				"en": "Frank Steffel is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Frank Steffel",
			"images": {
				"copyright": "Tobias Schult",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "538.jpg",
				"pathToThumb": "538_t.jpg"
			},
			"name": "Frank Steffel",
			"party": "CDU",
			"pid": "538",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 26869088,
				"twitterUserName": "franksteffel"
			}
		},
		"540": {
			"citizen_bird": "haussperling",
			"cv": {
				"de": "Wolfgang Stefinger (* 20. April 1985 in M\u00fcnchen) ist ein deutscher Politiker (CSU). Seit dem 22. September 2013 ist er Mitglied des Deutschen Bundestages und vertritt den Bundestagswahlkreis M\u00fcnchen-Ost.",
				"en": "Wolfgang Stefinger is a German politician. He is a member of the CSU."
			},
			"full_name": "Dr. Wolfgang Stefinger",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "540.jpg",
				"pathToThumb": "540_t.jpg"
			},
			"name": "Wolfgang Stefinger",
			"party": "CSU",
			"pid": "540",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": "4866955749",
				"twitterUserName": "StefingerMdB"
			}
		},
		"544": {
			"citizen_bird": "gimpel",
			"cv": {
				"de": "Peer Steinbr\u00fcck (* 10. Januar 1947 in Hamburg) ist ein deutscher Politiker und Mitglied der SPD. Von 2002 bis 2005 war Steinbr\u00fcck Ministerpr\u00e4sident des Landes Nordrhein-Westfalen, von 2005 bis 2009 Bundesminister der Finanzen und stellvertretender Bundesvorsitzender der SPD. Seit 2009 ist er Mitglied des Deutschen Bundestages. Bei der Bundestagswahl 2013 unterlag er als SPD-Kanzlerkandidat der Amtsinhaberin Angela Merkel. Von M\u00e4rz bis Juni 2015 war er nebenberuflich als Berater f\u00fcr die Agentur zur Modernisierung der Ukraine t\u00e4tig.",
				"en": "Peer Steinbr\u00fcck is a German politician. He is a member of the SPD."
			},
			"full_name": "Peer Steinbr\u00fcck",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "544.jpg",
				"pathToThumb": "544_t.jpg"
			},
			"name": "Peer Steinbr\u00fcck",
			"party": "SPD",
			"pid": "544",
			"self_bird": "gartengrasmuecke",
			"twittering": {
				"twitterId": 856641566,
				"twitterUserName": "peersteinbrueck"
			}
		},
		"545": {
			"citizen_bird": "eichelhaeher",
			"cv": {
				"de": "Sebastian Steineke (* 19. Juni 1973 in Hamburg) ist ein deutscher Politiker (CDU). Er wohnt mit seiner Familie in Neuruppin.",
				"en": "Sebastian Steineke is a German politician. He is a member of the CDU."
			},
			"full_name": "Sebastian Steineke",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "545.jpg",
				"pathToThumb": "545_t.jpg"
			},
			"name": "Sebastian Steineke",
			"party": "CDU",
			"pid": "545",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 277006619,
				"twitterUserName": "SteinekeCDU"
			}
		},
		"546": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Johannes Eberhard Steiniger (* 18. Juni 1987 in Bad D\u00fcrkheim) ist ein deutscher Politiker der CDU und seit der Bundestagswahl 2013 Mitglied des Bundestags.",
				"en": "Johannes Steiniger is a German politician. He is a member of the CDU."
			},
			"full_name": "Johannes Steiniger",
			"images": {
				"copyright": "Thomas Wierer",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "546.jpg",
				"pathToThumb": "546_t.jpg"
			},
			"name": "Johannes Steiniger",
			"party": "CDU",
			"pid": "546",
			"self_bird": "buchfink",
			"twittering": {
				"twitterId": 83474160,
				"twitterUserName": "JoSteiniger"
			}
		},
		"549": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Christian Freiherr von Stetten MdB, (* 24. Juli 1970 in Stuttgart) ist ein deutscher Unternehmer und Politiker (CDU).",
				"en": "Christian von Stetten is a German politician. He is a member of the CDU."
			},
			"full_name": "Christian Frhr. von Stetten",
			"images": {
				"copyright": "Prell",
				"license": "public domain",
				"pathToImage": "549.jpg",
				"pathToThumb": "549_t.jpg"
			},
			"name": "Christian Frhr. von Stetten",
			"party": "CDU",
			"pid": "549",
			"self_bird": "buchfink",
			"twittering": {
				"twitterId": 1334019378,
				"twitterUserName": "Stettenchris"
			}
		},
		"554": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Christoph Str\u00e4sser (* 12. Juni 1949 in Velbert) ist ein deutscher Politiker (SPD). Seit Januar 2014 ist er Beauftragter der Bundesregierung f\u00fcr Menschenrechtspolitik und Humanit\u00e4re Hilfe. Er war von 2005 bis 2014 Sprecher der Arbeitsgruppe Menschenrechte und humanit\u00e4re Hilfe der SPD-Bundestagsfraktion.",
				"en": "Christoph Str\u00e4sser is a German politician. He is a member of the SPD."
			},
			"full_name": "Christoph Str\u00e4sser",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "554.jpg",
				"pathToThumb": "554_t.jpg"
			},
			"name": "Christoph Str\u00e4sser",
			"party": "SPD",
			"pid": "554",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": 1509832074,
				"twitterUserName": "ChStraesser"
			}
		},
		"557": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Wolfgang Strengmann-Kuhn (* 20. Mai 1964 in Dinslaken) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). Er ist Wirtschaftswissenschaftler und Mitglied des Deutschen Bundestages.",
				"en": "Wolfgang Strengmann-Kuhn is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Dr. Wolfgang Strengmann-Kuhn",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "557.jpg",
				"pathToThumb": "557_t.jpg"
			},
			"name": "Wolfgang Strengmann-Kuhn",
			"party": "GR\u00dcNE",
			"pid": "557",
			"self_bird": "tuerkentaube",
			"twittering": {
				"twitterId": 20840049,
				"twitterUserName": "W_SK"
			}
		},
		"56": {
			"citizen_bird": "tuerkentaube",
			"cv": {
				"de": "Franziska Katharina Brantner (* 24. August 1979 in L\u00f6rrach) ist eine deutsche Politikwissenschaftlerin und Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie geh\u00f6rt dem 18. Deutschen Bundestag an.",
				"en": "Franziska Brantner is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Dr. Franziska Brantner",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "56.jpg",
				"pathToThumb": "56_t.jpg"
			},
			"name": "Franziska Brantner",
			"party": "GR\u00dcNE",
			"pid": "56",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 32323724,
				"twitterUserName": "fbrantner"
			}
		},
		"560": {
			"citizen_bird": "elster",
			"cv": {
				"de": "Hans-Christian Str\u00f6bele (* 7. Juni 1939 in Halle an der Saale) ist ein deutscher Rechtsanwalt und Politiker der Partei B\u00fcndnis 90/Die Gr\u00fcnen. Er war von 2002 bis 2009 stellvertretender Vorsitzender der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen und ist das dienst\u00e4lteste Mitglied des Parlamentarischen Kontrollgremiums (PKG) des Deutschen Bundestages zur Kontrolle der Geheimdienste.",
				"en": "Hans-Christian Str\u00f6bele is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Hans-Christian Str\u00f6bele",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "560.jpg",
				"pathToThumb": "560_t.jpg"
			},
			"name": "Hans-Christian Str\u00f6bele",
			"party": "GR\u00dcNE",
			"pid": "560",
			"self_bird": "gruenfink",
			"twittering": {
				"twitterId": 304342650,
				"twitterUserName": "MdB_Stroebele"
			}
		},
		"565": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Kerstin Tack (* 11. November 1968 in Wittingen) ist eine deutsche Politikerin (SPD). Sie ist seit der Bundestagswahl 2009 Mitglied des Deutschen Bundestages.",
				"en": "Kerstin Tack is a German politician. She is a member of the SPD."
			},
			"full_name": "Kerstin Tack",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "565.jpg",
				"pathToThumb": "565_t.jpg"
			},
			"name": "Kerstin Tack",
			"party": "SPD",
			"pid": "565",
			"self_bird": "dohle",
			"twittering": {
				"twitterId": 37733252,
				"twitterUserName": "kerstin_tack"
			}
		},
		"566": {
			"citizen_bird": "gruenfink",
			"cv": {
				"de": "Kirsten Tackmann geb. Kohl (* 24. September 1960 in Schmalkalden, DDR) ist eine deutsche Politikerin (Die Linke). Sie ist seit 2005 Mitglied des Deutschen Bundestages.",
				"en": "Kirsten Tackmann is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Dr. Kirsten Tackmann",
			"images": {
				"license": "custom-linke",
				"pathToImage": "566.jpg",
				"pathToThumb": "566_t.jpg"
			},
			"name": "Kirsten Tackmann",
			"party": "DIE LINKE",
			"pid": "566",
			"self_bird": "rabenkraehe",
			"twittering": {
				"twitterId": 1294725486,
				"twitterUserName": "KirstenTackmann"
			}
		},
		"568": {
			"citizen_bird": "heckenbraunelle",
			"cv": {
				"de": "Peter Michael Tauber (* 22. August 1974 in Frankfurt am Main) ist ein deutscher christdemokratischer Politiker (CDU). Er ist Bundestagsabgeordneter und seit dem 16. Dezember 2013 Generalsekret\u00e4r der CDU.",
				"en": "Peter Tauber is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Peter Tauber",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "568.jpg",
				"pathToThumb": "568_t.jpg"
			},
			"name": "Peter Tauber",
			"party": "CDU",
			"pid": "568",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 22260144,
				"twitterUserName": "petertauber"
			}
		},
		"570": {
			"citizen_bird": "buntspecht",
			"cv": {
				"de": "Frank Tempel (* 19. Januar 1969 in Belzig, damals Bezirk Potsdam, DDR) ist ein deutscher Politiker (Die Linke) und seit 2009 Mitglied des Deutschen Bundestages. Seit Mai 2010 ist er Drogenpolitischer Sprecher der Bundestagsfraktion. Im Januar 2014 wurde Frank Tempel ohne Gegenstimmen zum stellvertretenden Vorsitzenden des Innenausschusses des Bundestages gew\u00e4hlt. Er ist der erste Politiker der Partei Die Linke, der diese Posten bekleidet.",
				"en": "Frank Tempel is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Frank Tempel",
			"images": {
				"license": "custom-linke",
				"pathToImage": "570.jpg",
				"pathToThumb": "570_t.jpg"
			},
			"name": "Frank Tempel",
			"party": "DIE LINKE",
			"pid": "570",
			"self_bird": "buntspecht",
			"twittering": {
				"twitterId": "730695836288585728",
				"twitterUserName": "frank_tempel"
			}
		},
		"571": {
			"citizen_bird": "rotkehlchen",
			"cv": {
				"de": "Harald Terpe, (* 17. Juli 1954 in Greifswald, DDR; vollst\u00e4ndiger Name Frank Harald Terpe) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen, bis November 2006 parteilos).",
				"en": "Harald Terpe is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Dr. Harald Terpe",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "571.jpg",
				"pathToThumb": "571_t.jpg"
			},
			"name": "Harald Terpe",
			"party": "GR\u00dcNE",
			"pid": "571",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 40451562,
				"twitterUserName": "terpeundteam"
			}
		},
		"572": {
			"citizen_bird": "haussperling",
			"cv": {
				"de": "Michael Thews (* 6. September 1964 in Bremerhaven) ist ein deutscher Politiker (SPD) und Mitglied des Bundestages. Er ist direkt gew\u00e4hlter Abgeordneter f\u00fcr den Wahlkreis Hamm \u2013 Unna II als Nachfolger von Dieter Wiefelsp\u00fctz, der nicht mehr antrat.",
				"en": "Michael Thews is a German politician. He is a member of the SPD."
			},
			"full_name": "Michael Thews",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "572.jpg",
				"pathToThumb": "572_t.jpg"
			},
			"name": "Michael Thews",
			"party": "SPD",
			"pid": "572",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 409258073,
				"twitterUserName": "michael_thews"
			}
		},
		"574": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Franz Th\u00f6nnes (* 16. September 1954 in Essen) ist ein deutscher Politiker (SPD). Er ist seit 1994 Mitglied des Deutschen Bundestages. Von 2002 bis 2005 war er Parlamentarischer Staatssekret\u00e4r bei der Bundesministerin f\u00fcr Gesundheit und Soziale Sicherung, danach bis November 2009 Parlamentarischer Staatssekret\u00e4r beim Bundesminister f\u00fcr Arbeit und Soziales.",
				"en": "Franz Th\u00f6nnes is a German politician. He is a member of the SPD."
			},
			"full_name": "Franz Th\u00f6nnes",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "574.jpg",
				"pathToThumb": "574_t.jpg"
			},
			"name": "Franz Th\u00f6nnes",
			"party": "SPD",
			"pid": "574",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 36048137,
				"twitterUserName": "franzthoennes"
			}
		},
		"579": {
			"citizen_bird": "fitis",
			"cv": {
				"de": "Markus Tressel (* 17. April 1977 in Saarlouis) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen).",
				"en": "Markus Tressel is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Markus Tressel",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "579.jpg",
				"pathToThumb": "579_t.jpg"
			},
			"name": "Markus Tressel",
			"party": "GR\u00dcNE",
			"pid": "579",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 2306519694,
				"twitterUserName": "MarkusTressel"
			}
		},
		"580": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "J\u00fcrgen Trittin (* 25. Juli 1954 in Bremen-Vegesack) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). Er ist Bundestagsabgeordneter und Mitglied im Ausw\u00e4rtigen Ausschuss.",
				"en": "J\u00fcrgen Trittin is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "J\u00fcrgen Trittin",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "580.jpg",
				"pathToThumb": "580_t.jpg"
			},
			"name": "J\u00fcrgen Trittin",
			"party": "GR\u00dcNE",
			"pid": "580",
			"self_bird": "gartenrotschwanz",
			"twittering": {
				"twitterId": 485751594,
				"twitterUserName": "JTrittin"
			}
		},
		"581": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Axel Troost (* 1. September 1954 in Hagen) ist ein deutscher Wirtschaftswissenschaftler und Politiker (Die Linke), der seit dem 3. Juni 2012 auch stellvertretender Vorsitzender seiner Partei ist.",
				"en": "Axel Troost is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Dr. Axel Troost",
			"images": {
				"license": "custom-linke",
				"pathToImage": "581.jpg",
				"pathToThumb": "581_t.jpg"
			},
			"name": "Axel Troost",
			"party": "DIE LINKE",
			"pid": "581",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": "73990900",
				"twitterUserName": "axeltroost"
			}
		},
		"583": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Volker Michael Ullrich (* 14. Oktober 1975 in Illertissen) ist ein deutscher Politiker (CSU).",
				"en": "Volker Michael Ullrich is a German politician. He is a member of the CSU."
			},
			"full_name": "Dr. Volker Ullrich",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "583.jpg",
				"pathToThumb": "583_t.jpg"
			},
			"name": "Volker Ullrich",
			"party": "CSU",
			"pid": "583",
			"self_bird": "mehlschwalbe",
			"twittering": {
				"twitterId": 28446356,
				"twitterUserName": "VolkerUllrich"
			}
		},
		"588": {
			"citizen_bird": "schneeeule",
			"cv": {
				"de": "Julia Verlinden (* 18. Januar 1979 in Bergisch Gladbach) ist eine deutsche Umweltwissenschaftlerin und gew\u00e4hlte Abgeordnete im Deutschen Bundestag (B\u00fcndnis 90/Die Gr\u00fcnen).",
				"en": "Julia Verlinden is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Dr. Julia Verlinden",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "588.jpg",
				"pathToThumb": "588_t.jpg"
			},
			"name": "Julia Verlinden",
			"party": "GR\u00dcNE",
			"pid": "588",
			"self_bird": "buntspecht",
			"twittering": {
				"twitterId": 1475734592,
				"twitterUserName": "julia_verlinden"
			}
		},
		"59": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Helge Reinhold Braun (* 18. Oktober 1972 in Gie\u00dfen) ist ein deutscher Politiker der CDU, direkt gew\u00e4hlter Bundestagsabgeordneter des Wahlkreises Gie\u00dfen und Staatsminister bei der Bundeskanzlerin.",
				"en": "Helge Braun is a German politician. He is a member of the CDU."
			},
			"full_name": "Prof. Dr. Helge Braun",
			"images": {
				"copyright": "Dr. Helge Braun",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "59.jpg",
				"pathToThumb": "59_t.jpg"
			},
			"name": "Helge Braun",
			"party": "CDU",
			"pid": "59",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": 19108071,
				"twitterUserName": "HBraun"
			}
		},
		"592": {
			"citizen_bird": "mauersegler",
			"cv": {
				"de": "Kathrin Vogler (* 29. September 1963 in M\u00fcnchen) ist eine deutsche Friedensaktivistin und Politikerin (Die Linke). Seit Herbst 2009 geh\u00f6rt sie dem 17. Deutschen Bundestag an.",
				"en": "Kathrin Vogler is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Kathrin Vogler",
			"images": {
				"license": "custom-linke",
				"pathToImage": "592.jpg",
				"pathToThumb": "592_t.jpg"
			},
			"name": "Kathrin Vogler",
			"party": "DIE LINKE",
			"pid": "592",
			"self_bird": "heckenbraunelle",
			"twittering": {
				"twitterId": 21287514,
				"twitterUserName": "voglerk"
			}
		},
		"593": {
			"citizen_bird": "dohle",
			"cv": {
				"de": "Ute Vogt (* 3. Oktober 1964 in Heidelberg) ist eine deutsche Politikerin (SPD). Sie war von 2002 bis 2005 parlamentarische Staatssekret\u00e4rin beim Bundesminister des Innern. Von 1999 bis 2009 war sie Landesvorsitzende der SPD Baden-W\u00fcrttemberg. Sie ist Mitglied des Deutschen Bundestages. Seit Dezember 2013 ist sie eine der sieben stellvertretenden Vorsitzenden der SPD-Bundestagsfraktion (Liste hier).",
				"en": "Ute Vogt is a German politician. She is a member of the SPD."
			},
			"full_name": "Ute Vogt",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "593.jpg",
				"pathToThumb": "593_t.jpg"
			},
			"name": "Ute Vogt",
			"party": "SPD",
			"pid": "593",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 106684561,
				"twitterUserName": "UteVogt"
			}
		},
		"594": {
			"citizen_bird": "kiwi",
			"cv": {
				"de": "Sven Volmering (* 15. April 1976 in Rhede) ist ein deutscher Politiker und seit 2013 Bundestagsabgeordneter der CDU.",
				"en": "Sven Volmering is a German politician. He is a member of the CDU."
			},
			"full_name": "Sven Volmering",
			"images": {
				"copyright": "Steffen Pr\u00f6\u00dfdorf",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "594.jpg",
				"pathToThumb": "594_t.jpg"
			},
			"name": "Sven Volmering",
			"party": "CDU",
			"pid": "594",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 332698790,
				"twitterUserName": "SvenVolmering"
			}
		},
		"598": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Johann David Wadephul (* 10. Februar 1963 in Husum) ist ein deutscher Politiker (CDU). Er war von 2005 bis 2009 Vorsitzender der CDU-Fraktion im Landtag von Schleswig-Holstein und ist seitdem Bundestagsabgeordneter.",
				"en": "Johann Wadephul is a German politician. He is a member of the CDU."
			},
			"full_name": "Dr. Johann Wadephul",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "598.jpg",
				"pathToThumb": "598_t.jpg"
			},
			"name": "Johann Wadephul",
			"party": "CDU",
			"pid": "598",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 389682667,
				"twitterUserName": "JoeWadephul"
			}
		},
		"599": {
			"citizen_bird": "rotkehlchen",
			"cv": {
				"de": "Sahra Wagenknecht (geboren am 16. Juli 1969 in Jena, amtlich zun\u00e4chst Sarah Wagenknecht) ist eine deutsche Volkswirtin, Publizistin und Politikerin (PDS, Die Linke).",
				"en": "Sahra Wagenknecht is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Dr. Sahra Wagenknecht",
			"images": {
				"license": "custom-linke",
				"pathToImage": "599.jpg",
				"pathToThumb": "599_t.jpg"
			},
			"name": "Sahra Wagenknecht",
			"party": "DIE LINKE",
			"pid": "599",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": 47375691,
				"twitterUserName": "SWagenknecht"
			}
		},
		"6": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Kerstin Andreae (* 21. Oktober 1968 in Schramberg) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie ist Mitglied des Deutschen Bundestages und seit Ende Februar 2012 eine der f\u00fcnf stellvertretenden Vorsitzenden der Bundestagsfraktion der Gr\u00fcnen.",
				"en": "Kerstin Andreae is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Kerstin Andreae",
			"images": {
				"copyright": "Eva H\u00e4berle",
				"license": "CC-BY-SA-2.0",
				"pathToImage": "6.jpg",
				"pathToThumb": "6_t.jpg"
			},
			"name": "Kerstin Andreae",
			"party": "GR\u00dcNE",
			"pid": "6",
			"self_bird": "bachstelze",
			"twittering": {
				"twitterId": 41548943,
				"twitterUserName": "KerstinAndreae"
			}
		},
		"60": {
			"citizen_bird": "dohle",
			"cv": {
				"de": "Heike Brehmer (* 5. November 1962 in Sta\u00dffurt) ist eine deutsche Politikerin der CDU und ehemalige Landr\u00e4tin des Landkreises Aschersleben-Sta\u00dffurt.",
				"en": "Heike Brehmer is a German politician. She is a member of the CDU."
			},
			"full_name": "Heike Brehmer",
			"images": {
				"copyright": "Foto-Objekt-Design-Koglin",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "60.jpg",
				"pathToThumb": "60_t.jpg"
			},
			"name": "Heike Brehmer",
			"party": "CDU",
			"pid": "60",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 150372237,
				"twitterUserName": "HeikeBrehmerMdB"
			}
		},
		"600": {
			"citizen_bird": "elster",
			"cv": {
				"de": "Doris Wagner (* 28. Januar 1963 in Bremen) ist eine deutsche \u00dcbersetzerin und gew\u00e4hlte Abgeordnete im Deutschen Bundestag (B\u00fcndnis 90/Die Gr\u00fcnen).",
				"en": "Doris Wagner is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Doris Wagner",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "600.jpg",
				"pathToThumb": "600_t.jpg"
			},
			"name": "Doris Wagner",
			"party": "GR\u00dcNE",
			"pid": "600",
			"self_bird": "feldsperling",
			"twittering": {
				"twitterId": 2230052774,
				"twitterUserName": "DorisWagner_MdB"
			}
		},
		"601": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Beate Walter-Rosenheimer (* 20. November 1964 in Wei\u00dfenburg in Bayern) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Als Nachr\u00fcckerin f\u00fcr Christine Scheel ist sie seit dem 16. Januar 2012 Mitglied des Deutschen Bundestages. Bei den Bundestagswahlen am 22. September 2013 wurde sie zudem als Abgeordnete f\u00fcr die 18. Wahlperiode in den Deutschen Bundestag gew\u00e4hlt.",
				"en": "Beate Walter-Rosenheimer is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Beate Walter-Rosenheimer",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "601.jpg",
				"pathToThumb": "601_t.jpg"
			},
			"name": "Beate Walter-Rosenheimer",
			"party": "GR\u00dcNE",
			"pid": "601",
			"self_bird": "hausrotschwanz",
			"twittering": {
				"twitterId": 47049428,
				"twitterUserName": "BeateWaRo"
			}
		},
		"602": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Marco Wanderwitz (* 10. Oktober 1975 in Karl-Marx-Stadt) ist ein deutscher Politiker (CDU). Er ist seit Januar 2014 kultur- und medienpolitischer Sprecher der CDU/CSU-Bundestagsfraktion.",
				"en": "Marco Wanderwitz is a German politician. He is a member of the CDU."
			},
			"full_name": "Marco Wanderwitz",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "602.jpg",
				"pathToThumb": "602_t.jpg"
			},
			"name": "Marco Wanderwitz",
			"party": "CDU",
			"pid": "602",
			"self_bird": "gartenrotschwanz",
			"twittering": {
				"twitterId": 16361044,
				"twitterUserName": "wanderwitz"
			}
		},
		"603": {
			"citizen_bird": "elster",
			"cv": {
				"de": "Nina Warken (* 15. Mai 1979 in Bad Mergentheim als Nina Bender) ist eine deutsche Politikerin (CDU) und Mitglied des Deutschen Bundestages.",
				"en": "Nina Warken is a German politician. She is a member of the CDU."
			},
			"full_name": "Nina Warken",
			"images": {
				"copyright": "Tobias Koch",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "603.jpg",
				"pathToThumb": "603_t.jpg"
			},
			"name": "Nina Warken",
			"party": "CDU",
			"pid": "603",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 65413134,
				"twitterUserName": "ninawarken"
			}
		},
		"604": {
			"citizen_bird": "eichelhaeher",
			"cv": {
				"de": "Halina Wawzyniak (* 17. Juli 1973 in K\u00f6nigs Wusterhausen) ist eine deutsche Rechtsanw\u00e4ltin und Politikerin. Bis Juni 2012 war sie stellvertretende Vorsitzende der Partei Die Linke. Seit dem 3. Bundesparteitag fungiert sie nur noch als Beisitzerin im Vorstand. Seit der Bundestagswahl 2009 ist sie Mitglied des Deutschen Bundestages.",
				"en": "Halina Wawzyniak is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Halina Wawzyniak",
			"images": {
				"license": "custom-linke",
				"pathToImage": "604.jpg",
				"pathToThumb": "604_t.jpg"
			},
			"name": "Halina Wawzyniak",
			"party": "DIE LINKE",
			"pid": "604",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 18866407,
				"twitterUserName": "Halina_Waw"
			}
		},
		"605": {
			"citizen_bird": "kleiber",
			"cv": {
				"de": "Gabriele \u201eGabi\u201c Weber (* 24. M\u00e4rz 1955 in Ebernhahn bei Montabaur, Rheinland-Pfalz) ist eine deutsche Politikerin (SPD) und Abgeordnete des Deutschen Bundestages.",
				"en": "Gabi Weber is a German politician. She is a member of the SPD."
			},
			"full_name": "Gabi Weber",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "605.jpg",
				"pathToThumb": "605_t.jpg"
			},
			"name": "Gabi Weber",
			"party": "SPD",
			"pid": "605",
			"self_bird": "schwanzmeise",
			"twittering": {
				"twitterId": 992808439,
				"twitterUserName": "GabiWeberSPD"
			}
		},
		"606": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Kai Wegner (* 15. September 1972 in Berlin-Spandau) ist ein deutscher Politiker (CDU).",
				"en": "Kai Wegner is a German politician. He is a member of the CDU."
			},
			"full_name": "Kai Wegner",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "606.jpg",
				"pathToThumb": "606_t.jpg"
			},
			"name": "Kai Wegner",
			"party": "CDU",
			"pid": "606",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 39999872,
				"twitterUserName": "kaiwegner"
			}
		},
		"608": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Harald Weinberg (* 13. Februar 1957 in Bad Godesberg) ist ein deutscher Politiker der Partei Die Linke und seit 2009 Mitglied des Deutschen Bundestages.",
				"en": "Harald Weinberg is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Harald Weinberg",
			"images": {
				"license": "custom-linke",
				"pathToImage": "608.jpg",
				"pathToThumb": "608_t.jpg"
			},
			"name": "Harald Weinberg",
			"party": "DIE LINKE",
			"pid": "608",
			"self_bird": "zilpzalp",
			"twittering": {
				"twitterId": 625642009,
				"twitterUserName": "HaraldWeinberg"
			}
		},
		"609": {
			"citizen_bird": "rauchschwalbe",
			"cv": {
				"de": "Marcus Weinberg (* 4. Juni 1967 in Hamburg) ist ein deutscher Politiker (CDU). Seit 2005 ist er Mitglied des Deutschen Bundestages und war von 2011 bis 2015 Landesvorsitzender der CDU Hamburg.",
				"en": "Marcus Weinberg is a German politician. He is a member of the CDU."
			},
			"full_name": "Marcus Weinberg",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "609.jpg",
				"pathToThumb": "609_t.jpg"
			},
			"name": "Marcus Weinberg",
			"party": "CDU",
			"pid": "609",
			"self_bird": "gartenbaumlaeufer",
			"twittering": {
				"twitterId": 884382990,
				"twitterUserName": "MarcusWeinberg"
			}
		},
		"61": {
			"citizen_bird": "gruenfink",
			"cv": {
				"de": "Ralph Brinkhaus (* 15. Juni 1968 in Wiedenbr\u00fcck, Kreis Wiedenbr\u00fcck), ist ein deutscher Politiker (CDU). Er ist seit 2009 Mitglied des Deutschen Bundestages.",
				"en": "Ralph Brinkhaus is a German politician. He is a member of the CDU."
			},
			"full_name": "Ralph Brinkhaus",
			"images": {
				"copyright": "Die Hoffotografen Berlin",
				"license": "CC-BY-SA-3.0",
				"pathToImage": "61.jpg",
				"pathToThumb": "61_t.jpg"
			},
			"name": "Ralph Brinkhaus",
			"party": "CDU",
			"pid": "61",
			"self_bird": "tannenmeise",
			"twittering": {
				"twitterId": 38001527,
				"twitterUserName": "rbrinkhaus"
			}
		},
		"610": {
			"citizen_bird": "gruenfink",
			"cv": {
				"de": "Anja Weisgerber (* 11. M\u00e4rz 1976 in Schweinfurt) ist deutsche Politikerin der CSU. Bei der Bundestagswahl 2013 gewann sie das Direktmandat im Wahlkreis Schweinfurt.",
				"en": "Anja Weisgerber is a German politician. She is a member of the CSU."
			},
			"full_name": "Dr. Anja Weisgerber",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "610.jpg",
				"pathToThumb": "610_t.jpg"
			},
			"name": "Anja Weisgerber",
			"party": "CSU",
			"pid": "610",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 133770069,
				"twitterUserName": "anjaweisgerber"
			}
		},
		"611": {
			"citizen_bird": "saatkraehe",
			"cv": {
				"de": "Peter Wei\u00df (* 12. M\u00e4rz 1956 in Freiburg im Breisgau) ist ein deutscher Politiker (CDU).",
				"en": "Peter Wei\u00df is a German politician. He is a member of the CDU."
			},
			"full_name": "Peter Wei\u00df",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "611.jpg",
				"pathToThumb": "611_t.jpg"
			},
			"name": "Peter Wei\u00df",
			"party": "CDU",
			"pid": "611",
			"self_bird": "tukan",
			"twittering": {
				"twitterId": 54202670,
				"twitterUserName": "PeterWeissMdB"
			}
		},
		"613": {
			"citizen_bird": "rabenkraehe",
			"cv": {
				"de": "Ingo Wellenreuther MdB, (* 16. Dezember 1959 in Karlsruhe) ist ein deutscher Jurist, Politiker (CDU) und Sportfunktion\u00e4r.",
				"en": "Ingo Wellenreuther is a German politician. He is a member of the CDU."
			},
			"full_name": "Ingo Wellenreuther",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "613.jpg",
				"pathToThumb": "613_t.jpg"
			},
			"name": "Ingo Wellenreuther",
			"party": "CDU",
			"pid": "613",
			"self_bird": "gimpel",
			"twittering": {
				"twitterId": 54535670,
				"twitterUserName": "Wellenreuther"
			}
		},
		"614": {
			"citizen_bird": "zaunkoenig",
			"cv": {
				"de": "Karl-Georg Ernst Gottlob Wellmann (* 18. November 1952 in Berlin) ist ein deutscher Politiker (CDU) und Mitglied des Deutschen Bundestages.",
				"en": "Karl-Georg Wellmann is a German politician. He is a member of the CDU."
			},
			"full_name": "Karl-Georg Wellmann",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "614.jpg",
				"pathToThumb": "614_t.jpg"
			},
			"name": "Karl-Georg Wellmann",
			"party": "CDU",
			"pid": "614",
			"self_bird": "kiwi",
			"twittering": {
				"twitterId": 391355501,
				"twitterUserName": "KGWellmann"
			}
		},
		"615": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Marian Wendt (* 9. Juni 1985 in Torgau) ist ein deutscher Politiker (CDU) und Mitglied des Deutschen Bundestages.",
				"en": "Marian Wendt is a German politician. He is a member of the CDU."
			},
			"full_name": "Marian Wendt",
			"images": {
				"copyright": "Sven Teschke",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "615.jpg",
				"pathToThumb": "615_t.jpg"
			},
			"name": "Marian Wendt",
			"party": "CDU",
			"pid": "615",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": 2430137675,
				"twitterUserName": "MdbWendt"
			}
		},
		"616": {
			"citizen_bird": "tukan",
			"cv": {
				"de": "Katrin Werner (* 25. Mai 1973 in Berlin) ist eine deutsche Politikerin der Partei Die Linke.",
				"en": "Katrin Werner is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Katrin Werner",
			"images": {
				"license": "custom-linke",
				"pathToImage": "616.jpg",
				"pathToThumb": "616_t.jpg"
			},
			"name": "Katrin Werner",
			"party": "DIE LINKE",
			"pid": "616",
			"self_bird": "mauersegler",
			"twittering": {
				"twitterId": 224630872,
				"twitterUserName": "Katrin_Werner"
			}
		},
		"62": {
			"citizen_bird": "gimpel",
			"cv": {
				"de": "Agnieszka Brugger geb. Malczak (auch Agnes Brugger; * 8. Februar 1985 in Legnica, Polen) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen) polnischer Herkunft.",
				"en": "Agnieszka Brugger is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Agnieszka Brugger",
			"images": {
				"copyright": "Stefan Kaminski",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "62.jpg",
				"pathToThumb": "62_t.jpg"
			},
			"name": "Agnieszka Brugger",
			"party": "GR\u00dcNE",
			"pid": "62",
			"self_bird": "buntspecht",
			"twittering": {
				"twitterId": 172269309,
				"twitterUserName": "agnieszka_mdb"
			}
		},
		"621": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Andrea Meta Wicklein geb. K\u00f6hler (* 6. M\u00e4rz 1958 in Potsdam-Babelsberg) ist eine deutsche Politikerin (SPD). Sie ist seit 2002 Mitglied des Deutschen Bundestages und dort von 2006 bis 2009 Sprecherin der Arbeitsgruppe Aufbau Ost der SPD-Bundestagsfraktion. Seit 2011 ist sie Beauftragte der SPD-Bundestagsfraktion f\u00fcr den Mittelstand/Freie Berufe.",
				"en": "Andrea Wicklein is a German politician. She is a member of the SPD."
			},
			"full_name": "Andrea Wicklein",
			"images": {
				"copyright": "spdfraktion.de",
				"license": "custom-spd",
				"pathToImage": "621.jpg",
				"pathToThumb": "621_t.jpg"
			},
			"name": "Andrea Wicklein",
			"party": "SPD",
			"pid": "621",
			"self_bird": "stieglitz",
			"twittering": {
				"twitterId": 1148893550,
				"twitterUserName": "AndreaWicklein"
			}
		},
		"622": {
			"citizen_bird": "buchfink",
			"cv": {
				"de": "Annette Widmann-Mauz geb. Widmann (* 13. Juni 1966 in T\u00fcbingen) ist eine deutsche Politikerin (CDU) und seit 2009 parlamentarische Staatssekret\u00e4rin beim Bundesminister f\u00fcr Gesundheit im Kabinett Merkel II und Kabinett Merkel III.",
				"en": "Annette Widmann-Mauz is a German politician. She is a member of the CDU."
			},
			"full_name": "Annette Widmann-Mauz",
			"images": {
				"copyright": "Laurence Chaperon",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "622.jpg",
				"pathToThumb": "622_t.jpg"
			},
			"name": "Annette Widmann-Mauz",
			"party": "CDU",
			"pid": "622",
			"self_bird": "girlitz",
			"twittering": {
				"twitterId": 3351905747,
				"twitterUserName": "AWidmannMauz"
			}
		},
		"626": {
			"citizen_bird": "bachstelze",
			"cv": {
				"de": "Valerie Wilms (* 22. Januar 1954 in Hannover) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen).",
				"en": "Valerie Wilms is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Dr. Valerie Wilms",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "626.jpg",
				"pathToThumb": "626_t.jpg"
			},
			"name": "Valerie Wilms",
			"party": "GR\u00dcNE",
			"pid": "626",
			"self_bird": "schwanzmeise",
			"twittering": {
				"twitterId": 396069600,
				"twitterUserName": "WilmsVal"
			}
		},
		"629": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Dagmar Gabriele W\u00f6hrl (geb. Winkler; * 5. Mai 1954 in Stein bei N\u00fcrnberg) ist eine deutsche Politikerin (CSU), ehemalige Miss Germany und fr\u00fchere parlamentarische Staatssekret\u00e4rin beim Bundesminister f\u00fcr Wirtschaft und Technologie. Sie ist verheiratet mit Hans Rudolf W\u00f6hrl.",
				"en": "Dagmar W\u00f6hrl is a German politician. She is a member of the CSU."
			},
			"full_name": "Dagmar G. W\u00f6hrl",
			"images": {
				"copyright": "Henning Schacht",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "629.jpg",
				"pathToThumb": "629_t.jpg"
			},
			"name": "Dagmar G. W\u00f6hrl",
			"party": "CSU",
			"pid": "629",
			"self_bird": "gimpel",
			"twittering": {
				"twitterId": 28525166,
				"twitterUserName": "DWoehrl"
			}
		},
		"630": {
			"citizen_bird": "ringeltaube",
			"cv": {
				"de": "Waltraud Katharina Wolff geb. Rudolf (* 15. M\u00e4rz 1956 in Wei\u00dfandt-G\u00f6lzau) ist eine deutsche Politikerin (SPD).",
				"en": "Waltraud Wolff is a German politician. She is a member of the SPD."
			},
			"full_name": "Waltraud Wolff",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "630.jpg",
				"pathToThumb": "630_t.jpg"
			},
			"name": "Waltraud Wolff",
			"party": "SPD",
			"pid": "630",
			"self_bird": "star",
			"twittering": {
				"twitterId": 47940273,
				"twitterUserName": "waltraud_wolff"
			}
		},
		"633": {
			"citizen_bird": "gimpel",
			"cv": {
				"de": "J\u00f6rn Heinz Adolf Wunderlich (* 16. Januar 1960 in Gladbeck) ist ein deutscher Politiker (Die Linke).",
				"en": "J\u00f6rn Wunderlich is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "J\u00f6rn Wunderlich",
			"images": {
				"license": "custom-linke",
				"pathToImage": "633.jpg",
				"pathToThumb": "633_t.jpg"
			},
			"name": "J\u00f6rn Wunderlich",
			"party": "DIE LINKE",
			"pid": "633",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": "3748945281",
				"twitterUserName": "jwunderlichbt"
			}
		},
		"636": {
			"citizen_bird": "girlitz",
			"cv": {
				"de": "Tobias Josef Zech (* 9. Juli 1981 in Trostberg) ist ein deutscher Politiker (CSU). Seit 2013 ist er Mitglied des Deutschen Bundestages.",
				"en": "Tobias Zech is a German politician. He is a member of the CSU."
			},
			"full_name": "Tobias Zech",
			"images": {
				"copyright": "Harald Kumpfm\u00fcller",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "636.jpg",
				"pathToThumb": "636_t.jpg"
			},
			"name": "Tobias Zech",
			"party": "CSU",
			"pid": "636",
			"self_bird": "ara",
			"twittering": {
				"twitterId": 1720247887,
				"twitterUserName": "TobiasZech"
			}
		},
		"639": {
			"citizen_bird": "mehlschwalbe",
			"cv": {
				"de": "Dagmar Ziegler (* 28. September 1960 in Leipzig) ist eine deutsche Politikerin (SPD). Sie ist seit 2009 Mitglied des Deutschen Bundestages und seit dem 16. Dezember 2013 Parlamentarische Gesch\u00e4ftsf\u00fchrerin der SPD-Bundestagsfraktion. Zuvor war sie von 2000 bis 2009 in verschiedenen Ressorts Ministerin des Landes Brandenburg.",
				"en": "Dagmar Ziegler is a German politician. She is a member of the SPD."
			},
			"full_name": "Dagmar Ziegler",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "639.jpg",
				"pathToThumb": "639_t.jpg"
			},
			"name": "Dagmar Ziegler",
			"party": "SPD",
			"pid": "639",
			"self_bird": "stieglitz",
			"twittering": {
				"twitterId": 2290961736,
				"twitterUserName": "ZieglerMdB"
			}
		},
		"64": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Christine Buchholz MdB, (* 2. April 1971 in Hamburg) ist eine deutsche Politikerin der Partei Die Linke. Sie z\u00e4hlt zum linken Fl\u00fcgel der Partei.",
				"en": "Christine Buchholz is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Christine Buchholz",
			"images": {
				"copyright": "Christine Buchholz",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "64.jpg",
				"pathToThumb": "64_t.jpg"
			},
			"name": "Christine Buchholz",
			"party": "DIE LINKE",
			"pid": "64",
			"self_bird": "zilpzalp",
			"twittering": {
				"twitterId": 426403057,
				"twitterUserName": "ch_buchholz"
			}
		},
		"640": {
			"citizen_bird": "rotkehlchen",
			"cv": {
				"de": "Stefan Zierke (* 5. Dezember 1970 in Prenzlau) ist ein deutscher Tourismusfachwirt und gew\u00e4hlter Abgeordneter im Deutschen Bundestag (SPD).",
				"en": "Stefan Zierke is a German politician. He is a member of the SPD."
			},
			"full_name": "Stefan Zierke",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "640.jpg",
				"pathToThumb": "640_t.jpg"
			},
			"name": "Stefan Zierke",
			"party": "SPD",
			"pid": "640",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 64220313,
				"twitterUserName": "zierke"
			}
		},
		"641": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Matthias Rainer Zimmer (* 3. Mai 1961 in Marburg) ist ein deutscher Politikwissenschaftler, Publizist und Politiker (CDU).",
				"en": "Matthias Zimmer is a German politician. He is a member of the CDU."
			},
			"full_name": "Prof. Dr. Matthias Zimmer",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "641.jpg",
				"pathToThumb": "641_t.jpg"
			},
			"name": "Matthias Zimmer",
			"party": "CDU",
			"pid": "641",
			"self_bird": "zilpzalp",
			"twittering": {
				"twitterId": 52994606,
				"twitterUserName": "matthiaszimmer"
			}
		},
		"642": {
			"citizen_bird": "haussperling",
			"cv": {
				"de": "Jens Zimmermann (* 9. September 1981 in Gro\u00df-Umstadt) ist ein deutscher Politiker (SPD). Er ist seit 2013 Mitglied des Deutschen Bundestages.",
				"en": "Jens Zimmermann is a German politician. He is a member of the SPD."
			},
			"full_name": "Dr. Jens Zimmermann",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "642.jpg",
				"pathToThumb": "642_t.jpg"
			},
			"name": "Jens Zimmermann",
			"party": "SPD",
			"pid": "642",
			"self_bird": "dohle",
			"twittering": {
				"twitterId": 449619355,
				"twitterUserName": "JensZimmermann1"
			}
		},
		"646": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Gudrun Anna Therese Zollner MdB, (* 21. Juli 1960 in Straubing) ist eine deutsche Politikerin (CSU).",
				"en": "Gudrun Zollner is a German politician. She is a member of the CSU."
			},
			"full_name": "Gudrun Zollner",
			"images": {
				"copyright": "Foto-AG Gymnasium Melle",
				"license": "CC-BY-SA-4.0 int",
				"pathToImage": "646.jpg",
				"pathToThumb": "646_t.jpg"
			},
			"name": "Gudrun Zollner",
			"party": "CSU",
			"pid": "646",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 2573238432,
				"twitterUserName": "GudrunZollner"
			}
		},
		"647": {
			"citizen_bird": "klappergrasmuecke",
			"cv": {
				"de": "Brigitte Zypries (* 16. November 1953 in Kassel) ist eine deutsche Politikerin (SPD), sie war von 2002 bis 2009 Bundesministerin der Justiz in den Kabinetten Schr\u00f6der II und Merkel I und ist seit dem 17.\u00a0Dezember\u00a02013 Parlamentarische Staatssekret\u00e4rin beim Bundesminister f\u00fcr Wirtschaft und Energie. Sie ist in diesem Rahmen zust\u00e4ndig f\u00fcr IT sowie f\u00fcr Luft- und Raumfahrt.",
				"en": "Brigitte Zypries is a German politician. She is a member of the SPD."
			},
			"full_name": "Brigitte Zypries",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "647.jpg",
				"pathToThumb": "647_t.jpg"
			},
			"name": "Brigitte Zypries",
			"party": "SPD",
			"pid": "647",
			"self_bird": "moenchsgrasmuecke",
			"twittering": {
				"twitterId": 574472515,
				"twitterUserName": "brigittezypries"
			}
		},
		"67": {
			"citizen_bird": "gartengrasmuecke",
			"cv": {
				"de": "Marco B\u00fclow (* 14. Juni 1971 in Dortmund) ist ein deutscher Politiker (SPD). Seit 2002 geh\u00f6rt er als direkt gew\u00e4hlter Abgeordneter im Bundestagswahlkreis Dortmund I dem Deutschen Bundestag an. Seine politischen Schwerpunkte sind Transparenz und Lobbyismus sowie Bau- und Umweltpolitik.",
				"en": "Marco B\u00fclow is a German politician. He is a member of the SPD."
			},
			"full_name": "Marco B\u00fclow",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "67.jpg",
				"pathToThumb": "67_t.jpg"
			},
			"name": "Marco B\u00fclow",
			"party": "SPD",
			"pid": "67",
			"self_bird": "schneeeule",
			"twittering": {
				"twitterId": 52119203,
				"twitterUserName": "marcobuelow"
			}
		},
		"68": {
			"citizen_bird": "ara",
			"cv": {
				"de": "Martin Burkert (* 14. Oktober 1964 in W\u00fcrzburg) ist ein deutscher Politiker (SPD).",
				"en": "Martin Burkert is a German politician. He is a member of the SPD."
			},
			"full_name": "Martin Burkert",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "68.jpg",
				"pathToThumb": "68_t.jpg"
			},
			"name": "Martin Burkert",
			"party": "SPD",
			"pid": "68",
			"self_bird": "blaumeise",
			"twittering": {
				"twitterId": "36329898",
				"twitterUserName": "BurkertMartin"
			}
		},
		"7": {
			"citizen_bird": "singdrossel",
			"cv": {
				"de": "Niels Annen (* 6. April 1973 in Hamburg-Eimsb\u00fcttel) ist ein deutscher Politiker (SPD). Er ist seit 2013 wieder Mitglied des Deutschen Bundestages, dem er bereits von 2005 bis 2009 angeh\u00f6rte. Im SPD-Parteivorstand sitzt Annen ohne Unterbrechung seit 2003. Von 2001 bis 2004 war er Bundesvorsitzender der Jusos, der Jugendorganisation der Partei. Er war von 2010 bis 2011 Senior Transatlantic Fellow beim German Marshall Fund in Washington und von 2011 bis 2013 arbeitete er f\u00fcr das Referat Internationale Politikanalyse der Friedrich-Ebert-Stiftung in Berlin.",
				"en": "Niels Annen is a German politician. He is a member of the SPD."
			},
			"full_name": "Niels Annen",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "7.jpg",
				"pathToThumb": "7_t.jpg"
			},
			"name": "Niels Annen",
			"party": "SPD",
			"pid": "7",
			"self_bird": "kohlmeise",
			"twittering": {
				"twitterId": 42401449,
				"twitterUserName": "NielsAnnen"
			}
		},
		"70": {
			"citizen_bird": "star",
			"cv": {
				"de": "Lars Castellucci (* 24. Februar 1974 in Heidelberg) ist ein deutscher Politiker (SPD) und seit 2013 Mitglied des Deutschen Bundestages f\u00fcr den Wahlkreis Rhein-Neckar.",
				"en": "Lars Castellucci is a German politician. He is a member of the SPD."
			},
			"full_name": "Prof. Dr. Lars Castellucci",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "70.jpg",
				"pathToThumb": "70_t.jpg"
			},
			"name": "Lars Castellucci",
			"party": "SPD",
			"pid": "70",
			"self_bird": "tannenmeise",
			"twittering": {
				"twitterId": 17469649,
				"twitterUserName": "larscastellucci"
			}
		},
		"72": {
			"citizen_bird": "rotkehlchen",
			"cv": {
				"de": "J\u00fcrgen Co\u00dfe (* 16. August 1969) ist ein deutscher Politiker (SPD). Er ist seit 1986 Mitglied der SPD. Dort ist er Vorsitzender des Unterbezirkes Steinfurt und geh\u00f6rt zurzeit dem Ortsverein Neuenkirchen an.",
				"en": "J\u00fcrgen Co\u00dfe is a German politician. He is a member of the SPD."
			},
			"full_name": "J\u00fcrgen Co\u00dfe",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "72.jpg",
				"pathToThumb": "72_t.jpg"
			},
			"name": "J\u00fcrgen Co\u00dfe",
			"party": "SPD",
			"pid": "72",
			"self_bird": "rauchschwalbe",
			"twittering": {
				"twitterId": "161395160",
				"twitterUserName": "juergencosse"
			}
		},
		"74": {
			"citizen_bird": "weisskopfseeadler",
			"cv": {
				"de": "Sevim Da\u011fdelen (* 4. September 1975 in Duisburg) ist eine deutsche Politikerin der Linken. Sie ist seit 2005 Mitglied des Deutschen Bundestages.",
				"en": "Sevim Da\u011fdelen is a German politician. She is a member of the DIE LINKE."
			},
			"full_name": "Sevim Da\u011fdelen",
			"images": {
				"license": "custom-linke",
				"pathToImage": "74.jpg",
				"pathToThumb": "74_t.jpg"
			},
			"name": "Sevim Da\u011fdelen",
			"party": "DIE LINKE",
			"pid": "74",
			"self_bird": "grauschnaepper",
			"twittering": {
				"twitterId": 139407967,
				"twitterUserName": "SevimDagdelen"
			}
		},
		"76": {
			"citizen_bird": "zilpzalp",
			"cv": {
				"de": "Diether Dehm (* 3. April 1950 in Frankfurt am Main) ist ein deutscher Musikproduzent, Liedermacher und Politiker (SPD/PDS/Die Linke). Dem deutschen Bundestag geh\u00f6rte er 1994 und seit 2005 an. Der von ihm geschriebene Song 1000 und 1 Nacht war 1984 mehrere Wochen in den Charts. In den 1970er Jahren wurde Dehm vom Ministerium f\u00fcr Staatssicherheit als Inoffizieller Mitarbeiter gef\u00fchrt. Dehm erkl\u00e4rte dazu, dass er unwissentlich als Quelle abgesch\u00f6pft worden sei.",
				"en": "Diether Dehm is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Dr. Diether Dehm",
			"images": {
				"license": "custom-linke",
				"pathToImage": "76.jpg",
				"pathToThumb": "76_t.jpg"
			},
			"name": "Diether Dehm",
			"party": "DIE LINKE",
			"pid": "76",
			"self_bird": "klappergrasmuecke",
			"twittering": {
				"twitterId": 20986770,
				"twitterUserName": "Diether_Dehm"
			}
		},
		"77": {
			"citizen_bird": "tuerkentaube",
			"cv": {
				"de": "Ekin Delig\u00f6z (* 21. April 1971 in Tokat, T\u00fcrkei) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie ist seit 1998 Mitglied des Deutschen Bundestages und seit 2013 Mitglied des Haushaltsausschuss f\u00fcr die Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen.",
				"en": "Ekin Delig\u00f6z is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Ekin Delig\u00f6z",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "77.jpg",
				"pathToThumb": "77_t.jpg"
			},
			"name": "Ekin Delig\u00f6z",
			"party": "GR\u00dcNE",
			"pid": "77",
			"self_bird": "ringeltaube",
			"twittering": {
				"twitterId": 22125770,
				"twitterUserName": "ekindeligoez"
			}
		},
		"78": {
			"citizen_bird": "haussperling",
			"cv": {
				"de": "Karamba Diaby (* 27. November 1961 in Marsassoum, Senegal) ist ein deutscher Politiker (SPD) und seit der Bundestagswahl 2013 Mitglied des Deutschen Bundestages. Er ist promovierter Chemiker und Geo\u00f6kologe.",
				"en": "Karamba Diaby is a German politician. He is a member of the SPD."
			},
			"full_name": "Dr. Karamba Diaby",
			"images": {
				"copyright": "Ute Langkafel",
				"license": "custom-spd",
				"pathToImage": "78.jpg",
				"pathToThumb": "78_t.jpg"
			},
			"name": "Karamba Diaby",
			"party": "SPD",
			"pid": "78",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": 316355056,
				"twitterUserName": "KarambaDiaby"
			}
		},
		"85": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Katja D\u00f6rner (* 18. Februar 1976 in Siegen) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen). Sie ist seit 2009 Mitglied des Deutschen Bundestages und seit 2013 stellvertretende Vorsitzende der Bundestagsfraktion B\u00fcndnis 90/Die Gr\u00fcnen.",
				"en": "Katja D\u00f6rner is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Katja D\u00f6rner",
			"images": {
				"copyright": "Gerd Seidel (User:Rob Irgendwer)",
				"license": "CC-BY-SA-3.0 unported",
				"pathToImage": "85.jpg",
				"pathToThumb": "85_t.jpg"
			},
			"name": "Katja D\u00f6rner",
			"party": "GR\u00dcNE",
			"pid": "85",
			"self_bird": "ringeltaube",
			"twittering": {
				"twitterId": 19738866,
				"twitterUserName": "katjadoerner"
			}
		},
		"88": {
			"citizen_bird": "heckenbraunelle",
			"cv": {
				"de": "Katharina Dr\u00f6ge (* 16. September 1984 in M\u00fcnster) ist eine deutsche Politikerin (B\u00fcndnis 90/Die Gr\u00fcnen) und Abgeordnete im 18. Deutschen Bundestag.",
				"en": "Katharina Dr\u00f6ge is a German politician. She is a member of the GR\u00dcNE."
			},
			"full_name": "Katharina Dr\u00f6ge",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "88.jpg",
				"pathToThumb": "88_t.jpg"
			},
			"name": "Katharina Dr\u00f6ge",
			"party": "GR\u00dcNE",
			"pid": "88",
			"self_bird": "zaunkoenig",
			"twittering": {
				"twitterId": 15722010,
				"twitterUserName": "katdro"
			}
		},
		"89": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Hansj\u00f6rg Durz (* 29. Juli 1971 in Augsburg) ist ein bayerischer Politiker (CSU). Am 22. September 2013 wurde er in den Deutschen Bundestag gew\u00e4hlt. Vorher war er B\u00fcrgermeister der Stadt Neus\u00e4\u00df.",
				"en": "Hansj\u00f6rg Durz is a German politician. He is a member of the CSU."
			},
			"full_name": "Hansj\u00f6rg Durz",
			"images": {
				"copyright": "Marcus Mark",
				"license": "CC-BY-SA-3.0 de",
				"pathToImage": "89.jpg",
				"pathToThumb": "89_t.jpg"
			},
			"name": "Hansj\u00f6rg Durz",
			"party": "CSU",
			"pid": "89",
			"self_bird": "singdrossel",
			"twittering": {
				"twitterId": 1662533040,
				"twitterUserName": "Hansjoerg_Durz"
			}
		},
		"9": {
			"citizen_bird": "feldsperling",
			"cv": {
				"de": "Rainer Arnold (* 21. Juni 1950 in Stuttgart) ist ein deutscher Politiker (SPD). Er ist seit 2002 verteidigungspolitischer Sprecher der SPD-Bundestagsfraktion.",
				"en": "Rainer Arnold is a German politician. He is a member of the SPD."
			},
			"full_name": "Rainer Arnold",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "9.jpg",
				"pathToThumb": "9_t.jpg"
			},
			"name": "Rainer Arnold",
			"party": "SPD",
			"pid": "9",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 56990207,
				"twitterUserName": "rainerarnold"
			}
		},
		"91": {
			"citizen_bird": "fitis",
			"cv": {
				"de": "Harald Ebner (* 8. Juli 1964 in G\u00f6ppingen) ist ein deutscher Politiker (B\u00fcndnis 90/Die Gr\u00fcnen). Er ist seit dem 25. Mai 2011 als Nachr\u00fccker Mitglied des Deutschen Bundestages. Ebner ist Sprecher seiner Fraktion f\u00fcr Gentechnik- und Bio\u00f6konomiepolitik und Mitglied im Ausschuss f\u00fcr Ern\u00e4hrung und Landwirtschaft sowie stellvertretendes Mitglied in den Aussch\u00fcssen f\u00fcr Verkehr und digitale Infrastruktur und f\u00fcr Bildung, Forschung und Technikfolgenabsch\u00e4tzung.",
				"en": "Harald Ebner is a German politician. He is a member of the GR\u00dcNE."
			},
			"full_name": "Harald Ebner",
			"images": {
				"license": "custom-gruene",
				"pathToImage": "91.jpg",
				"pathToThumb": "91_t.jpg"
			},
			"name": "Harald Ebner",
			"party": "GR\u00dcNE",
			"pid": "91",
			"self_bird": "kleiber",
			"twittering": {
				"twitterId": 52431948,
				"twitterUserName": "ebner_sha"
			}
		},
		"95": {
			"citizen_bird": "schwanzmeise",
			"cv": {
				"de": "Michaela Barbara Engelmeier MdB, (fr\u00fcher Engelmeier-Heite, * 10. Oktober 1960 in Hagen) ist eine deutsche Politikerin (SPD), sowie ehemaliges Mitglied der deutschen Judo-Nationalmannschaft der Frauen.",
				"en": "Michaela Engelmeier-Heite is a German politician. She is a member of the SPD."
			},
			"full_name": "Michaela Engelmeier",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "95.jpg",
				"pathToThumb": "95_t.jpg"
			},
			"name": "Michaela Engelmeier",
			"party": "SPD",
			"pid": "95",
			"self_bird": "saatkraehe",
			"twittering": {
				"twitterId": 26713568,
				"twitterUserName": "michaelaengel"
			}
		},
		"97": {
			"citizen_bird": "blaumeise",
			"cv": {
				"de": "Klaus Ernst (* 1. November 1954 in M\u00fcnchen) ist ein deutscher Politiker (Die Linke) und ehemaliger Gewerkschaftsfunktion\u00e4r.",
				"en": "Klaus Ernst is a German politician. He is a member of the DIE LINKE."
			},
			"full_name": "Klaus Ernst",
			"images": {
				"license": "custom-linke",
				"pathToImage": "97.jpg",
				"pathToThumb": "97_t.jpg"
			},
			"name": "Klaus Ernst",
			"party": "DIE LINKE",
			"pid": "97",
			"self_bird": "elster",
			"twittering": {
				"twitterId": 64994645,
				"twitterUserName": "ernst_klaus"
			}
		},
		"98": {
			"citizen_bird": "grauschnaepper",
			"cv": {
				"de": "Petra Ernstberger geb. Rosenberger (* 11. November 1955 in Remscheid) ist eine deutsche Politikerin (SPD).",
				"en": "Petra Ernstberger is a German politician. She is a member of the SPD."
			},
			"full_name": "Petra Ernstberger",
			"images": {
				"copyright": "Susie Knoll",
				"license": "custom-spd",
				"pathToImage": "98.jpg",
				"pathToThumb": "98_t.jpg"
			},
			"name": "Petra Ernstberger",
			"party": "SPD",
			"pid": "98",
			"self_bird": "fitis",
			"twittering": {
				"twitterId": 408506969,
				"twitterUserName": "PErnstberger"
			}
		},
		"99": {
			"citizen_bird": "mauersegler",
			"cv": {
				"de": "Saskia Esken (* 28. August 1961 in Stuttgart) ist eine deutsche Politikerin (SPD). Bei der Bundestagswahl 2013 wurde sie \u00fcber die Landesliste Baden-W\u00fcrttemberg in den Bundestag gew\u00e4hlt.",
				"en": "Saskia Esken is a German politician. She is a member of the SPD."
			},
			"full_name": "Saskia Esken",
			"images": {
				"copyright": "Susie Knoll / Florian J\u00e4nicke",
				"license": "custom-spd",
				"pathToImage": "99.jpg",
				"pathToThumb": "99_t.jpg"
			},
			"name": "Saskia Esken",
			"party": "SPD",
			"pid": "99",
			"self_bird": "buchfink",
			"twittering": {
				"twitterId": 1423622834,
				"twitterUserName": "EskenSaskia"
			}
		},
		"hot": {
			"citizen_bird": "amsel",
			"cv": {
				"de": "Das sind wir",
				"en": "HoT is an ordinary German politician. He is a member of the Gr\u00fcn."
			},
			"images": {
				"pathToImage": "group_greengr\u00fcn.jpg",
				"pathToThumb": "tgroup_greengr\u00fcn.jpg"
			},
			"name": "House Of Tweets",
			"party": "Gr\u00fcn",
			"pid": "hot",
			"self_bird": "amsel",
			"twittering": {
				"twitterId": "4718199753",
				"twitterUserName": "HouseOfTweetsSB"
			}
		}
	}
